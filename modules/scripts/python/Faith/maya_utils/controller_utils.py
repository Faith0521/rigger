...
from . import function_utils as functions
from . import naming_utils as naming
reload(functions)
reload(naming)
    ...
    def create_icon(self,
        ...
        if icon_color:
            if isinstance(icon_color, int):
                cmds.setAttr("%s.overrideEnabled" % cont, True)
                cmds.setAttr("%s.overrideColor" % cont, icon_color)
            else:
                cmds.setAttr("%s.overrideEnabled" % cont, True)
                cmds.setAttr("%s.overrideRGBColors" % cont, True)
                cmds.setAttr("%s.overrideColorRGB" % cont, icon_color[0], icon_color[1], icon_color[2])
            ...
            for obj in objList:
                for attr in attrList:
                    try:
                        ...
                        cmds.error("Error: Cannot set", obj, ".", attr, "as lock=", l, "and keyable=", k, "and channelBox=", cb)
