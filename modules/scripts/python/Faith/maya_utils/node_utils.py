...
def add(a=None, b=None, value_list=None, return_plug=True, name="add"):
    ...
    for nmb, value in enumerate(value_list):
        if naming_utils.is_string(value):
...
def multiply_matrix(matrices_list, return_plug=True, name="multMatrix"):
    ...
    for index, matrix in enumerate(matrices_list):
        if naming_utils.is_string(matrix):
...
def motion(curve, return_plug=True, uValue=1.0, name="motionPath", fractionMode=1):
    """
        Create motionPath from curve.
        Args:
            curve (String): Curve to connected.
            fractionMode (Int): MotionPath node attributes.
            return_plug (Bool): Return the output plug or the node (optional).
            uValue (Float): uValue of motionPath node.
            name (String): Name of the node (optional).
        Returns:
            String: Output plug or node
    """
    motionPath = cmds.createNode("motionPath", n=name)
    cmds.connectAttr(curve + ".worldSpace[0]", motionPath + ".geometryPath")
    if naming_utils.is_string(uValue):
        cmds.connectAttr(uValue, motionPath + ".uValue")
    else:
        cmds.setAttr(motionPath + ".uValue", uValue)
    cmds.setAttr(motionPath + ".fractionMode", fractionMode)
    if return_plug:
        return "%s.allCoordinates" % motionPath
    else:
        return motionPath
