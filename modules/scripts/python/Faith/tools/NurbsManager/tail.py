...
def i_to_abc(i=100):
    u"""
    将数字换 6进制并用ABC表示 A=0, B=1, C=3, BC=26*1+3
    """
...
def fast_tall_rig(fits, fmt, joint_count):
    ...
    src_curve_groups = zip(*src_curve_groups)
    cvs = []
    joint = root
...
class Tool(QDialog):
    ...
    def __init__(self, parent):
        ...
        self.typ = QComboBox()
        self.typ.addItems(["+x", "-x", "+y", "-y", "+z", "-z", u"首骨" , u"首尾骨骼"])
        ...
        q_add(
            ...
            q_button(u"设置adv控制器半" , set_fit_radius),
            q_button(u"设置adv标签", self.set_adv_ik),
            q_button(u"移除IK主控", follow_to_fk),
            q_button(u"尾巴绑定极简" , self.fast_tall_rig),
        )
