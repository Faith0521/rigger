...
def template(prefix, topnode, jnts):
    ...
    mc.pointConstraint(jnts[-2], l2)
    mc.pointConstraint(jnts[-2:], l3)
    mc.pointConstraint(jnts[-1], l4)

    mc.orientConstraint(jnts[0], l0)
    mc.orientConstraint(jnts[0], l1)
    mc.orientConstraint(jnts[-2], l2)
    mc.orientConstraint(jnts[-2], l3)
    ...
    # A
    mc.parentConstraint(jnts[0], ctrls[0] + '_grp', mo=1)
    ...
    if len(jnts) == 4:
        ...
        oc = mc.orientConstraint(jnts[:2], ctrls[2] + '_grp')[0]

    mc.setAttr(oc + '.interpType', 2)
    mc.hide(ctrls[2] + '_grp')

    # D
    mc.pointConstraint(jnts[-2:], ctrls[3] + '_grp')
    mc.orientConstraint(jnts[-2], ctrls[3] + '_grp')

    # E
    mc.parentConstraint(jnts[-2], ctrls[4] + '_grp', mo=1)
    ...
    # clean
    mc.parent(bendyUpJntsGrp, bendyLoJntsGrp, topnode + 'Nox')
    mc.parent(upCrv, loCrv, topnode + 'Nox')
    mc.hide(upCrv)
    mc.hide(loCrv)
    mc.delete(nodes)


def anim(prefix, jnts, twistBridges, midjnt='Elbow', volume=True):
    # create ctrls
    ctrls = []
    ctrls.append(controlTools.create(prefix + 'BendyA_ctrl', snapTo=prefix + 'BendyA_ctrlPrep', useShape=prefix + 'BendyA_ctrlPrep')[-1])
    ctrls.append(controlTools.create(prefix + 'BendyB_ctrl', snapTo=prefix + 'BendyB_ctrlPrep', useShape=prefix + 'BendyB_ctrlPrep')[-1])
    ctrls.append(controlTools.create(prefix + 'BendyC_ctrl', snapTo=prefix + 'BendyC_ctrlPrep', useShape=prefix + 'BendyC_ctrlPrep')[-1])
    ctrls.append(controlTools.create(prefix + 'BendyD_ctrl', snapTo=prefix + 'BendyD_ctrlPrep', useShape=prefix + 'BendyD_ctrlPrep')[-1])
    ctrls.append(controlTools.create(prefix + 'BendyE_ctrl', snapTo=prefix + 'BendyE_ctrlPrep', useShape=prefix + 'BendyE_ctrlPrep')[-1])
    ...
    loMidloc = aboutPublic.snapLoc(jnts[-2], name=prefix + '_loMid_loc')
    mc.hide(upMidloc)
    mc.hide(loMidloc)
    ...
    # D
    mc.delete(mc.pointConstraint(jnts[-2:], ctrls[3] + '_grp'))
    mc.delete(mc.orientConstraint(jnts[-2], ctrls[3] + '_grp'))

    # E
    mc.parentConstraint(jnts[-1], ctrls[4] + '_grp')
    mc.hide(ctrls[4] + '_grp')

    # dyn
    mc.pointConstraint(ctrls[0], upMidloc, ctrls[1] + '_grp', mo=0)
    mc.pointConstraint(loMidloc, ctrls[4], ctrls[3] + '_grp', mo=0)
    ...
    # create elbow smooth
    # ----------------------------------------------------------------------------------------------------------------------------
    ctrlsOri = mc.createNode('transform', n=ctrls[2] + 'Ori', p=ctrls[1])
    ctrlsOff = mc.createNode('transform', p=ctrlsOri, n=ctrls[2] + 'Off')

    mc.pointConstraint(ctrls[1], ctrlsOri, n=ctrlsOri + '_pc')
    mc.pointConstraint(ctrls[2], ctrlsOff, n=ctrlsOff + '_pc')
    mc.aimConstraint(ctrls[3], ctrlsOri, aim=[0, 1, 0], u=[1, 0, 0], wu=[1, 0, 0], wut='objectRotation', wuo=ctrls[2], n=ctrlsOri + '_ac')

    # >> create {0} blend point
    bup = mc.createNode('transform', n=prefix + '_Up{0}_Staic'.format(midjnt), p=ctrlsOff)
    blo = mc.createNode('transform', n=prefix + '_Lo{0}_Staic'.format(midjnt), p=ctrlsOff)
    mc.pointConstraint(ctrls[1], upMidloc, bup, n=bup + '_pc')
    mc.pointConstraint(loMidloc, ctrls[3], blo, n=blo + '_pc')

    sup = mc.duplicate(bup, po=1, n=prefix + '_Up{0}_Smooth'.format(midjnt))[0]
    slo = mc.duplicate(blo, po=1, n=prefix + '_Lo{0}_Smooth'.format(midjnt))[0]
    mc.setAttr(sup + '.tx', 0)
    mc.setAttr(sup + '.tz', 0)
    mc.setAttr(slo + '.tx', 0)
    mc.setAttr(slo + '.tz', 0)
    ...
    blup = mc.duplicate(sup, po=1, n=prefix + '_Up{0}_Blend'.format(midjnt))[0]
    bllo = mc.duplicate(slo, po=1, n=prefix + '_Lo{0}_Blend'.format(midjnt))[0]

    upc = mc.pointConstraint(sup, bup, blup, n=blup + '_pc')[0]
    lpc = mc.pointConstraint(slo, blo, bllo, n=bllo + '_pc')[0]

    mc.addAttr(ctrls[2], ln='___', nn=' ', at='enum', en=' ', k=1)
    mc.addAttr(ctrls[2], ln='smooth', at='double', min=0, max=1, k=1)
    mc.addAttr(ctrls[2], ln='upTangent', at='double', dv=1, min=0, max=10, k=1)
    mc.addAttr(ctrls[2], ln='loTangent', at='double', dv=1, min=0, max=10, k=1)

    mc.connectAttr(ctrls[2] + '.smooth', upc + '.w0')
    mc.connectAttr(ctrls[2] + '.smooth', lpc + '.w0')
    mc.setDrivenKeyframe(upc + '.w1', cd=ctrls[2] + '.smooth', dv=0, v=1)
    mc.setDrivenKeyframe(upc + '.w1', cd=ctrls[2] + '.smooth', dv=1, v=0)
    mc.setDrivenKeyframe(lpc + '.w1', cd=ctrls[2] + '.smooth', dv=0, v=1)
    mc.setDrivenKeyframe(lpc + '.w1', cd=ctrls[2] + '.smooth', dv=1, v=0)

    mc.setDrivenKeyframe(sup + '.ty', cd=ctrls[2] + '.upTangent', dv=1)
    mc.setDrivenKeyframe(sup + '.ty', cd=ctrls[2] + '.upTangent', dv=0, v=0)
    mc.setDrivenKeyframe(sup + '.ty', cd=ctrls[2] + '.upTangent', dv=10, v=mc.getAttr(blup + '.ty') * 30)

    mc.setDrivenKeyframe(slo + '.ty', cd=ctrls[2] + '.loTangent', dv=1)
    mc.setDrivenKeyframe(slo + '.ty', cd=ctrls[2] + '.loTangent', dv=0, v=0)
    mc.setDrivenKeyframe(slo + '.ty', cd=ctrls[2] + '.loTangent', dv=10, v=mc.getAttr(bllo + '.ty') * 30)
    ...
    stBPos = mc.xform(jnts[-2], q=1, t=1, ws=1)
    edBPos = mc.xform(jnts[-1], q=1, t=1, ws=1)
    ...
    # delete two cvs
    needDeleteCv = [crvA_cvs[1], crvA_cvs[-2], crvB_cvs[1], crvB_cvs[-2]]
    ...
    mc.pointConstraint(ctrls[1], ctrls[2], crvALocs[3] + '_grp', mo=1)
    ...
    mc.pointConstraint(ctrls[2], ctrls[3], crvBLocs[1] + '_grp', mo=1)
    mc.pointConstraint(ctrls[3], ctrls[4], crvBLocs[3] + '_grp', mo=1)

    # smooth elbow
    mc.pointConstraint(blup, crvALocs[3], mo=1)
    mc.pointConstraint(bllo, crvBLocs[1], mo=1)

    # create jnts
    # posMpnode = prefix + 'Bendy_pos_grp'
    # jointNum = 5
    ...
    loBikSys = crvSpline.ikSplineByJoints(prefix + '_lb', crvB, lojntlist, 1, strechAxis='tx', twistFix=False, volume=volume, mpNode=None)
    upBjnts = upBikSys['jointList']
    loBjnts = loBikSys['jointList']

    mc.connectAttr(twistBridges[0] + '.twist', upBikSys['ikh'] + '.twist')
    mc.connectAttr(twistBridges[1] + '.twist', loBikSys['ikh'] + '.twist')
    # ------------------------------------------------------------------------------------------------------------------------------------
    ...
    bendyNoxs = [crvLocMpnode, crvA, crvB, upBikSys['rootGrp'], loBikSys['rootGrp']]
    ...
    bjnts = upBjnts[0:-2] + loBjnts[0:-2]
    result = {'rootGrp': ctrlsGrp, 'noTrans': bendyNoxGrp, 'jointList': bjnts, 'upjnts': upBjnts, 'lojnts': loBjnts}
    return result
