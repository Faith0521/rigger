...
# stretchTools.createIkStretch('lf_', 'lf_shoulder', jnts, 'lf_armIk_ctrl', 'lf_elbowIk_ctrl', sticky=True, mpNode=None )
...
def stretchSoftIk(prefix, parent, jnts, ikCtrl, pvCtrl, stretch=True, mpNode=None):
    ...
    if not mc.objExists(cbridge):
        ...
        aboutPublic.addNewAttr(objects=[cbridge], attrName=['soft', 'stretch'], 
                               lock=False, attributeType='float', keyable=True, dv=0, min=0, max=1)

        aboutPublic.addNewAttr(objects=[cbridge], attrName=['upLength', 'midLength', 'loLength'], 
    ...
    if not mc.objExists(vbridge):
        ...
        aboutPublic.addNewAttr(objects=[vbridge], attrName=['orgA', 'orgB', 'orgC'],
                               lock=False, attributeType='float', keyable=False)

        aboutPublic.addNewAttr(objects=[vbridge], attrName=['soft', 'stretch'], 
                               lock=False, attributeType='float', keyable=True, dv=0, min=0, max=1)

        aboutPublic.addNewAttr(objects=[vbridge], attrName=['inA', 'inB', 'inC', 'chain', 'controlDist', 'softDist'], 
    ...
    #>> 'stretch', 'sticky', 'soft', 'upLength', 'midLength'
    aboutPublic.addNewAttr(objects=[ikCtrl], attrName=['soft','stretch'], 
                           lock=False, attributeType='float', keyable=True, dv=0, min=0, max=1)

    aboutPublic.addNewAttr(objects=[ikCtrl], attrName=['upLength', 'midLength', 'loLength'], 
                           lock=False, attributeType='float', keyable=True, dv=1, min=-10, max=10)
    ...
    # >> inB
    inB = abs( mc.getAttr(midBJnt+'.tx') )
    mc.setAttr(vbridge + '.orgB', inB)
    realInB_mdl = mc.createNode('multDoubleLinear', n=prefix+'real_inB_mdl')
    mc.connectAttr(cbridge+'.midLength', realInB_mdl+'.input1')
    mc.setAttr(realInB_mdl+'.input2', inB)
    mc.connectAttr(realInB_mdl+'.output', vbridge+'.inB')

    #>> inC
    inC = abs( mc.getAttr(edJnt+'.tx') )
    mc.setAttr(vbridge + '.orgC', inC)
    realInC_mdl = mc.createNode('multDoubleLinear', n=prefix+'real_inC_mdl')
    mc.connectAttr(cbridge+'.loLength', realInC_mdl+'.input1')
    mc.setAttr(realInC_mdl+'.input2', inC)
    mc.connectAttr(realInC_mdl+'.output', vbridge+'.inC')

    #>> chain
    realChainAB_adl = mc.createNode('addDoubleLinear', n=prefix+'real_chainAB_adl')
    mc.connectAttr(realInA_mdl+'.output', realChainAB_adl+'.input1')
    mc.connectAttr(realInB_mdl+'.output', realChainAB_adl+'.input2')
    realChainBC_adl = mc.createNode('addDoubleLinear', n=prefix+'real_chainBC_adl')
    mc.connectAttr(realChainAB_adl+'.output', realChainBC_adl+'.input1')
    mc.connectAttr(realInC_mdl+'.output', realChainBC_adl+'.input2')
    mc.connectAttr(realChainBC_adl+'.output', vbridge+'.chain')
    ...
    needStretchJnts = [midJnt, midBJnt, edJnt]  
    ...
    if stretch:
        ...
        for i, jnt in enumerate(needStretchJnts):
            ...
            mc.connectAttr(addBoneLen_adl+'.output', side_mdl+'.input1')
            mc.setAttr(side_mdl+'.input2', mirror)
            mc.connectAttr(side_mdl+'.output', jnt+'.tx')

    else:
        mc.pointConstraint(softloc, blendloc, n=blendloc+'_pc')
        mc.deleteAttr(ikCtrl+'.stretch')
        mc.deleteAttr(ikCtrl+'.upLength')
        mc.deleteAttr(ikCtrl+'.midLength')
...
def stretchFk( prefix, FkCtrls, mpNode=None):
    ...
    for i in range(ctrlNum):
        stretchLoc = aboutPublic.snapLoc(FkCtrls[i], name=FkCtrls[i]+'_stretch')
