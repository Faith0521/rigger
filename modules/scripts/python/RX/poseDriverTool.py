...
def multiplyCombine(attrA='', attrB='', toAttr=''):
    ...
    :return:
    """
    combineNode = mc.createNode('multDoubleLinear', n='%s_combine_mdl' % (toAttr.replace('.', '_')), ss=1)
    ...
    input1 = mc.connectionInfo(combineNode + '.input1', sfd=1)
    input2 = mc.connectionInfo(combineNode + '.input2', sfd=1)

    while 'unitConversion' in input1:
        input1 = mc.connectionInfo(input1.replace('output', 'input'), sfd=1)
    while 'unitConversion' in input2:
        input2 = mc.connectionInfo(input2.replace('output', 'input'), sfd=1)
