...
def addShape(crvName, shapeName):

    shapeNode = mc.createNode('nurbsCurve', n=crvName + 'Shape', p=crvName)
    if shapeName:
        datePath = aboutPath.filePath('curve') + shapeName + '.cs'
        readCrvData(datePath)
    return shapeNode

# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

def replaceShape(crvName, shapeName):
    shapes = mc.listRelatives(crvName, f=1, s=1)
    for shape in shapes:
        mc.delete(shape)
    result = addShape(crvName, shapeName)
    return result

# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

def createCrv(crvName, shapeName):
    # name
    crv = mc.createNode('transform', n=crvName)
    # shape
    if shapeName:
        addShape(crv, shapeName)
    return crv
...
def copyShape(nodes=None, copyColor=True, mirror=False):
    ...
    newnode = mc.duplicate(srcCtrl, rc=1)
...
def editCrvHull(crvName, xyzValue=None, modifyType='rotate'):
