...
CgRig provides a framework to build you're own marking menus and node based triggers.
...
inside the cgrig_maya package.::

    cgrig_maya/examples/markingmenus/
...
CgRig discovers commands and layouts when the user first attempts to right click in the viewport on a node
...
2 environment variables to your cgrig_package.json

Note::

    It's possible to reload the marking menu plugin registry manually see :ref:`API Reference<markingmenu-api>`

The 2 environment variables are the below, for our example we'll path to our example folder in our cgrig_package.json

.. code-block:: json

     "CGRIG_MM_COMMAND_PATH": [
            "{self}/examples/markingmenus"
        ],
     "CGRIG_MM_LAYOUT_PATH": [
        "{self}/examples/markingmenus"
     ],
...
Now reload cgrig via our developer shelf or menu.
...
From here we just need to update or add another environment to our cgrig_package.json just like before.
...
Now reload cgrig and this time create a new sphere and select it then run the below.
...
Now update the cgrig_package.json file with the below.

.. code-block:: json

    "CGRIG_TRIGGER_COMMAND_PATH": [
