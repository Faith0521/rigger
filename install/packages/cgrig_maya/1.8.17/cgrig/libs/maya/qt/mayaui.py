...
class BootStrapWidget(MayaQWidgetDockableMixin, QtWidgets.QWidget):
    ...
    def show(self, **kwargs):
        ...
        kwargs[
            "uiScript"] = 'try: import cgrig.libs.maya.qt.mayaui as cgrigmayaui;cgrigmayaui.rebuild("{}")\nexcept ImportError: pass'.format(
            self.objectName())
        # create the close callback string autodesk wacky design decisions again
        kwargs[
            "closeCallback"] = 'try: import cgrig.libs.maya.qt.mayaui as cgrigmayaui;cgrigmayaui.bootstrapDestroyWindow("{}")\nexcept ImportError: pass'.format(
            self.objectName())
