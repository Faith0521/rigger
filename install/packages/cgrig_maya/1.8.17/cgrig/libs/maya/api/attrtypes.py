...
module in conjunction with :mod:`cgrig.libs.maya.api.plugs` and
:func:`cgrig.libs.maya.api.nodes.addAttribute` functions you will have a single entry
...
def mayaNumericTypeToInternalType(mayaType):
    """Returns the cgrig internal attribute type for the maya numeric type
...
def numericTypeToMayaFnType(Type):
    ...
    for the provided cgrig type or None,None.
...
def mayaUnitTypeToInternalType(mayaType):
    """Returns the cgrig internal attribute type for the maya unit attribute type
...
def mayaMFnDataTypeToInternalType(mayaType):
    """Returns the cgrig internal attribute type for the maya TypeAttribute type.
...
def mayaTypeToInternalType(mayaType):
    """Returns the cgrig internal attribute type for the maya attribute type
...
def typeToString(attrType, default=None):
    """Returns the cgrig attribute type as as a string name
    .. example:
...
def mayaTypeFromType(Type):
    """Converts the cgrig attribute constant type to the maya type.

    :param Type: the cgrigType eg. kMFnMessageAttribute
    :type Type: int
...
def mayaTypeToPythonType(mayaType):
    ...
    elif isinstance(mayaType, (om2.MMatrix, om2.MVector, om2.MPoint, om2.MFloatMatrix,
