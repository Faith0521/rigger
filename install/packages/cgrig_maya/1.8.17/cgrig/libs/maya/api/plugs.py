...
def connectPlugs(source, destination, mod=None, force=True, apply=True):
    ...
    if destination.isDestination:
        ...
        if force:
            ...
            raise ValueError(
                "Plug {} has incoming connection {}".format(
                    destination.name(), destinationSource.name()
                )
            )
...
def connectVectorPlugs(
    ...
    if all(connectionValues):
        connectPlugs(
            sourceCompound, destinationCompound, mod=modifier, force=force, apply=apply
        )
        return
    ...
    if childCount < requestLength or sourceCount < requestLength:
        raise ValueError(
            "ConnectionValues arg count is larger then the compound child count"
        )
...
def iterDependencyGraph(
        plug, alternativeName="", depthLimit=256, transverseType="down"
):
...
def serializePlug(plug):
    ...
    if not dynamic:
        ...
        if plug.isDefaultValue():
            ...
            data["children"] = [
                serializePlug(plug.child(i)) for i in range(plug.numChildren())
            ]
        ...
        if plug.isCompound:
            ...
            if minValue is not None:
                data["min"] = getPythonTypeFromValue(attrType, minValue)
            if maxValue is not None:
                data["max"] = getPythonTypeFromValue(attrType, maxValue)
            if softMinValue is not None:
                data["softMin"] = getPythonTypeFromValue(attrType, softMinValue)
            if softMaxValue is not None:
                data["softMax"] = getPythonTypeFromValue(attrType, softMaxValue)
            data["default"] = mayaTypeToPythonType(plugDefault(plug))
...
def serializeConnection(plug):
    ...
    if source:
        sourceN = source.node()
        sourceNPath = (
            om2.MFnDagNode(sourceN).fullPathName()
            if sourceN.hasFn(om2.MFn.kDagNode)
            else om2.MFnDependencyNode(sourceN).name()
        )
    destN = plug.node()
    return {
        ...
        "source": sourceNPath,
        "destination": (
            om2.MFnDagNode(destN).fullPathName()
            if destN.hasFn(om2.MFn.kDagNode)
...
def setAttributeFnDefault(attribute, default):
    ...
    if attribute.hasFn(om2.MFn.kNumericAttribute):
        ...
        attr.default = (
            tuple(default) if Type in attrtypes.mayaNumericMultiTypes else default
        )
        return True
...
def getPlugAndType(plug, ctx=om2.MDGContext.kNormal):
    ...
    if plug.isArray:
        ...
        data = [
            getPlugAndType(plug.elementByPhysicalIndex(i), ctx) for i in range(count)
        ]
...
def getNumericValue(plug, ctx=om2.MDGContext.kNormal):
    ...
    if dataType == om2.MFnNumericData.kBoolean:
        ...
        return (
            attrtypes.kMFnNumeric2Double,
            om2.MFnNumericData(plug.asMObject(ctx)).getData(),
        )
    elif dataType == om2.MFnNumericData.k2Float:
        return (
            attrtypes.kMFnNumeric2Float,
            om2.MFnNumericData(plug.asMObject(ctx)).getData(),
        )
    elif dataType == om2.MFnNumericData.k2Int:
        return (
            attrtypes.kMFnNumeric2Int,
            om2.MFnNumericData(plug.asMObject(ctx)).getData(),
        )
    elif dataType == om2.MFnNumericData.k2Long:
        return (
            attrtypes.kMFnNumeric2Long,
            om2.MFnNumericData(plug.asMObject(ctx)).getData(),
        )
    elif dataType == om2.MFnNumericData.k2Short:
        return (
            attrtypes.kMFnNumeric2Short,
            om2.MFnNumericData(plug.asMObject(ctx)).getData(),
        )
    elif dataType == om2.MFnNumericData.k3Double:
        return attrtypes.kMFnNumeric3Double, om2.MVector(
            om2.MFnNumericData(plug.asMObject(ctx)).getData()
        )
    elif dataType == om2.MFnNumericData.k3Float:
        return attrtypes.kMFnNumeric3Float, om2.MFloatVector(
            om2.MFnNumericData(plug.asMObject(ctx)).getData()
        )
        ...
        return (
            attrtypes.kMFnNumeric3Long,
            om2.MFnNumericData(plug.asMObject(ctx)).getData(),
        )
    elif dataType == om2.MFnNumericData.k3Short:
        return (
            attrtypes.kMFnNumeric3Short,
            om2.MFnNumericData(plug.asMObject(ctx)).getData(),
        )
...
def getTypedValue(plug, ctx=om2.MDGContext.kNormal):
    ...
    #     return attrtypes.kMFnDataFloatArray, om2.MFnFloatArrayData(plug.asMObject()).array()
        ...
        return (
            attrtypes.kMFnDataDoubleArray,
            om2.MFnDoubleArrayData(plug.asMObject(ctx)).array(),
        )
    elif dataType == om2.MFnData.kIntArray:
        return (
            attrtypes.kMFnDataIntArray,
            om2.MFnIntArrayData(plug.asMObject(ctx)).array(),
        )
    elif dataType == om2.MFnData.kPointArray:
        return (
            attrtypes.kMFnDataPointArray,
            om2.MFnPointArrayData(plug.asMObject(ctx)).array(),
        )
    elif dataType == om2.MFnData.kVectorArray:
        return (
            attrtypes.kMFnDataVectorArray,
            om2.MFnVectorArrayData(plug.asMObject(ctx)).array(),
        )
    elif dataType == om2.MFnData.kStringArray:
        return (
            attrtypes.kMFnDataStringArray,
            om2.MFnStringArrayData(plug.asMObject(ctx)).array(),
        )
    elif dataType == om2.MFnData.kMatrixArray:
        return (
            attrtypes.kMFnDataMatrixArray,
            om2.MFnMatrixArrayData(plug.asMObject(ctx)).array(),
        )
    return None, None
...
def setPlugInfoFromDict(plug, **kwargs):
    ...
    if plug.isDynamic:
        ...
        minValue = kwargs.get("min")  # type: float or None
        maxValue = kwargs.get("max")  # type: float or None
        softMin = kwargs.get("softMin")  # type: float or None
        softMax = kwargs.get("softMax")  # type: float or None
        if default is not None:
            ...
            try:
                ...
                logger.error(
                    "Failed to set plug default values: {}".format(plug.name()),
                    exc_info=True,
                    extra={"data": default},
                )
                raise
        if minValue is not None:
            setMin(plug, minValue)
        if maxValue is not None:
            setMax(plug, maxValue)
...
def setPlugValue(plug, value, mod=None, apply=True):
    ...
    if plug.isArray:
        for i in range(len(value)):
            setPlugValue(plug.elementByLogicalIndex(i), value[i], mod=mod)
        return mod
    elif plug.isCompound:
        count = plug.numChildren()
        if count != len(value):
    ...
    if obj.hasFn(om2.MFn.kUnitAttribute):
        ...
        at = attr.numericType()
        if at in (
                om2.MFnNumericData.k2Double,
                om2.MFnNumericData.k2Float,
                om2.MFnNumericData.k2Int,
                om2.MFnNumericData.k2Long,
                om2.MFnNumericData.k2Short,
                om2.MFnNumericData.k3Double,
                om2.MFnNumericData.k3Float,
                om2.MFnNumericData.k3Int,
                om2.MFnNumericData.k3Long,
                om2.MFnNumericData.k3Short,
...
def hasChildPlugByName(parentPlug, childName):
    ...
    for child in iterChildren(parentPlug):
        if childName in child.partialName(
                includeNonMandatoryIndices=True,
                useLongNames=True,
                includeInstancedIndices=True,
        ):
...
def getPythonTypeFromPlugValue(plug, ctx=om2.MDGContext.kNormal):
    ...
    return getPythonTypeFromValue(dataType, value)


def getPythonTypeFromValue(dataType, value):
    types = (
        attrtypes.kMFnDataMatrix,
        attrtypes.kMFnDataFloatArray,
        attrtypes.kMFnDataFloatArray,
        attrtypes.kMFnDataDoubleArray,
        attrtypes.kMFnDataIntArray,
        attrtypes.kMFnDataPointArray,
        ...
        attrtypes.kMFnNumeric2Long,
        attrtypes.kMFnNumeric2Short,
        attrtypes.kMFnNumeric3Double,
        attrtypes.kMFnNumeric3Float,
        attrtypes.kMFnNumeric3Int,
        attrtypes.kMFnNumeric3Long,
        attrtypes.kMFnNumeric3Short,
        attrtypes.kMFnNumeric4Double,
        ...
        for idx, dt in enumerate(dataType):
            if dt == attrtypes.kMFnDataMatrix:
                res.append(tuple(value[idx]))
                    ...
                    attrtypes.kMFnUnitAttributeDistance,
                    attrtypes.kMFnUnitAttributeAngle,
...
def getNextAvailableElement(plug:om2.MPlug):
    """
