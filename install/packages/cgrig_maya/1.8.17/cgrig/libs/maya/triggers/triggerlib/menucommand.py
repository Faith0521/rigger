...
from cgrig.libs.maya import markingmenu
from cgrig.libs.maya.triggers import triggerbase
...
TOP_LEVEL_MENU_NAME = "cgrigTriggerMenu"
...
class TriggerMenuCommand(triggerbase.TriggerCommand):
    ...
    COMMAND_ATTR_NAME = "cgrigTriggerMenuName"
    _OLD_COMMAND_ATTR_NAME = "cgrigTriggerString"
...
def _runCgRigMarkingMenu(selectedNode):

    # get the current menu path so cgrig can clear the existing default items
    # todo: probably should have an option to append instead of clear.
    currentMenu = cmds.setParent(menu=True, query=True)
    # do our menu
    buildTriggerMenu(currentMenu, selectedNode)

def resetMarkingMenuTrigger():
    cmds.callbacks(clearCallbacks=True, owner='CgRigToolsPro')


def setupMarkingMenuTrigger():
    """
    Installs modifications to the dagProcMenu script for the current session
    """
    cmds.callbacks(addCallback=_runCgRigMarkingMenu, hook='addRMBBakingMenuItems', owner='CgRigToolsPro')
