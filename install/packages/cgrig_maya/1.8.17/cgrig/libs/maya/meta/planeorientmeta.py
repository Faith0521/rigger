...
from cgrig.libs.maya.rig import align
from maya import cmds

from cgrig.libs.maya.cmds.rig import controls, controlconstants
from cgrig.libs.maya.cmds.objutils import shapenodes
...
WORLD_UP_VECTOR_FOUND_MSG = "Start Node or EndNode uses a World Up Vector Guide on the component," \
                       "Please use that instead."
MISSING_RIG_WARN_MSG = "Unable to find Hive Rig from node: {}"
...
class CoPlanarMeta(meta.MetaBase):
    ...
    def metaAttributes(self):
        ...
        base.extend([
            {"name": "cgrigCoPlanarRefPlane", "value": None, "Type": zapi.attrtypes.kMFnMessageAttribute},
            {"name": "cgrigCoPlanarStartNode", "value": None, "Type": zapi.attrtypes.kMFnMessageAttribute},
            {"name": "cgrigCoPlanarEndNode", "value": None, "Type": zapi.attrtypes.kMFnMessageAttribute},
            {"name": "cgrigCoPlanarMode", "value": 0, "Type": zapi.attrtypes.kMFnkEnumAttribute,
             "enums": ["Position", "AxisAligned"]},
            {"name": "cgrigCoPlanarPrimaryAxis", "value": 0, "Type": zapi.attrtypes.kMFnkEnumAttribute,
             "enums": ["X", "Y", "Z"]},
            {"name": "cgrigCoPlanarSecondaryAxis", "value": 2, "Type": zapi.attrtypes.kMFnkEnumAttribute,
             "enums": ["X", "Y", "Z"]},
            {"name": "cgrigCoPlanarNegatePrimaryAxis", "value": 0, "Type": zapi.attrtypes.kMFnNumericBoolean},
            {"name": "cgrigCoPlanarNegateSecondaryAxis", "value": 0, "Type": zapi.attrtypes.kMFnNumericBoolean},
            {"name": "cgrigCoPlanarAxisAlignedAxis", "value": 2, "Type": zapi.attrtypes.kMFnkEnumAttribute,
             "enums": ["X", "Y", "Z"]},
            {"name": "cgrigCoPlanarNegateAxisAlignedAxis", "value": 0, "Type": zapi.attrtypes.kMFnNumericBoolean},
            {"name": "cgrigCoplanarPositionSnap", "value": 0, "Type": zapi.attrtypes.kMFnNumericBoolean},
        ])
        return base
    ...
    def createReferencePlane(self, createPlane=True):
        ...
        """
        if createPlane:
            ctrl = zapi.createPolyPlane("OrientPlane_Reference_CgRig", createUVs=False, constructionHistory=False,
            ...
            ctrl = zapi.nodeByName(cmds.group(name="OrientPlane_Reference_CgRig", empty=True))
        # Build arrow cntrl --------------------------------
        arrowCtrl = zapi.nodeByName(createArrowControl("tempCtrlCgRigDelMe", 10.0))
        ...
        self.connectTo("cgrigCoPlanarRefPlane", ctrl)

    def referencePlane(self):
        return self.cgrigCoPlanarRefPlane.sourceNode()
    ...
    def deleteAllReferencePlanesScene(self):
        ...
        allReferencePlanes = cmds.ls("OrientPlane_Reference_CgRig*")
    ...
    def setStartNode(self, jnt, updateRefPlane=True):
        ...
        :rtype:
        """
        self.connectTo("cgrigCoPlanarStartNode", jnt)
        if updateRefPlane:
            ref = self.referencePlane()
            if ref:
    ...
    def setEndNode(self, jnt):
        ...
        :rtype:
        """
        self.connectTo("cgrigCoPlanarEndNode", jnt)
    ...
    def startNode(self):
        return self.cgrigCoPlanarStartNode.sourceNode()

    def endNode(self):
        return self.cgrigCoPlanarEndNode.sourceNode()
        ...
        if hiveapi is not None and hiveapi.Guide.isGuide(endNode) and hiveapi.Guide.isGuide(startNode):
            try:
                ...
                if hiveapi.componentFromNode(startNode).definition.guideLayer.guide(hiveapi.constants.WORLD_UP_VEC_ID):
                    output.displayWarning(WORLD_UP_VECTOR_FOUND_MSG)
                    return []
            # small edge case where the rig is missing or can't be detected #todo: Fix this detection error
                ...
                output.displayWarning(MISSING_RIG_WARN_MSG.format(startNode.name()))
                return []
    ...
    def setMode(self, mode):
        self.cgrigCoPlanarMode.set(mode)

    def mode(self):
        return self.cgrigCoPlanarMode.value()

    def setPositionSnap(self, state):
        self.cgrigCoplanarPositionSnap.set(state)

    def positionSnap(self):
        return self.cgrigCoplanarPositionSnap.value()
    ...
    def setPrimaryAxis(self, state):
        self.cgrigCoPlanarPrimaryAxis.set(state)

    def setSecondaryAxis(self, state):
        self.cgrigCoPlanarSecondaryAxis.set(state)

    def setNegatePrimaryAxis(self, state):
        self.cgrigCoPlanarNegatePrimaryAxis.set(state)

    def setNegateSecondaryAxis(self, state):
        self.cgrigCoPlanarNegateSecondaryAxis.set(state)

    def setNegateAxisAlignedAxis(self, state):
        return self.cgrigCoPlanarNegateAxisAlignedAxis.set(state)

    def setAxisAligned(self, state):
        self.cgrigCoPlanarAxisAlignedAxis.set(state)

    def primaryAxis(self):
        return self.cgrigCoPlanarPrimaryAxis.value()

    def secondaryAxis(self):
        return self.cgrigCoPlanarSecondaryAxis.value()

    def negatePrimaryAxis(self):
        return self.cgrigCoPlanarNegatePrimaryAxis.value()

    def negateSecondaryAxis(self):
        return self.cgrigCoPlanarNegateSecondaryAxis.value()

    def axisAligned(self):
        return self.cgrigCoPlanarAxisAlignedAxis.value()

    def negateAxisAlignedAxis(self):
        return self.cgrigCoPlanarNegateAxisAlignedAxis.value()
