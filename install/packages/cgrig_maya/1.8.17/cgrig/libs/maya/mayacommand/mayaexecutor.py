...
from cgrig.core.util import env, classtypes, zlogging
from cgrig.core.plugin import pluginmanager
from cgrig.libs.utils import output
from maya import cmds
from maya.api import OpenMaya as om2
from cgrig.libs.maya.mayacommand import errors
...
class MayaExecutor(object):
    """Maya Executor class for safely injecting cgrig commands into the maya undo stack via MPXCommands.
    Always call executor.execute() method when executing commands
    """

    def __init__(self, interface=None, registerEnv="CGRIG_COMMAND_LIB"):
        interface = interface or [command.CgRigCommandMaya]
    ...
    def execute(self, commandName, **kwargs):
        """Function to execute CgRig commands which lightly wrap maya MPXCommands.
        Deals with prepping the CgRig plugin with the command instance. Safely opens and closes the undo chunks via
        ...
        try:
            ...
            cmds.cgrigAPIUndo(commandId=cmd.id)
            ...
            if cmd.isUndoable and cmd.useUndoChunk:
                cmds.undoInfo(closeChunk=True, chunkName=cmd.id)
            cmd.stats.finish(tb)
...
def execute(executeId, **kwargs):
    """ Helper to execute cgrig commands by id
