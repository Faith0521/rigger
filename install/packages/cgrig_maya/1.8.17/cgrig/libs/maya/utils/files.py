...
from cgrig.libs.utils import filesystem, profiling, output
from cgrig.core.util import zlogging

logger = zlogging.getLogger(__name__)

FBX_DEFAULT_TAKE_NAME = "Take 001"
...
def exportContext(rootNode):
    ...
    for i in nodes.iterChildren(rootNode, recursive=True):
        ...
        with plugs.setLockedContext(plug):
            if plug.asFloat() == 0:
                plugs.setPlugValue(plug, True)
                changed.append(dp)
    yield
    for i in iter(changed):
        plug = i.findPlug("visibility", False)
        with plugs.setLockedContext(plug):
            plugs.setPlugValue(plug, False)
...
def exportObj(filePath, sceneNode):
    ...
    cmds.file(filePath, force=True, options="groups=0;ptgroups=0;materials=0;smoothing=1;normals=1", type="OBJexport",
...
def exportFbx(filePath, exportNodes, skeletonDefinition=False, constraints=False, **kwargs):
    """Exports selected nodes as an FBX file to the specified file path with various optional settings.

    :param filePath: The absolute file path where the FBX will be saved.
    :type filePath: str
    :param exportNodes: A list or string representing the nodes to be exported. If a string is provided, it is treated as a single node.
    :type exportNodes: list or str
    :param skeletonDefinition: Whether to include skeleton definitions in the export. Default is False.
    :type skeletonDefinition: bool, optional
    :param constraints: Whether to include constraints in the export. Default is False.
    :type constraints: bool, optional
    :param kwargs: Additional optional parameters for controlling export behavior.
    :type kwargs: dict

    **kwargs can include the following options:**

    - **upAxis** (str): The up axis for the FBX export (e.g., "Y", "Z"). Default is the current up axis in the scene.
    - **startFrame** (float): The start frame for animation baking. If not specified, uses the scene's start frame.
    - **endFrame** (float): The end frame for animation baking. If not specified, uses the scene's end frame.
    - **smoothingGroups** (bool): Whether to export smoothing groups. Default is False.
    - **hardEdges** (bool): Whether to export hard edges. Default is False.
    - **tangents** (bool): Whether to export tangents. Default is False.
    - **smoothMesh** (bool): Whether to export smooth meshes. Default is False.
    - **instances** (bool): Whether to export instances. Default is False.
    - **triangulate** (bool): Whether to triangulate the geometry. Default is False.
    - **shapes** (bool): Whether to export shapes. Default is False.
    - **skins** (bool): Whether to export skins. Default is False.
    - **cameras** (bool): Whether to export cameras. Default is False.
    - **lights** (bool): Whether to export lights. Default is False.
    - **textures** (bool): Whether to export embedded textures. Default is False.
    - **inputConnections** (bool): Whether to export input connections. Default is False.
    - **animation** (bool): Whether to bake and export complex animations. Default is False.
    - **resample** (bool): Whether to resample animations. Default is False.
    - **step** (float): The step value for baking complex animations. Default is 1.0.
    - **includeChildren** (bool): Whether to include child nodes in the export. Default is False.
    - **ascii** (bool): Whether to export the file in ASCII format. Default is False.
    - **version** (str): The FBX version to export (e.g., "FBX202000", "FBX201900"). Default is None.
    - **takes** (dict[str, tuple[float, float]]): A dictionary mapping take names to start and end frame ranges.

    .. note::
        When specifying **takes** as a parameter the min/max range must be within the start/end baking range.

    :return: The file path of the exported FBX file.
    :rtype: str
    """
    general.loadPlugin("fbxmaya")
    if isinstance(exportNodes, string_types):
        exportNodes = [exportNodes]
    upAxis = kwargs.get("upAxis", general.upAxis())
    ...
    with exportMultiContext(map(nodes.asMObject, exportNodes)):
        ...
        for takeName, takeRange in kwargs.get("takes", {}).items():
            mel.eval('FBXExportSplitAnimationIntoTakes -v "{takeName}" {startFrame} {endFrame};'.format(
                takeName=takeName,
                startFrame=takeRange[0],
                endFrame=takeRange[1],
            ))
