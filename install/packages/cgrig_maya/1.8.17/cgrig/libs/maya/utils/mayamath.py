...
AXIS_NAMES = ["X", "Y", "Z"]
MIRROR_PLANE_BY_AXIS = {"x": "yz", "y": "xz", "z": "xy"}
MIRROR_PLANE_INDEX_BY_NAME = {"xy": 0, "yz": 1, "xz": 2}
MIRROR_PLANE_AXIS_BY_INDEX = ["xy", "yz", "xz"]
MIRROR_AXIS_BY_PLANE_INDEX = [2, 0, 1]

MIRROR_BEHAVIOUR = 0
MIRROR_RELATIVE = 1
MIRROR_SCALE = 2
MIRROR_ORIENTATION = 3
...
def aimToNode(
    ...
    rot = lookAt(
        eyePivotPos, targetPivotPos, aimVector, upVector, worldUpVector, constrainAxis
    )
    # align the aim
...
def lookAt(
    ...
    try:
        angle = math.acos(cgrigmath.clamp(upRotated * eyeV, minValue=-1.0, maxValue=1.0))
...
def slerp(qa, qb, weight):
    ...
    if cgrigmath.almostEqual(math.fabs(sinhalfTheta), 0.0, 2):
...
def toEulerXYZ(rotMatrix, degrees=False):
    ...
    """
    rotXZ = rotMatrix[2]
    if cgrigmath.almostEqual(rotXZ, 1.0, 2):
        z = math.pi
        y = -math.pi * 0.5
        x = -z + math.atan2(-rotMatrix[4], -rotMatrix[7])
    elif cgrigmath.almostEqual(rotXZ, -1.0, 2):
        z = math.pi
        y = math.pi * 0.5
        x = z + math.atan2(rotMatrix[4], rotMatrix[7])
    else:
...
def mirrorTranslation(transformationMatrix, translateAxis):
    ...
    :type transformationMatrix: :class:`om2.MTransformationMatrix`
    ...
    :rtype: :class:`om2.MTransformationMatrix`
    """
...
def axisNamesToNegativeVector(axisNames, vector):
...
def mirrorMatrix(matrix, translate, rotate, mirrorFunction=MIRROR_BEHAVIOUR):
    """
    ...
    :return: Mirrored Transformation matrix
    :rtype: :class:`om2.MTransformationMatrix`
    """
    transMat = om2.MTransformationMatrix(matrix)
    # behavior
    if mirrorFunction == MIRROR_BEHAVIOUR:
        ...
        if rotate == "xy":
            quaternion = om2.MQuaternion(
                quaternion.y * -1, quaternion.x, quaternion.w, quaternion.z * -1
            )
        elif rotate == "yz":
            quaternion = om2.MQuaternion(
                quaternion.w * -1, quaternion.z, quaternion.y * -1, quaternion.x
            )
        else:
            quaternion = om2.MQuaternion(
                quaternion.z, quaternion.w, quaternion.x * -1, quaternion.y * -1
            )
        transMat = mirrorTranslation(transMat, translate)
        transMat.setRotation(quaternion)
    elif mirrorFunction == MIRROR_RELATIVE:
        ...
        scaleVector = axisNamesToNegativeVector(translate, om2.MVector(originalScale))
        ...
        transMat = om2.MTransformationMatrix(
            transMat.asMatrix() * resultTransform.asMatrix()
        )
        if rotate:
            transMat.setScale(originalScale, om2.MSpace.kWorld)
    elif mirrorFunction == MIRROR_SCALE:
        scaleVector = axisNamesToNegativeVector(
            translate, om2.MVector(1.0, 1.0, 1.0)
        )
        mirrorMat = om2.MMatrix(
            [
                scaleVector[0], 0, 0, 0,
                0, scaleVector[1], 0, 0,
                0, 0, scaleVector[2], 0,
        ...
        transMat = om2.MTransformationMatrix(transMat.asMatrix() * mirrorMat)
    else:
        transMat = mirrorTranslation(transMat, translate)
    return transMat
...
def angleBetween3PointsRadians(a, b, c):
    ...
    :rtype: float
    """
    return (b - a).normalize().angle((c - b).normalize())


def weightedPositionFromTriangle(start, mid, end):
    """Returns the weighted position between the start and end points using a triangle.
    This accounts for the rotation of the start by using a lookat function to generate a Plane normal
    before computing the closest point on the plane.

    :param start: The start vector eg.start of triangle(uprLeg/arm etc)
    :type start: :class:`om2.MVector`
    :param mid: The mid vector eg. middle of triangle(knee/elbow etc)
    :type mid: :class:`om2.MVector`
    :param end: The end vector eg. end of triangle(ankle/wrist etc)
    :type end: :class:`om2.MVector`
    :return: The weighted position between the start and end points and the weight value between 0-1.
    :rtype: tuple[:class:`om2.MVector`, float]
    """
    plane = om2.MPlane()
    worldUpVector = (start - mid).normal()
    # Create a rotation for the plane, that way we can project the mid point to the plane
    outputRotation = lookAt(
        start,
        end,
        om2.MVector(0, 1, 0),
        om2.MVector(0, 0, 1),
    ...
    # create the plane rotation which will be used to compute the plane
    transform = om2.MTransformationMatrix()
    transform.setTranslation(start, om2.MSpace.kWorld)
    transform.setRotation(outputRotation)
    # plane normal
    basis = basisVectorsFromMatrix(transform.asMatrix())[ZAXIS]

    plane.setPlane(basis, -basis * start)
    outputPos = closestPointOnPlane(mid, plane)
    # output the weighted mid-Point and the fraction(0-1) between start and end that
    # the projected point sits on.
    totalLength = (start - end).length()
    midLength = (start - outputPos).length()
    fract = cgrigmath.remap(midLength, 0, totalLength, 0, 1)
    return outputPos, fract
...
def averagePosition(nodes):
    ...
    if count == 0:
        raise ValueError(
            "Invalid number of nodes, must be above 0: provided: {}".format(count)
        )
...
def averageNormalVector(nodes, axis):
    ...
    :type nodes: list[:class:`cgrig.libs.maya.zapi.DagNode`]
...
def axisInDirection(matrix, compareVector, defaultAxis):
    ...
    try:
        dots = (
            compareVector * xPrime.normalize(),
            compareVector * yPrime.normalize(),
            compareVector * zPrime.normalize(),
        )
...
def basisYVectorFromMatrix(matrix):
    ...
    :rtype matrix: :class:`om2.MVector`
    """
    return om2.MVector(matrix[4], matrix[5], matrix[6])


def basisZVectorFromMatrix(matrix):
    ...
    :rtype matrix: :class:`om2.MVector`
    """
    return om2.MVector(matrix[8], matrix[9], matrix[10])


def alignToWorldAxis(
    matrix, rotationAxis=YAXIS, forwardAxis=ZAXIS_VECTOR, defaultAxisDirection=ZAXIS
):
...
def perpendicularAxisFromAlignVectors(aimVector, upVector):
    ...
    :param aimVector: The normalized aimVector
    :type aimVector: :class:`zapi.Vector`
    :param upVector: The normalized upVector
...
def isVectorNegative(vector):
    ...
    :return:
    """
    return any(i < 0 for i in vector)
...
def nonPrimaryAxisNamesFromVector(vector):
    ...
    return {"X": ["Y", "Z"], "Y": ["X", "Z"], "Z": ["X", "Y"]}.get(primaryAxis)
...
def bezierSegments(points):
    ...
    for i in range(len(points)):
        if i == 0:
            continue
        if (i % 3) == 0:
            results.append(points[lastIndex : i + 1])
            lastIndex = i
    return results
