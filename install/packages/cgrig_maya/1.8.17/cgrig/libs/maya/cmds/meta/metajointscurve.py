...
class CgRigJointsCurve(base.MetaBase):
    ...
    def metaAttributes(self):
        ...
        defaults = super(CgRigJointsCurve, self).metaAttributes()
        defaults.extend(CgRigJointsCurve._default_attrs)
        return defaults
...
def deleteSplineJoints(nodes, message=False):
    ...
    :rtype success: bool
    """
    metaNodes = base.findRelatedMetaNodesByClassType(nodes, CgRigJointsCurve.__name__)
    if not metaNodes:
        if message:
            relatedObjs = list(zapi.fullNames(nodes))
            om2.MGlobal.displayWarning("No `{}` meta nodes found related to objects "
                                       "{}".format(CgRigJointsCurve.__name__, relatedObjs))
        return False
