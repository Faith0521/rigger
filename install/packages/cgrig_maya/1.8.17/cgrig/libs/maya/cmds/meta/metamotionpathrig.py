...
CgRig meta nodes use Maya's "network" nodes to track and control multiple objects in node setups.

Build a single motion path rig with a meta node:

.. code-block:: python

    from cgrig.libs.maya.cmds.meta import metamotionpathrig
    metaNode = metamotionpathrig.CgRigMotionPathRig()
...
from cgrig.libs.maya.cmds.objutils import filtertypes, namehandling, attributes, curves, constraints, connections, \
    shapenodes
from cgrig.libs.maya.cmds.rig import controls, motionpaths
from cgrig.libs.maya.cmds.objutils.constraints import MOPATH_PATH_ATTR, MOPATH_FRONTTWIST_ATTR, MOPATH_UPTWIST_ATTR, \
    MOPATH_SIDETWIST_ATTR

META_TYPE = "CgRigMotionPathRig"
...
class CgRigMotionPathRig(base.MetaBase):
    ...
    Found in cgrig.libs.maya.cmds.objutils.constraints
    ...
    def metaAttributes(self):
        ...
        defaults = super(CgRigMotionPathRig, self).metaAttributes()
        defaults.extend(CgRigMotionPathRig._default_attrs)
        return defaults
    ...
    def build(self, obj, curve, group=True, controlObj="", followAxis="z", upAxis="y", worldUpVector=(0, 1, 0),
              worldUpObject="", worldUpType=3, parentConstrain=False, follow=True, message=True, default=0.0):
    ...
    def buildUI(self, obj, curve, group=True, controlObj="", followAxis="z", upAxis="y", worldUpVector=(0, 1, 0),
        ...
        parentConstraint = self.build(obj, curve, group=group, controlObj=controlObj, followAxis=followAxis,
                                      ...
                                      follow=follow, message=message, default=default)
...
def switchCurvesSelection():
    ...
    metaNodes = base.findRelatedMetaNodesByClassType(nodes, CgRigMotionPathRig.__name__)  # the metaNodes
    if not metaNodes:
...
def buildMotionPathRigsSelection(curve="", group=True, followAxis="z", upAxis="y", worldUpVector=(0, 1, 0),
    ...
    """Creates a motion path rigs from the current selection with CgRigMotionPathRig meta nodes.
    ...
    :return metaList: The motionPath node that was created list of cgrig metaNodes
...
def autoCleanEmptyMotionPathMeta(cleanMotionPaths=True):
    ...
    metaNodes = base.findMetaNodesByClassType(CgRigMotionPathRig.__name__)
    for meta in metaNodes:
...
def deleteMotionPathRigs(nodes, message=False):
    ...
    :param nodes: any zapi nodes, can be any node related to motionpath setup
    ...
    :rtype success: bool
    """
    metaNodes = base.findRelatedMetaNodesByClassType(nodes, CgRigMotionPathRig.__name__)
    if not metaNodes:
        if message:
            relatedObjs = list(zapi.fullNames(nodes))
            om2.MGlobal.displayWarning("No `{}` meta nodes found related to objects "
                                       "{}".format(CgRigMotionPathRig.__name__, relatedObjs))
        return False
...
def sceneMetaNodes():
    """Returns all CgRigMotionPathRig meta nodes"""
    return base.findMetaNodesByClassType(CgRigMotionPathRig.__name__)


def selectedMetaNodes():
    """Returns meta nodes of type CgRigMotionPathRig.__name__"""
    selectedNodes = list(zapi.selected())
    return base.findRelatedMetaNodesByClassType(selectedNodes, CgRigMotionPathRig.__name__)
