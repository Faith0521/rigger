...
.. code-block:: python
    ...
    from cgrig.libs.maya.cmds.lighting import lightsmultiarea
...
from cgrig.libs.maya.cmds.lighting.areatypes import areabase, arnoldarea, redshiftarea, rendermanarea, vrayarea, mayaarea
...
def areaInstancer(areaType, areaName="", genAttrDict=None, create=False, ingest=True, suffixName=False, message=False):
    ...
    :return: A cgrig area light instance of the specified light.
...
def areaInstanceFromShape(areaShape, message=True):
    ...
    :return: A cgrig area light instance of the specified light, None if the area light does not exist
    :rtype: :class:`areabase.AreaBase`
    """
    ...
    if areaType not in lgtcn.AREA_TYPES:
        if message:
            output.displayWarning("The node type `{}` is not a CgRig supported area light".format(areaType))
        return None
...
def createAreaInstanceType(areaType, areaName="", genAttrDict=None, suffixName=False, message=True):
    ...
    :return: A cgrig area light instance of the specified light type
    :rtype: :class:`areabase.AreaBase`
    """
...
def createDefaultAreaRenderer(renderer, areaName="", genAttrDict=None, lightGroup=True, select=True, suffixName=False,
    ...
    if lightGroup:
        areaInstance.parentCgRigLightGroup()
...
def createDefaultAreaAutoRenderer(areaName="", genAttrDict=None, lightGroup=True, select=True, suffixName=False,
                                  position="selected", message=True):
    """Creates a default area light from the renderer that's currently loaded in CgRig Tools Pro.
    ...
    :return: A cgrig Area instance of the specified light
    :rtype: :class:`areabase.AreaBase`
    """
...
def areaInstancesFromNodes(nodeTransforms, areaTypes=lgtcn.AREA_TYPES, message=True):
    ...
    :return: A list of cgrig area light instances
...
def areaInstancesFromSelected(areaTypes=lgtcn.AREA_TYPES, message=True):
    """From selected lights return a list of associated cgrig area light instances
...
def areaInstanceRenderer(renderer, message=True):
    ...
    :param message: Report warnings to the user?
    :type message: bool
    :return: A cgrig area light instance or None if none found.
    :rtype: :class:`areabase.AreaBase`
    """
