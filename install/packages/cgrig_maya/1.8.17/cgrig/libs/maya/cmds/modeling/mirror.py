...
from cgrig.libs.maya.cmds.objutils import namehandling, objhandling
from cgrig.libs.utils import output

CGRIG_INST_MIRROR_NETWORK = "cgrigInstMirNetwork"
...
def createMirrorInstanceNetwork(groupMirror, origGroup, mirrorSetupObjName, suffixSide01, suffixSide02):
    ...
    networkNodeName = "{}_{}".format(CGRIG_INST_MIRROR_NETWORK, mirrorSetupObjName)
...
def uninstanceMirrorInstacesAll():
    ...
    :rtype networkNodeList: list(str)
    """
    networkNodeList = cmds.ls("*{}*".format(CGRIG_INST_MIRROR_NETWORK))
    if not networkNodeList:
