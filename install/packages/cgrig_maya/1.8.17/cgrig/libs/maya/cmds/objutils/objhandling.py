...
def uninstanceObj(obj):
    ...
    from cgrig.libs.maya.cmds.objutils.connections import getDestinationSourceAttrs  # can't import `connections` cycle
...
def zeroFrozenTransform(obj, freeze=True, buildLocator=True, parentToWorld=True):
    ...
    if cmds.getAttr("{}.rotatePivot".format(obj))[0] == (0.0, 0.0, 0.0):
        return ""
    if buildLocator:
        loc = cmds.spaceLocator(name="cgrig_mirror_delMe")[0]
