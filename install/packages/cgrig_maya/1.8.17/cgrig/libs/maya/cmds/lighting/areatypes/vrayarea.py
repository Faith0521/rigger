...
.. code-block:: python
    ...
    from cgrig.libs.maya.cmds.lighting.areatypes import vrayarea
...
from cgrig.libs.maya.cmds.lighting import lightconstants, lightingutils as lu
...
class VRayArea(areabase.AreaBase):
    ...
    def applyCurrentSettings(self, applyName=False):
        ...
        self._createCgRigIntExpAttr()  # creates the cgrig tracker attrs
    ...
    def _setIntensityVrayType(self, intensity, exposure, index, normalize=False, unnormalize=False, setCgRigAttrs=True):
        ...
        :param setCgRigAttrs: Also set the internal CgRig attrs in generic units
        :type setCgRigAttrs: bool
        """
        if setCgRigAttrs:
    ...
    def setIntensity(self, intensity):
        ...
        try:
            self.setIntensityPure(intensityVal)  # ignores the cgrig intensity tracker attrs, sets maya directly
        except RuntimeError:  # catch, rarely happens when UI is out of sync with VRay
            output.displayWarning("Setting VRay intensity failed `.intensityMult` likely set past its maximum value")
    ...
    def setShape(self, shape):
        if shape == 2:
            ...
            output.displayWarning("Sphere Area Light for VRay is not yet supported by CgRig Tools")
            return
    ...
    def setNormalize(self, normalize):
        ...
        # Check if normalization has changed for the CgRig custom intensity attribute ----------
        ...
        if newNormalize:
            ...
            intensityGeneric = lu.convertToNonNormalizedIntensity(intensityGeneric,
                                                                  ...
                                                                  scale[0], scale[1], scale[2],
                                                                  shape=lightShapeName)[0]
        # Updates cgrig intensity attr in generic units only, vray handles itself while changing units.
        cmds.setAttr(".".join([self.shapeName(), self.cgrigIntensityAttr]), intensityGeneric)
