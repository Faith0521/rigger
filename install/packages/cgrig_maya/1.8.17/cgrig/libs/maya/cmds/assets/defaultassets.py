# -*- coding: utf-8 -*-
"""Module  related to default internal assets included withing the CgRig Tools Pro scripts folder.
Light Presets, HDRI skydome texture paths, control curves etc.

.. code-block:: python

    from cgrig.libs.maya.cmds.assets import defaultassets
...
from cgrig.libs.maya.cmds.assets import assetsimportexport, assetsconstants
...
from cgrig.libs.maya.cmds.lighting import lightsmultihdri, lightconstants
from cgrig.libs.maya.cmds.shaders import shdmultconstants, shadermultirenderer as shdmult
...
def importCgRigSceneDefault(fullPath, renderer, replaceByType=False):
    """Imports a cgrig scene file with default settings for the current renderer

    :param fullPath: The fullpath to the .cgrigScene file
    :type fullPath: str
    ...
    """
    assetsimportexport.importCgRigSceneAsAsset(fullPath,
                                             renderer,
...
def setCameraResolution(cameraShape="cameraShape1", width=520, height=520, clipPlanes=(0.9, 5000.0), antiAlias=True,
    ...
    if cameraShape:
        # Set Camera -----------------------------
        cameras.setCgRigResolutionGate(cameraShape, resolutionGate=True)
...
def importLightPresetAutoRenderer(internalLightDict=PRESET_DEFAULT, showIbl=False, message=True):
    """Import a light preset from a folder path and image with basic kwargs.  Automatically uses the CgRig set renderer
...
def buildDefaultLightSceneCyc(renderer=RENDERER, lightPresetPath=DEFAULT_LIGHT_PRESET_PATH, message=True):
    ...
    :param renderer: The renderer nicename to set the lights and shaders for
    :type renderer: str
    :param lightPresetPath: The full path to the .cgrigScene file
    :type lightPresetPath:
    :param message: Report messages to the user?
    :type message: bool
    """
    # Import Assets Settings -----------------------------
    importCgRigSceneDefault(os.path.join(MODEL_ASSETS_PATH, ASSET_SHADER_BOT), renderer)  # Shader Bot
    importCgRigSceneDefault(os.path.join(MODEL_ASSETS_PATH, ASSET_LIGHT_SCENE), renderer)  # Light Scene
...
def buildDefaultAssetsSceneCyc(renderer=RENDERER,
                               ...
                               assetCgRigScene=ASSET_CYC_DARK_SCENE,
    ...
    if assetCgRigScene:
        # Import Cyc
        importCgRigSceneDefault(os.path.join(assetDirPath, assetCgRigScene), renderer, replaceByType=replaceByType)
        if darkShader:
            # Add dark shader -----------------------------
            cgrigScenePath = os.path.join(SHADERS_PATH, SHADERS_DARK_BACKGROUND)
            shaderName = "cycStudio_{}".format(shdmultconstants.SHADER_SUFFIX_DICT[renderer])
            shaderType = shdmult.RENDERERSHADERS[renderer][0]
            exportabcshaderlights.setShaderAttrsCgRigScene(cgrigScenePath,
                                                         shaderName,
                                                         shaderType,
                                                         renameToCgRigName=False,
                                                         message=message)
    if buildBot:  # Shader Bot
        importCgRigSceneDefault(os.path.join(MODEL_ASSETS_PATH, ASSET_SHADER_BOT), renderer, replaceByType=replaceByType)
...
def buildDefaultShaderSceneCyc(renderer=RENDERER, message=True):
    ...
    importCgRigSceneDefault(os.path.join(MODEL_ASSETS_PATH, ASSET_CYC_GREY_SCENE), renderer)  # Cyc Grey
    importCgRigSceneDefault(os.path.join(MODEL_ASSETS_PATH, ASSET_SHADER_BOT), renderer)  # Shader Bot
    # Light Preset -----------------------------
    importLightPreset(renderer=renderer, lightPresetPath=DEFAULT_LIGHT_PRESET_PATH, hdrImage=HDR_F_PUMPS_BW)
    # Set Camera, Resolution and Globals -----------------------------
    ...
    cgrigScenePath = os.path.join(SHADERS_PATH, SHADERS_SKIN_DARK_BACKLIT)
