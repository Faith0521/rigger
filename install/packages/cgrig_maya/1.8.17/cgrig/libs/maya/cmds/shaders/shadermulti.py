...
from cgrig.libs.maya.cmds.shaders import shdmultconstants as shdcn
from cgrig.libs.maya.cmds.shaders.shadertypes import openpbrsurface, arnoldstandardsurface, mayablinn, mayalambert, \
...
def shaderInstance(shaderType, shaderName="", create=False, ingest=True, suffixName=False):
    ...
    :return shaderInstance: A cgrig shader instance of the specified shader
    ...
    elif shaderType == openpbrsurface.NODE_TYPE:
        return openpbrsurface.OpenPBRSurface(shaderName=shaderName, create=create, ingest=ingest,
                                             suffixName=suffixName)
...
def shaderInstanceFromShader(shaderName, suffixName=False):
    ...
    :param suffixName: Add an autosuffix for the shader name?  Example shader_STRD
    :type suffixName: bool
    :return shaderInstance: A cgrig shader instance of the specified shader, None if the shader does not exist
    :rtype shaderInstance: :class:`shaderbase.ShaderBase`
    """
...
def createDefaultShadRendererAuto(shaderName="", suffixName=False, assignGeo=True, message=False):
    """Creates a default shader and assigns to geo of the currently loaded renderer in CgRig Tools Pro
...
def shaderInstancesFromNodes(nodes):
    """From shaders geometry or shading groups return a list of associated cgrig shader instances

    :param nodes: Maya nodes, geometry, shaders or shading groups
    :type nodes: list(str)

    :return shaderInstances: A list of cgrig shader instances
...
def shaderInstancesFromSelected(message=True):
    """From selected shaders geometry or shading groups return a list of associated cgrig shader instances
...
def copyShaderSelected(message=True):
    """Copies a shaders name and cgrig supported attributes to a singleton class:

        shaderTrackerInst = shaderutils.CgRigShaderTrackerSingleton()
    ...
    if not shaderInstances:  # no shaders found
        ...
        return dict(), ""
    shaderDataSingleton = shaderutils.CgRigShaderTrackerSingleton()  # tracks shader data in cgrig session
...
def pasteShaderAttrsSelected(message=True):
    ...
    Shader data is from a dictionary with cgrig supported attributes.  Shaders remain with tweaked values.
