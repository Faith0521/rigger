...
from cgrig.libs.maya.cmds.objutils import namehandling, attributes
from cgrig.libs.maya.cmds.display import viewportmodes
from cgrig.libs.maya.cmds.math import mayadag
from cgrig.libs.maya.cmds.objutils import moveorient, mesh, curves
...
def jointChains(jointList, ignoreConnectedJoints=True, type="joint"):
    ...
    # Joint string names must be long and in the same string format for comparisons
    zapiJoints = list(zapi.nodesByNames(jointList))
    jointList = zapi.fullNames(zapiJoints)
...
def displayLocalRotationAxisJointList(jntList, children=True, display=True, message=True):
    ...
    if not jntList:
        ...
        return
    if children:
        newJntList = filterChildJointList(jntList)  # adds the children
    else:
        newJntList = list(jntList)
...
def alignJointCgRig(jointList, primaryAxisVector=(1.0, 0.0, 0.0), secondaryAxisVector=(0.0, 1.0, 0.0),
                  worldUpAxisVector=(0.0, 1.0, 0.0), orientChildren=False, ignoreConnectedJoints=True,
                  freezeJoints=True, message=True):
    """CgRig Tools Pro align joint function, aligns the joints given a joint list.
    ...
    if orientChildren:  # orient all children, orient nodes command can be done in one go. ---------------------
        ...
        if newJoints:
            operateJoints += list(newJoints)
    # Don't include children, needs to calculate on straight chains ---------------------
    jointChainList, ignoreJoints = jointChains(list(operateJoints), ignoreConnectedJoints=ignoreConnectedJoints)
    for chainList in jointChainList:
        ...
        mayacommands.orientNodes(nodes, primaryAxisMVector, secondaryAxisMVector, worldUpAxisMVector, skipEnd=skipEnd)
        success = True
    ...
    if message:
        if ignoreJoints and success:
            ...
            output.displayInfo("Success: Joints Aligned: {}".format(operateJoints))
        elif not success and ignoreJoints:
            output.displayWarning("No joints aligned. Joints have connections, are locked or keyframes.")


def alignJointCgRigSel(primaryAxisVector=(1.0, 0.0, 0.0), secondaryAxisVector=(0.0, 1.0, 0.0),
                     worldUpAxisVector=(0.0, 1.0, 0.0), orientChildren=False, ignoreConnectedJoints=True,
                     freezeJoints=True, message=True):
    """CgRig Tools Pro align joint function, aligns the selected joints.
    Unparents orients and reparents and gives more options than regular maya orient joint tool and cmds.
    ...
    if not selJnts:
        output.displayWarning("No joints selected. Please select some joints.")
        return
    alignJointCgRig(selJnts, primaryAxisVector, secondaryAxisVector, worldUpAxisVector, orientChildren,
...
def alignJointPlaneOrient(jointList, metaNode, orientChildren, ignoreConnectedJoints=True, freezeJoints=True,
    ...
    :type jointList: list(str)
    :param metaNode: The cgrig object - plane orient meta node
    :type metaNode: :class:`cgrig.libs.maya.meta.planeorientmeta.CoPlanarMeta`
    :param orientChildren: Orient all child joints?
    :type orientChildren: bool
    :param ignoreConnectedJoints: skip joints that are connected, have constraints, locked or keyframes.
    :type ignoreConnectedJoints: bool
    ...
    for chainList in jointChainList:
        ...
        mayacommands.coPlanarAlign(create=False, align=True, metaNode=metaNode, skipEnd=skipEnd)
        success = True
