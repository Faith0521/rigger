...
from cgrig.libs.maya.cmds.objutils import objhandling, namehandling
from cgrig.libs.maya.cmds.renderer import exportabcshaderlights
from cgrig.libs.maya.cmds.rig import nodes, connections
...
PRESETSUFFIX = ["cgrigScene"]  # the file type recognised as an package (asset)
...
CGRIGSCENEGRPNAME = "cgrigPackage_grp"  # the name of the grp for importing assets
ASSETSUFFIX = "cgrigAssetNetwork"
...
def createAssetGrps(assetName):
    ...
    :type assetName: str
    :return assetGrpLongName: the long name of the asset group, no need to pass the other group as its CGRIGSCENEGRPNAME
    ...
    if not cmds.objExists(CGRIGSCENEGRPNAME):  # create the scene grp
        cmds.group(em=True, name=CGRIGSCENEGRPNAME)
    assetGrpName = cmds.group(em=True, name=assetGrpName)
    assetGrpName = cmds.parent(assetGrpName, CGRIGSCENEGRPNAME)
...
def createAssetFromSelection(newAssetName, message=True):
    ...
    copies it to the CGRIGSCENEGRPNAME
...
def deleteAssetNodeList(cgrigAssetNodeList):
    """For the Maya scene deletes multiple package asset nodes and their connections

    :param cgrigAssetNodeList: a list of asset nodes who's connections need to be deleted
    :type cgrigAssetNodeList:
    """
    for assetNode in cgrigAssetNodeList:
    ...
    if cmds.objExists("|{}".format(CGRIGSCENEGRPNAME)):  # "|cgrigAsset_grp"
        if not cmds.listRelatives("|{}".format(CGRIGSCENEGRPNAME), children=True):  # no children so delete it too
            cmds.delete("|{}".format(CGRIGSCENEGRPNAME))


def deleteCgRigAssetObjList(objList):
...
def selectCgRigAssetGrps(message=True, uiSelectedName=""):
...
def deleteCgRigAssetSelected(uiSelectedName="", message=True):
    ...
    if not assetGrps:
        return
    assetNodes = deleteCgRigAssetObjList(assetGrps)
    if not assetNodes:
        if message:
...
def deleteCgRigAssets(onlyInAssetsGrp=False):
    ...
    """
    cgrigAssetNodeList = cmds.ls("*_{}".format(ASSETSUFFIX))
    if not cgrigAssetNodeList:
        return
    ...
    if onlyInAssetsGrp:
        ...
        cgrigAssetNodeList = parentAssetNodeList
    deleteAssetNodeList(cgrigAssetNodeList)
    return cgrigAssetNodeList
...
def getAssetTypeFile(filePathCgRigScene):
    """gets the asset type from a cgrigScene file, will auto find the .cgrigInfo file

    :param filePathCgRigScene:  full path to the cgrigscene file
    ...
    :rtype assetType: str
    """
    infoDict, fileFound = cgrigscenefiles.getCgRigInfoFromFile(filePathCgRigScene, message=False)
    if infoDict:
...
def getAllAssetNamesInTheScene():
    ...
    :return cgrigAssetNames: current asset names in the scene
    :rtype cgrigAssetNames: list
    """
    cgrigAssetNames = list()
    cgrigAssetNodeList = cmds.ls("*_{}".format(ASSETSUFFIX))
    if not cgrigAssetNodeList:
        return cgrigAssetNames
    assetGrpList = getAssetGroup(cgrigAssetNodeList)
    if not assetGrpList:
        return cgrigAssetNames
    for assetGrp in assetGrpList:
        if assetGrp.endswith(ASSETGRP):
            assetName = assetGrp[:-len(ASSETGRP) - 1]  # remove "_asset_grp"
            cgrigAssetNames.append(namehandling.getShortName(assetName))
    return cgrigAssetNames
...
def importCgRigSceneNoParent(filePathCgRigScene, rendererNiceName, assetName, importShaders=True, importLights=True,
    ...
    :rtype: str
    """
    allNodes = exportabcshaderlights.importAbcGenericShaderLights(filePathCgRigScene,
...
def importCgRigSceneAsAsset(filePathCgRigScene, rendererNiceName, replaceAssets=True, importAbc=True, importShaders=True,
    ...
    """Imports a .cgrig scene with checks and replace options, and auto group creation for cgrigAssets.
    ...
    :rtype: list
    """
    assetType = getAssetTypeFile(filePathCgRigScene)
    if replaceByType:
        ...
        deleteCgRigAssets(onlyInAssetsGrp=False)
    fileName = os.path.basename(filePathCgRigScene)
    ...
    assetGrpName = '_'.join([assetName, ASSETGRP])
    assetGrpLong = "|{}|{}".format(CGRIGSCENEGRPNAME, assetGrpName)
    if not cmds.objExists(CGRIGSCENEGRPNAME):  # create the assets grp
        cmds.group(em=True, name=CGRIGSCENEGRPNAME)
    if not cmds.objExists(assetGrpLong):  # create the asset grp
        assetGrpNameNew = cmds.group(em=True, name=CGRIGSCENEGRPNAME)
        assetGrpNameNew = cmds.parent(assetGrpNameNew, CGRIGSCENEGRPNAME)
        if assetGrpNameNew != assetGrpName:
            assetGrpLong = cmds.rename(assetGrpNameNew, assetGrpName)
    allNodes, networkNodeName = importCgRigSceneNoParent(filePathCgRigScene, rendererNiceName, assetName,
...
def getAssetRoots():
    ...
    :return: A list of asset root objects, cgrigAsset_grp and "\*_asset_grp" with long names.
    :rtype: list[str]
    """
    assetRootList = list()
    if cmds.objExists(CGRIGSCENEGRPNAME):
        assetRootList.append(CGRIGSCENEGRPNAME)
    cgrigAssetNodeList = cmds.ls("*_{}".format(ASSETSUFFIX))
    if not cgrigAssetNodeList:
        return assetRootList
    for cgrigAssetNode in cgrigAssetNodeList:
        assetGrp = cmds.listConnections('.'.join([cgrigAssetNode, ASSETROOTATTR]))
...
def removeAssetRootsFromAlembicRoots(alembicRoots):
    ...
    The groups are the cgrigAssets grp and each asset grp
...
def saveAsset(fullFilePath, rendererNiceName, exportSelected=False, exportShaders=True,
    ...
    exportSelected = True
    cgrigSceneFullPath, filePathAlembic = exportabcshaderlights.exportAbcGenericShaderLights(fullFilePath,
                                                                                           rendererNiceName=rendererNiceName,
...
def getAssetPresetDict(filename=ASSETFILEPATHPRESETS):
    """Retrieves the .cgrigScene preset dictionary from the .json file
    The preset dictionary only contains the file paths of the .cgrigScene assets
...
def writeAssetPresetDict(presetAssetFilePathDict):
    """Writes the selected asset to a cgrigScene .cgrigScene file and also Alembic .abc
...
def deleteQuickDirCgRigScene(filePath):
    """Deletes a cgrigscene file

    :return filesFullPathDeleted: a list of the files deleted full file path
    :rtype filesFullPathDeleted: list
    """
    return cgrigscenefiles.deleteCgRigSceneFiles(filePath)
