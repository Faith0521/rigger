...
from cgrig.libs.maya.cmds.objutils import nodemultibase, matching, namehandling
from cgrig.libs.maya.cmds.cameras import cameras
from cgrig.libs.maya.cmds.lighting.lightconstants import LIGHTS_GROUP_NAME
...
class LightsBase(nodemultibase.NodeMultiBase):
    ...
    def matchTo(self, position="selected"):  # or "camera"
        ...
        if position == "selected":  # create by matching to the first selected object
            ...
            if cam:
                matching.matchCgRigAlSimpErrConstrain(cam, self.node.fullPathName())
            else:
    ...
    def parentCgRigLightGroup(self):
