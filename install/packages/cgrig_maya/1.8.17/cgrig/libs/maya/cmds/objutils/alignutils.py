...
def matchObjTransRot(alignObj, srcObj, message=True):
    ...
    try:  # 2017 and above match
        ...
        if message:
            om2.MGlobal.displayInfo('note: `{}` object cgrig simple align failed, '
                                    'using constraint instead'.format(srcObj))
        logger.debug('note: `{}` object cgrig simple align failed, using constraint instead'.format(srcObj))
...
def placeObjectOnGround(obj, forcePivot=False):
    ...
    :type obj: str
    :param forcePivot: Force the pivot to the ground not the lowest point?
    :type forcePivot: bool
    """
    if forcePivot:  # ignores the mesh or lowest point, just moves the pivot down to the ground
        cmds.move(0, 0, 0, obj, worldSpace=True, worldSpaceDistance=True, moveY=True)
        return
    ...
    planeObj = cmds.polyPlane(name="tempCgRigXXX_planeMatch")[0]
    cmds.align([obj, planeObj], alignToLead=True, yAxis="Min")
    cmds.delete(planeObj)


def placeObjectOnGroundList(objList, forcePivot=False):
    ...
    """
    for obj in objList:
        placeObjectOnGround(obj, forcePivot=forcePivot)
