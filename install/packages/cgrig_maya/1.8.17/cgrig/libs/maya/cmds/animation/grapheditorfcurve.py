...
from cgrig.libs.maya.cmds.animation import grapheditorfcurve
...
from cgrig.libs.maya.cmds.animation import animobjects, timerange, keyframes
...
def selectGraphKeysInRange(start, end):
    """Selects all keys in the graph editor that are within the specified frame range"""
    animCurves = keyframes.animCurves(operationMode="selectedObjs", currentAnimLayer=True)
    cmds.selectKey(animCurves, clear=True)
    # Loop through each animation curve and select keys in the specified frame range
    cmds.selectKey(animCurves, time=(start, end), add=True)


def selectTimelineSelectionInGraph(ignoreEndFrame=True, timeControl=None):
    """Selects the keys in the graph editor that are currently selected in the timeline

    TimeControl is optional, if not specified it will use mayas global timeline.

    :param ignoreEndFrame: If ignore the end frame of the timeline, ie the last key [17.0, 122.0] frm 122.0 is ignored
    :type: str
    :param timeControl: The timeControl widget name. ie. "TimeSlider|MainTimeSliderLayout|formLayout8|frameLayout2|timeControl1"
    :type: str
    """
    timeRange = timerange.getSelectedFrameRange(timeControl=timeControl)
    if ignoreEndFrame:
        timeRange[1] -= 1
    selectGraphKeysInRange(timeRange[0], timeRange[1])
...
def animHold(reverse=False, message=True):
    ...
    for i, curve in enumerate(selCurves):
        ...
        if isLastKey == 1:  # then for this curve then there is a key on the frame 'lastKey'
            ...
            if not reverse:
                ...
                cmds.keyTangent(curve, time=(nextKey, nextKey), inTangentType="auto", outTangentType="auto")
                cmds.copyKey(curve, time=(nextKey, nextKey))
                cmds.pasteKey(curve, time=(lastKey, lastKey))
                cmds.keyTangent(curve, time=(lastKey, lastKey), inTangentType="auto", outTangentType="auto")
            # Report messages
            success = True
