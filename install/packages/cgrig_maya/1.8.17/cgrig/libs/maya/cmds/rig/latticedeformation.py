...
.. code-block:: python
    ...
    from cgrig.libs.maya.cmds.rig import latticedeformation
...
from cgrig.libs.hive.base.hivenodes import layers

MAYA_VERSION = float(mayaenv.mayaVersionNiceName())


def createMultNodeZapi(name):
    if MAYA_VERSION < 2026:
        return zapi.createDG(name, "multDoubleLinear")
    else:
        return zapi.createDG(name, "multDL")
...
class ThreeJointSquashStretchRig(object):
    ...
    def createGDNodes(self):
        ...
        # 6 multiply DoubleLinear ---------------
        # autoCurveX ( multiply DoubleLinear ) squash_C0_end_CTL > multDoubleLinear1
        self.autoCurveX = createMultNodeZapi("{}_autoCrvSpcMidX_mDblLin".format(self.rigName))
        # autoCurveZ ( multiply DoubleLinear ) squash_C0_end_CTL > multDoubleLinear2
        self.autoCurveZ = createMultNodeZapi("{}_autoCrvSpcMidZ_mDblLin".format(self.rigName))
        # headSquash_halfRelativeDistance ( multiply DoubleLinear ) headSquash_relativeDistance > squash_C0_mid_space1
        self.headSquash_halfRelativeDistance = createMultNodeZapi("{}_halfDstnce_mDblLin".format(self.rigName))
        # headSquash_relativeDistance ( multiply DoubleLinear ) headSquash_localInverseScale & distanceBetween1 > headSquash_halfRelativeDistance
        self.headSquash_relativeDistance = createMultNodeZapi("{}_dstnce_mDblLin".format(self.rigName))
        # multDoubleLinear1 ( multiply DoubleLinear ) autoCurveX > squash_C0_mid_space1
        self.multDoubleLinear1 = createMultNodeZapi("{}_autoXHalf_mDblLin".format(self.rigName))
        # multDoubleLinear2 ( multiply DoubleLinear ) autoCurveZ > squash_C0_mid_space1
        self.multDoubleLinear2 = createMultNodeZapi("{}_autoZHalf_mDblLin".format(self.rigName))
