...
def stringNameIsUnique(nodeName):
    """Checks to see if a short string names is unique in the scene, returns a bool

    Note: Will also return True if doesn't exist.  Use cmds.objExists()

    :param nodeName: string name of a Maya node, usually DAG node
    :type nodeName: str
    :return: True if unique, False if not
    :rtype: bool
    """
    if len(cmds.ls(nodeName)) > 1:
        return False
    return True
...
def forceRenameListSel(forceText, niceNameType, padding=2, renameShape=True, hierarchy=False, message=False):
    ...
    objList = renumberListSingleName("cgrigTempXXX", selObjs, padding=padding, renameShape=renameShape, message=message)
...
def renameNamespaceSuffixPrefix(name, namespace, prefix, suffix):
    ...
    :type namespace: str
    :param prefix: Adds a prefix to the object name eg. prefix\_ for prefix_pCube1
    :type prefix: str
    ...
    if not name:
        return ""
    longPrefix, existingNamespace, pureName = mayaNamePartTypes(name)  # break apart name "x|x:name"
    if prefix:
    ...
    if namespace:
        if not namespace.endswith(":"):
            ...
            pureName = "".join([namespace, pureName])
    return mayaNamePartsJoin(longPrefix, existingNamespace, pureName)  # rebuild name "x|x:name"

def rightNameFromLeft(name, leftId, rightId, LRIsPrefix, LRIsSuffix, LRSeparatorOnBorder):
    ...
    :param LRSeparatorOnBorder: Underscores are on either side of the identifier eg. "L" will be "_L" or "L\_"
    :type LRSeparatorOnBorder: bool
    :return: The name of the right side, if not found returns an empty string.
    :rtype: str
    """
    pureNameR = ""
    longPrefix, namespace, pureName = mayaNamePartTypes(name)  # break apart name "x|x:name"
    ...
    if not pureNameR:
        return ""
    return mayaNamePartsJoin(longPrefix, namespace, pureNameR)  # rebuild name "x|x:name"
...
def nameSpace(node, multi=False):
    """Returns the name space of a node if it has one.  Will be "" if None.

    :param node: Name of a maya node or object.
    :type node: str
    :param multi: If True returns full namespace if there are more than one namespace levels. Otherwise first namespace.
    :type multi: bool
    ...
    if len(nameList) == 1:
        return ""
    if multi and len(nameList) > 2:
        longNameAndNamespace = nameList[0]
        for nameItem in nameList[1:-1]:
            longNameAndNamespace = ":".join([longNameAndNamespace, nameItem])
        nameList[0] = longNameAndNamespace
