...
from cgrig.libs.maya.cmds.objutils import filtertypes, curves
from cgrig.libs.maya.cmds.rig import jointsflowpath, axis

XYZ_LIST = ["x", "y", "z"]


class CgRigAlongAPath(base.MetaBase):
    ...
    def metaAttributes(self):
        ...
        defaults = super(CgRigAlongAPath, self).metaAttributes()
        defaults.extend(CgRigAlongAPath._default_attrs)
        return defaults
...
def switchSelectedCurves():
    ...
    metaNodes = base.findRelatedMetaNodesByClassType(nodes, CgRigAlongAPath.__name__)  # the metaNodes
    if not metaNodes:
...
def selectedMetaNodes():
    ...
    return base.findRelatedMetaNodesByClassType(selectedNodes, CgRigAlongAPath.__name__)
