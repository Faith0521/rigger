...
from cgrig.libs.maya.cmds.objutils import filtertypes, selection, namehandling, connections, nodemultibase
from cgrig.libs.maya.utils import mayacolors
from cgrig.libs.utils import color

from cgrig.libs.maya.cmds.shaders import shaderutils, shdmultconstants
from cgrig.libs.maya.cmds.shaders.shdmultconstants import DIFFUSE, DIFFUSEWEIGHT, SPECWEIGHT, SPECCOLOR, \
...
class ShaderBase(nodemultibase.NodeMultiBase):
    ...
    def knownShader(self):
        """Returns True if the shader is supported by CgRig Tools, should be overridden

        :return: Returns True if the shader is supported by CgRig Tools, should be overridden
        :rtype: bool
        """
        return False
    ...
    def assignedGeo(self):
        """Returns the geometry objects assigned to the shader

        :return: A list of geometry objects assigned to the shader
        :rtype: list(str)
        """
        if self.exists():
            return shaderutils.getObjectsFacesAssignedToShader(self.shaderName())
        return list()
