...
from cgrig.libs.maya.cmds.renderer.rendererconstants import REDSHIFT, ARNOLD, RENDERMAN, MAYA, VRAY
from cgrig.libs.maya.cmds.renderer import redshiftrendersettings, rendermanrendersettings, arnoldrendersettings, \
    vrayrendersettings
from cgrig.libs.maya.qt.changerendererui import globalChangeRenderer
...
def currentRenderer():
    """Returns the current renderer that is set in CgRig Tools.

    returns

        "Arnold" or Redshift" or "Renderman"

    :return renderer: Returns the nice name of the currently active renderer in CgRig Tools
    :rtype renderer: str
    """
...
def changeRenderer(renderer, setDefault=True, load=True, message=True):
    """Change renderer for the whole of CgRig Tools Pro:
    ...
    :type load: bool
    :param setDefault: If True set the renderer to the CgRig Tools default settings
    ...
    if setDefault:  # Set the CgRig Tools default render globals settings
        success = setDefaultRenderSettings(renderer)
        if message and success:
            output.displayInfo("Render `{}` loaded, and render globals changed to CgRig defaults".format(renderer))
        if message and not success:  # usually with Renderman
            output.displayWarning("Render `{}` loaded, but render globals not changed to CgRig defaults, please run "
...
def setDefaultRenderSettings(renderer, message=True):
    """Sets the default CgRig render settings for each renderer
    ...
    if warnings:
        ...
        return False
    om2.MGlobal.displayInfo("`{}` cgrig default renderer settings set.".format(renderer))
    return True
