...
from cgrig.libs.maya.cmds.objutils import shapenodes, namehandling, attributes, selection
...
def checkValidOrigShape(mesh, deleteDeadShapes=False, displayMessage=False):
    ...
    returns the orig shape node name or None on fail
    ...
    for i, shape in reversed(list(enumerate(originalShapeList))):  # tests for legit nodes from outgoing connections
        ...
        if not outputs:
            if deleteDeadShapes:  # do Maya delete on this as it's most likely a dead origShape and worthless
                ...
                if displayMessage:
                    currentOriginalShape = namehandling.getShortName(originalShapeList[i])
                    output.displayInfo("Dead Original Mesh Deleted: {}".format(currentOriginalShape))
            # remove from list
            del originalShapeList[i]
    if not originalShapeList and deleteDeadShapes:  # original shape didn't exist or no longer exists
        return ""
...
def cleanDeadOrigShapes(objs, displayMessage=True):
    checkValidOrigShapeList(objs, deleteDeadShapes=True, displayMessage=displayMessage)
    if displayMessage:
        selObjUniqueShortNames = namehandling.getUniqueShortNameList(objs)
...
def cleanDeadOrigShapesSelected(displayMessage=True):
    ...
    return cleanDeadOrigShapes(selObj, displayMessage=displayMessage)
...
def duplicateSelectedBeforeBind(fullPath=True, suffix="_duplicate", message=True, selectDuplicate=True,
    ...
    for obj in selObjList:
        newObj = duplicateMeshBeforeBind(obj, fullPath=fullPath, suffix=suffix,
                                         message=message, transferShader=transferShader)
        if newObj:
            duplicatedMeshes.append(newObj)
    if selectDuplicate:
...
def geoFromSkinCluster(skinCluster, transforms=True):
    """Returns the geometry from a skin cluster

    :param skinCluster: The skin cluster name
    :type skinCluster: str
    :param transforms: Return the transform nodes instead of the shape nodes
    :type transforms: bool
    :return meshes: The meshes connected to the skin cluster
    :rtype meshes: list
    """
    geometry = list()
    meshShapes = cmds.skinCluster(skinCluster, query=True, geometry=True)
    if not meshShapes:
        return geometry
    # convert to transform nodes
    if transforms:
...
def jointsFromSkinCluster(skinCluster):
    """Returns the joints from a skin cluster

    :param skinCluster: A skin cluster node name
    :type skinCluster: str
    :return: A list of joint names
    :rtype: list(str)
    """
    return cmds.skinCluster(skinCluster, query=True, influence=True)
...
def unlockAttrsOldSkinCluster(obj):
    """In newer versions of Maya does not unlock the skinCluster from old versions, and only works in mel
...
def unbindSkinObjs(objList, message=True):
    ...
    :rtype success: bool
    """
    clusterRemovedList = list()
    for obj in objList:
        skinCluster = getSkinCluster(obj)
        if skinCluster:  # then mesh is skinned
            cmds.skinCluster(skinCluster, edit=True, unbind=True)
            unlockAttrsOldSkinCluster(obj)  # unlcock fix for old skinClusters in old versions of Maya
            clusterRemovedList.append(skinCluster)
    if message:
        output.displayInfo("Skin Clusters Deleted `{}`".format(clusterRemovedList))
...
def renameSkinClusters(objs, suffix="skin", message=False):
    """Renames the skin clusters based off the given object names

    :param objs: A list of objects with skinning
    :type objs: list(str)
    :param suffix: skin cluster of pCube1 will be named pCube1_skin
    :type suffix: str
    :param message: Report the message inside of maya?
    :type message: bool
    :return: A list of skin cluster node names
    :rtype: list(str)
    """
    skinClusters = list()
    for obj in objs:
...
def renameSelSkinClusters(suffix="skin", message=True):
    ...
    if not sel:
        ...
        return list()
    return renameSkinClusters(sel, suffix=suffix, message=message)
...
def removeInfluence(influenceList, skinCluster):
    """Removes Influence Objects from a Skin Cluster, usually joints.
    Checks if influences are valid
    Returns object list of influences removed

    :param influenceList: the list of influence objects to be removed
    :type influenceList:
    ...
    for influence in influenceList:
        if checkInfluenceObj(influence, skinCluster):
            cmds.skinCluster(skinCluster, removeInfluence=influence, edit=True)
            removedInfluenceList.append(influence)
...
"""
SET BIND POSE - DISABLE ENABLE TOGGLE SKIN CLUSTERS
"""


def setBindPoseSel(displayMessage=True):
    ...
    :type displayMessage: bool
    """
    # get skinned mesh from selection
    selMesh = cmds.ls(selection=True, long=True)
    if not selMesh:
        if displayMessage:
            output.displayWarning("Please select skinned mesh to reset its bind pose")
        return False
    setBindPose(selMesh)
    return True
...
def setBindPose(skinNodes, freeze=False):
    """Sets the bind pose for a skin cluster or skin clusters.

    :param skinNodes: A single mesh or skin cluster node name or a list of skin cluster node names
    :type skinNodes: str or list(str)
    :param freeze: Freeze the joints after setting the bind pose?
    :type freeze: bool

    :return skinClusterList: A list of skin cluster node names that were set to bind pose
    :rtype skinClusterList: list(str)
    """
    skinClusterList = list()
    if isinstance(skinNodes, str):
        skinNodes = [skinNodes]

    for skinNode in skinNodes:
        skinCluster = mel.eval('findRelatedSkinCluster {}'.format(skinNode))
        if not skinCluster:
            continue
        joints = cmds.listConnections('{}.matrix'.format(skinCluster), type='joint', d=True) or []
        indexs = cmds.getAttr('{}.matrix'.format(skinCluster), mi=True) or []  # get joint index
        for joint, index in zip(joints, indexs):
            worldInverMatrix = cmds.getAttr('{}.worldInverseMatrix'.format(joint))
            cmds.setAttr('{}.bindPreMatrix[{}]'.format(skinCluster, index), worldInverMatrix, type='matrix')
        cmds.dagPose(joints, cmds.listConnections('{}.bindPose'.format(skinCluster), type='dagPose'), reset=True)
        skinClusterList.append(skinCluster)
        if freeze:
            freezeJoints(joints)
    return skinClusterList
...
def freezeJoints(joints):
    """Freezes the joint transforms and sets the jointOrient to zero.

    :param joints: A single joint name or a list of joint names
    :type joints: str or list(str)
    :return: None
    :rtype: NoneType
    """
    ...
    for joint in joints:
        jointObject = om2.MFnTransform(
            om2.MDagPath.getAPathTo(om2.MGlobal.getSelectionListByName(joint).getDependNode(0)))
        localRotMatrix = jointObject.rotation().asMatrix()

        jointOrientPlug = jointObject.findPlug('jointOrient', False)
        newRotMatrix = localRotMatrix * om2.MEulerRotation(
            *[jointOrientPlug.child(i).asDouble() for i in range(3)]).asMatrix()
        newRotation = om2.MTransformationMatrix(newRotMatrix).rotation()

        cmds.setAttr('{}.rotate'.format(joint), 0, 0, 0)
        cmds.setAttr('{}.jointOrient'.format(joint), *(degrees(i) for i in newRotation), type='double3')


def disableSkinCluster(skinCluster):
    """Disables a single skin cluster

      (deprecated by setBindPose)
...
def toggleAllSkinClusters(displayMessage=True):
    """Toggles all skin clusters in a scene  (deprecated by setBindPose)
...
def copySkinWeightsSel(message=True):
    ...
    if not getSkinCluster(firstObj):
        ...
        return ""
    skinInstance = CgRigSkinTrackerSingleton()
...
def transferSkinningDuplicates(suffix="_duplicate"):
    """Automatically transfers skinning to a duplicate set usually named with a suffix "_duplicate"

    Select skinned objects and then run the function.

    :param suffix: The suffix to find and skin
    :type suffix: str
    :return: skinClusters, skinnedObjs
    :rtype: tuple(list(str), list(str))
    """
    skinnedObjs = list()
    skinClusters = list()
    ...
    selObjs = cmds.ls(selection=True)
    if not selObjs:
        output.displayWarning("No objects selected, please select skinned objects to transfer skin clusters")
        return
    for obj in selObjs:
        targetMesh = "".join([obj, suffix])
        if cmds.objExists(targetMesh):
            targetSkinCluster = transferSkinning(obj, targetMesh)
            if targetSkinCluster:
                skinClusters.append(targetSkinCluster)
                skinnedObjs.append(targetMesh)
            else:
                skippedObjs.append(obj)
    if not skinnedObjs:
        output.displayWarning("No skin clusters transferred, please select a skinned mesh")
        return
    output.displayInfo("Meshes now skinned: {}".format(skinnedObjs))
    if skippedObjs:
        output.displayInfo("Meshes skipped: {}".format(skippedObjs))
        output.displayInfo("Success: Skin Clusters Transferred and some meshes skipped")
...
class CgRigSkinTrackerSingleton(object):
