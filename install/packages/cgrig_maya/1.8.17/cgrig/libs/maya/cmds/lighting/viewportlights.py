...
from cgrig.libs.maya.cmds.lighting import lightsmultihdri, lightconstants, renderertransferlights as rtl
...
from cgrig.libs.maya.cmds.objutils import attributes, shapenodes, namehandling, layers, connections
...
def createMultNode(name):
    if MAYA_VERSION < 2026:
        return cmds.createNode('multDoubleLinear', n=name)
    else:
        return cmds.createNode('multDL', n=name)
...
def directionalControlIntensity(directionalTsfrmsStr, directionalShapeList, specularTsfrmStr, specularShape, intensity,
    ...
    if directionalShapeList:  # Might be being rebuilt
        ...
        intMultNode = createMultNode("vpLightIntMult")
    ...
    if specularShape:  # Might be being rebuilt
        if not cmds.attributeQuery(specAttr, node=cntrl, exists=True):
            cmds.addAttr(cntrl, longName=specAttr, attributeType='float', keyable=1, defaultValue=specularIntensity,
                         minValue=0.0)
        specMultNode = createMultNode("vpLightSpecMult")
