...
.. code-block:: python
    ...
    from cgrig.libs.maya.cmds.lighting import lightsmultidirectional
...
Default Directional dictionary example from cgrig.libs.maya.cmds.lighting.lightconstants:
...
from cgrig.libs.maya.cmds.lighting.directionaltypes import directionalbase, arnolddirectional, redshiftdirectional, \
...
def rendererFromCgRigAttr(directionalShape):
    """Given a shape type of "directionalLight" the node may be Arnold or VRay.

    This function checks for a tracking attribute "cgrigRendererTracker" and if it exists it returns the renderer.
...
def directionalInstancer(directionalType, directionalName="", forceRenderer="", create=False, ingest=True,
    ...
    :return: A cgrig directional instance of the specified light.
...
def directionalInstanceFromShape(directionalShape, message=True):
    ...
    :type message: bool
    :return: A cgrig directional instance of the specified directional light, None if the light does not exist
    :rtype: :class:`directionalbase.DirectionalBase`
    """
    ...
    if directionalType not in lgtcn.RENDERERDIRECTIONALLIGHTS.values():
        if message:
            output.displayWarning("The node type `{}` is not a CgRig supported directional light".format(directionalType))
        return None

    if directionalType == "directionalLight":  # could be Arnold or VRay or Maya
        forceRenderer = rendererFromCgRigAttr(directionalShape)
...
def createDirectionalInstanceType(directionalType, directionalName="", forceRenderer="", suffixName=False,
    ...
    :return: A cgrig directional instance of the specified directional type
    :rtype: :class:`directionalbase.DirectionalBase`
    """
...
def createDefaultDirectionalRenderer(renderer, directionalName="", lightGroup=True, select=True, suffixName=False,
    ...
    if lightGroup:
        directionalInstance.parentCgRigLightGroup()
...
def createDefaultDirectionalRendererAuto(directionalName="", lightGroup=True, select=True, suffixName=False,
                                         position="selected", message=True):
    """Creates a default directional light with the renderer set automatically as per the internal cgrig setting at:
...
def directionalInstancesFromNodes(nodeTransforms, directionalTypes=lgtcn.DIRECTIONAL_TYPES, message=True):
    ...
    :return: A list of cgrig directional instances
...
def directionalInstancesFromSelected(directionalTypes=lgtcn.DIRECTIONAL_TYPES, message=True):
    """From selected directional lights return a list of associated cgrig directional instances
...
def directionalInstanceRenderer(renderer, message=True):
    ...
    :param message: Report warnings to the user?
    :type message: bool
    :return: A cgrig directional instance or None if none found.
    :rtype: :class:`directionalbase.DirectionalBase`
    """
