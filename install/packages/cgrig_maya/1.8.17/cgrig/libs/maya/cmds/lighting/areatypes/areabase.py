...
.. code-block:: python
    ...
    from cgrig.libs.maya.cmds.lighting.areatypes import areabase
...
Default Area dictionary example from cgrig.libs.maya.cmds.lighting.lightconstants:
...
from cgrig.libs.maya.cmds.lighting import lightingutils, lightsbase
from cgrig.libs.maya.cmds.lighting.lightconstants import AREA_NAME, AREA_INTENSITY, AREA_EXPOSURE, AREA_LIGHTVISIBILITY, \
...
class AreaBase(lightsbase.LightsBase):
    ...
    cgrigExposureAttr = "cgrig_exposer_tracker"
    cgrigIntensityAttr = "cgrig_intensity_tracker"
    ...
    def _createCgRigIntExpAttr(self):
        if not cmds.attributeQuery(self.cgrigExposureAttr, node=self.shapeName(), exists=True):
            cmds.addAttr(self.shapeName(), longName=self.cgrigIntensityAttr, attributeType='float')
            cmds.addAttr(self.shapeName(), longName=self.cgrigExposureAttr, attributeType='float')
    ...
    def _intensityFromCgRigAttrs(self):
        """Gets the intensity in generic (UI) values and the cgrig attr values.
        Compares if the cgrig values match the user/Maya native values.

        :return: genericIntensity, intensity, exposure, cgrigAttrsMatch
        :rtype: tuple(float, float, float, bool)
        """
        self._createCgRigIntExpAttr()
        genericIntensity = self.pureIntensityToGeneric()
        exposure = cmds.getAttr(".".join([self.shapeName(), self.cgrigExposureAttr]))  # UI/TrackerValue
        intensity = cmds.getAttr(".".join([self.shapeName(), self.cgrigIntensityAttr]))  # UI/TrackerValue
        combinedCgRigAttrValue = lightingutils.convertExpAndIntToIntensity(intensity,
                                                                         exposure)[0]

        if nearlyEqual(genericIntensity, combinedCgRigAttrValue, percentDifferent=0.001):  # rounding issues
    ...
    def setExposureMissing(self, exposure):
        ...
        """
        self._createCgRigIntExpAttr()
        cmds.setAttr(".".join([self.shapeName(), self.cgrigExposureAttr]), exposure)
        intensity = cmds.getAttr(".".join([self.shapeName(), self.cgrigIntensityAttr]))  # UI Tracker Intensity
    ...
    def exposureMissing(self):
        ...
        If the cgrig exposure values do not match the actual light's real intensity, the exposure is returned as zero.

        :return intensity: The intensity value of the light
        :rtype intensity: float
        """
        genericIntensity, intensity, exposure, matchBool = self._intensityFromCgRigAttrs()
        if matchBool:
            return exposure  # as per UI value
        else:
            shapeName = self.shapeName()
            cmds.setAttr(".".join([shapeName, self.cgrigIntensityAttr]), genericIntensity)
            cmds.setAttr(".".join([shapeName, self.cgrigExposureAttr]), 0.0)
    ...
    def setIntensityMissing(self, intensity):
        ...
        """
        self._createCgRigIntExpAttr()
        cmds.setAttr(".".join([self.shapeName(), self.cgrigIntensityAttr]), intensity)
        exposure = cmds.getAttr(".".join([self.shapeName(), self.cgrigExposureAttr]))  # UI Tracker Intensity
        intensity, exposure = lightingutils.convertExpAndIntToIntensity(intensity, exposure)
        return intensity  # generic units
    ...
    def intensityMissing(self):
        ...
        If the cgrig exposure values do not match the actual light's real intensity, the pure intensity is returned.
