# -*- coding: utf-8 -*-
"""This module is the Maya viewport class for directional lights
...
.. code-block:: python
    ...
    from cgrig.libs.maya.cmds.lighting.directionaltypes import mayadirectional
    directionalInst = mayadirectional.MayaDirectional("directionalLightName", create=True)  # creates a new Directional light
    ...
    directionalInst = mayadirectional.MayaDirectional(name="existingDirectionalLight", create=False, ingest=True)  # ingest existing light
...
from cgrig.libs.maya.cmds.lighting import lightconstants
from cgrig.libs.maya.cmds.lighting.directionaltypes import directionalbase

NODE_TYPE = "directionalLight"


class MayaDirectional(directionalbase.DirectionalBase):
    ...
    tempBoolAttr = ""
    temperatureAttr = ""
    softAngleAttr = "aiAngle"

    intensityMultiply = 3.13
    scaleMultiply = 1.0
    ...
    def __init__(self, name="", genAttrDict=None, node=None, create=False, ingest=False, suffixName=False,
        ...
        :type message: bool
        """
        super(MayaDirectional, self).__init__(name=name,
    ...
    def create(self, message=True):
        super(MayaDirectional, self).create(message=True)
        # Add CgRig renderer tag, differentiates between VRay/Arnold/Maya, attr name is "cgrigRendererTracker" str renderer
        cmds.addAttr(self.shapeName(), longName=lightconstants.DIRECTIONAL_TRACKER_ATTR, dataType="string")
        cmds.setAttr(".".join([self.shapeName(), lightconstants.DIRECTIONAL_TRACKER_ATTR]),
                     self.renderer(), type="string")
    ...
    def softAngle(self):
        ...
        try:
            super(MayaDirectional, self).softAngle()
        except:
            pass
    ...
    def setSoftAngle(self, softAngle):
        """Tries to set the Arnold vis setting as does not exist on the default light"""
        try:
            return super(MayaDirectional, self).setSoftAngle(softAngle)
        except:
            return 0.0
