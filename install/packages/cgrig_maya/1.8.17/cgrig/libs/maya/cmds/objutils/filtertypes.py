...
from cgrig.libs.maya.cmds.lighting import arnoldlights, rendermanlights, redshiftlights
...
def shapeFromTransformOrShapeTypeList(objList, nodeTypeList):
    """Returns a list of shapes of the given type from a node list that can include transforms and shape nodes.

    :param objList: list of maya nodes string names
    :type objList: list(str)
    :param nodeTypeList: list of node types ["mesh", "nurbsSurface"]
    :type nodeTypeList: list(str)
    :return: list of shape nodes as strings
    :rtype: list(str)
    """
    shapes = list()
    for nodeType in nodeTypeList:
        nodes = shapeTypeFromTransformOrShape(objList, shapeType=nodeType)
        if nodes:
            shapes += nodes
    return shapes
...
def filterGeoOnly(objList, nurbs=True, polygons=True, fullpath=False):
    ...
    """
    geoList = list()
    transformList = cmds.ls(objList, type="transform", long=fullpath)
    if polygons:
        geoList += cmds.ls(objList, type="mesh", long=fullpath)
    if nurbs:
        geoList += cmds.ls(objList, type="nurbsSurface", long=fullpath)
    if transformList:
        for transform in transformList:
            shapes = cmds.listRelatives(transform, shapes=True, fullPath=fullpath)
            if not shapes:
                continue
            for shape in shapes:
                if cmds.ls(shape, type="nurbsSurface", long=fullpath) and nurbs:
                    geoList.append(shape)
                if cmds.ls(shape, type="mesh", long=fullpath) and polygons:
                    geoList.append(shape)
    return geoList
