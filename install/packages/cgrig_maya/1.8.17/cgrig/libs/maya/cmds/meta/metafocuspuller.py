# -*- coding: utf-8 -*-
""""
from cgrig.libs.maya.cmds.meta import metafocuspuller
...
from cgrig.libs.maya.cmds.cameras import focuspuller, cameras
from cgrig.libs.maya import triggers

CAM_DEFAULT_ATTRS = ["dofFocusDistance", "dofFStop", "dofFocusRegionScale", "dofVis", "dofCtrlVis", "dofPlaneVis",
                     "gridVis", "focusCtrlScale"]

META_CLASS_TYPES = ["CgRigFocusPullerRedshift", "CgRigFocusPullerRenderman", "CgRigFocusPullerArnold", "CgRigFocusPuller"]
...
def get_enum_items(obj, attr):
    """Returns the enum items of an attribute as a list useful for returning the DOF title enum item"""
    if cmds.attributeQuery(attr, node=obj, exists=True):
        attr_type = cmds.getAttr("{}.{}".format(obj, attr), type=True)
        if attr_type == 'enum':
            enum_string = cmds.attributeQuery(attr, node=obj, listEnum=True)[0]
            return enum_string.split(':')
    return []


def remove_focusAttrs(camera_name, attrs):
    """ Removes the given focus puller attributes from the camera.
    Also deletes the title attributes if they exist.

    :param camera_name: The camera name
    :type camera_name: str
    :param attrs: The attributes to remove
    :type attrs: list(str)"""
    # remove title attributes if they exist
    titleList = get_enum_items(camera_name, "_")
    if titleList:
        if titleList[0] == 'DOF':
            cmds.setAttr("{}._".format(camera_name), lock=False)
            cmds.deleteAttr("{}._".format(camera_name))
    titleList = get_enum_items(camera_name, "__")
    if titleList:
        if titleList[0] == 'Controls':
            cmds.setAttr("{}.__".format(camera_name), lock=False)
            cmds.deleteAttr("{}.__".format(camera_name))
    # remove the attributes per renderer
    for attr in attrs:
        if cmds.attributeQuery(attr, node=camera_name, exists=True):
            cmds.deleteAttr("{}.{}".format(camera_name, attr))


class CgRigFocusPuller(base.MetaBase):
    ...
    def metaAttributes(self):
        ...
        defaults = super(CgRigFocusPuller, self).metaAttributes()
        defaults.extend(CgRigFocusPuller._default_attrs)
        return defaults
...
def metaRenderer(meta):
    ...
    :type meta: CgRigFocusPuller
    :return renderer: The renderer type of the meta node eg "Arnold"
    :rtype renderer: str
    """
    classType = meta.mClassType()
    if classType == "CgRigFocusPullerArnold":
        return "Arnold"
    elif classType == "CgRigFocusPullerRedshift":
        return "Redshift"
    elif classType == "CgRigFocusPullerRenderman":
        return "Renderman"
    else:  # CgRigFocusPuller
        return "Viewport"
...
def allMetaScene():
    ...
    :rtype metaNodes: list(CgRigFocusPuller)
    """
    metaNodes = list()
...
def cleanupBrokenMetaNodes(message=True):
    ...
    :rtype deletedMeta: list(CgRigFocusPuller)
    """
...
def checkLoadedCamera():
    ...
    :return: camera name
    :rtype: :class:`cgrig.libs.maya.zapi.base.DagNode`
    """
...
def deleteInvalidMeta(metaNode, message=True):
    ...
    :type metaNode: CgRigFocusPuller
...
def buildRigSetup(camera_name):
    ...
    # no meta, so check the attrs of focus puller don't exist (duplicated camera), remove any that exist
    remove_focusAttrs(camera_name, CAM_DEFAULT_ATTRS)
    meta = CgRigFocusPuller()
