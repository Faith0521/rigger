...
from cgrig.libs.maya.cmds.animation import grapheditorfcurve, timerange, keyframes, animobjects, resetattrs, bakeanim, \
...
def animHold(reverse=False):
    ...
    CgRig Hotkey: alt a
    """
    grapheditorfcurve.animHold(reverse=reverse, message=True)
...
def createMotionTrail():
    ...
    Displays settings from the CgRigMotionTrailTrackerSingleton() data as shared across UIs and marking menus.
    """
...
class CgRigAnimationTrackerSingleton(object):
...
@six.add_metaclass(classtypes.Singleton)
class CgRigGraphEditorTrackerSingleton(object):
    """Used by the animation marking menu & UI, tracks data for animation between UIs and marking menus.
    """
