...
from cgrig.libs.maya.cmds.meta import metasplinerig, metaadditivefk
...
class SplineRigMarkingMenuCommand(menu.MarkingMenuCommand):
    ...
    def uiData(arguments):
        ...
        elif meta.isClassType(metaadditivefk.CgRigMetaAdditiveFk):
    ...
    def execute(self, arguments):
        ...
        for m in metaNodes:
            ...
            if not m.isClassType(metasplinerig.MetaSplineRig):
                if m.isClassType(metaadditivefk.CgRigMetaAdditiveFk):
                    if list(m.metaParents()):
                        meta = list(m.metaParents())[-1]
                else:
                    continue
            if operation == "switchToSpine":
                executor.execute("cgrig.maya.splinerig.switch", meta=meta, switchTo="spine")
            elif operation == "switchToFk":
                executor.execute("cgrig.maya.splinerig.switch", meta=meta, switchTo="fk")
            elif operation == "switchToFloat":
                executor.execute("cgrig.maya.splinerig.switch", meta=meta, switchTo="float")
            elif operation == "switchToRevFk":
                executor.execute("cgrig.maya.splinerig.switch", meta=meta, switchTo="revFk")
            elif operation == "duplicate":
                executor.execute("cgrig.maya.splinerig.duplicate", meta=meta)
            elif operation == "bake":
                executor.execute("cgrig.maya.splinerig.bake", meta=meta)
            elif operation == "rebuild":
                executor.execute("cgrig.maya.splinerig.rebuild", meta=meta, bake=False)
            elif operation == "rebuildBaked":
                executor.execute("cgrig.maya.splinerig.rebuild", meta=meta, bake=True)
            elif operation == "delete":
                executor.execute("cgrig.maya.splinerig.delete", meta=meta)
                ...
                if meta.isClassType(metasplinerig.MetaSplineRig):  # if there's a spline rig
                    ...
                    executor.execute("cgrig.maya.additiveFk.delete", meta=meta)
            elif operation == "bakeAdditiveFk":  # Only shows while no spline rig
                executor.execute("cgrig.maya.additiveFk.delete", meta=meta, bake=True)
