...
from cgrig.libs.maya.cmds.rig import nodes
from cgrig.libs.maya.cmds.skin import bindskin
from cgrig.libs.maya.cmds.objutils import namehandling
from cgrig.libs.maya.cmds.modeling import extrude
from cgrig.libs.maya.cmds.blendshapes import blendshape
from cgrig.libs.maya.cmds.ncloth import nclothconstants
from cgrig.libs.maya.cmds.ncloth.nclothconstants import TSHIRT_PRESET_DICT, ORIGINAL_MESH_TFRM, POLYSMOOTH_MESH_TFRM, \
    CLOTH_MESH_TFRM, CLOTH_MESH_SHAPE, BLENDER_MESH_TFRM, BLENDSHAPE_ORIG_NODE, BLENDSHAPE_CLOTH_NODE, \
    NCLOTH_NODE, NUCLEUS_NODE, NCLOTH_TRANSFORM, CGRIG_SK_NCLOTH_NETWORK, CLOTH_NODES_GRP, \
...
def addClothThickness(obj, thickness=1.0, weight=0.5, select=True):
    ...
    This function is not used see the module cgrig.libs.maya.cmds.modeling.extrude
...
def nClothDefaults(nClothShape, presetDict=TSHIRT_PRESET_DICT):
    ...
    :param presetDict: Preset Dictionary with ncloth attrs and values. See cgrig.libs.maya.cmds.ncloth.nclothconstants
...
def nClothSkinned(mesh, divisionsPolySmooth=2, nucleusNode='nucleus', nClothTransform='nCloth',
    ...
    :return: The name of the network node created, Dictionary of all nodes, uses keys from \
             cgrig.libs.maya.cmds.ncloth.nclothconstants
...
def skinnedNClothMessageNodeSetup(skinnedNClothNodes, divisionsPolySmooth, presetIndex=0):
    """Connects skinned NCloth nodes to a network node named "cgrigSkNClothNetwork_*" (CGRIG_SK_NCLOTH_NETWORK)

    The network node can be handy later, for deleting or managing objects, manage attrs etc

    :param skinnedNClothNodes: Dictionary of all nodes, uses keys from cgrig.libs.maya.cmds.ncloth.nclothconstants
    ...
    networkNodeName = "{}_{}".format(CGRIG_SK_NCLOTH_NETWORK, origObj)
...
def getSkinnedNClothDictNode(node):
    ...
    if CGRIG_SK_NCLOTH_NETWORK in node and cmds.nodeType(node) == "network":
...
def getNetworkNodesAll():
    ...
    """
    return cmds.ls("*{}*".format(CGRIG_SK_NCLOTH_NETWORK), type="network")
...
def setAttrNetwork(networkNode, nodeKey, attribute, value):
    ...
    A list of keys can be found in the dictionary CGRIG_SK_NCLOTH_NETWORK
...
def textCurrentPreset(nClothShape):
    ...
    Code use:
        ...
        from cgrig.libs.maya.cmds.ncloth import nclothskinned
