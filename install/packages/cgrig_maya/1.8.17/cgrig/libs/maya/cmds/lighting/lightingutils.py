...
.. code-block:: python
    ...
    from cgrig.libs.maya.cmds.lighting import lightingutils
...
from cgrig.core.util import env, classtypes
from cgrigvendor import six

if env.isMaya():
    import maya.cmds as cmds
from cgrig.libs.utils import output
from cgrig.libs.maya.cmds.lighting import lightconstants as lgtcn
...
def rendererFromNodeType(lightType):
    """Returns the renderer of the given node type if it is supported by CgRig Tools.
...
def placeReflection():
    ...
    try:  # Place reflection is a community tool and may not be installed with the light suite
        from cgrig.apps.braverabbit_place_reflection import placeReflection
...
class CgRigLightsTrackerSingleton(object):
