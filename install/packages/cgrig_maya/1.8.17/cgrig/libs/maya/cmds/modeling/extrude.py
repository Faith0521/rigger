...
CGRIG_THICKNESS_ATTR = "cgrigCenterThickness"
...
def extrudeCenterThickness(obj, thickness=1.0, weight=0.5, select=True):
    ...
    cmds.addAttr(moveVertNode, longName=CGRIG_THICKNESS_ATTR, attributeType='float', keyable=False)
    cmds.addAttr(extrudeNode, longName=CGRIG_THICKNESS_ATTR, attributeType='float', keyable=False)
...
def centerThicknessNode(obj, nodeType="polyExtrudeFace"):
    ...
    for extrudeNode in nodes:
        if cmds.attributeQuery(CGRIG_THICKNESS_ATTR, node=extrudeNode, exists=True):
            return extrudeNode
    return ""


def getExtrudeThickness(obj):
    """Search for the first extrude node found and if it has a CGRIG_THICKNESS_ATTR attribute then return it's \
    localTranslateZ
...
def getExtrudeThicknessSelected(message=True):
    """Of selected meshes search for the first extrude node found and if it has a CGRIG_THICKNESS_ATTR attribute then \
...
def setExtrudeThickness(obj, thickness=1.0, weight=0.5):
    ...
    Both nodes if a legitimate object with this setup will be marked with the attribute CGRIG_THICKNESS_ATTR
