...
class CgRigMetaAdditiveFk(base.MetaBase):
    ...
    def metaAttributes(self):
        ...
        defaults = super(CgRigMetaAdditiveFk, self).metaAttributes()
        defaults.extend(CgRigMetaAdditiveFk._default_attrs)
        return defaults
    ...
    def createAdditiveFk(self, jointList, rigName="additive", controlSpacing=1, lockHideJoints=True,
        ...
        if transferSkinning:  # Transfer any skinning to the original joints\
            skinreplacejoints.replaceSkinJointList(jointList, addFkBlendJntList, moveWithJoints=True)
    ...
    def setScale(self, scl):
        ...
        for ctrl in rigControls:
            scaleDefaults = ("cgrigScaleDefault_x", "cgrigScaleDefault_y", "cgrigScaleDefault_z")
    ...
    def deleteSetup(self, transferSkinning=True, deleteOrigJnts=False, bake=False, message=True):
        ...
        if transferSkinning:  # Transfer any skinning to the original joints
            skinreplacejoints.replaceSkinJointList(zapi.fullNames(blendJoints),
                                                   zapi.fullNames(origJoints), moveWithJoints=True)
        # Delete
