...
.. code-block:: python
    ...
    from cgrig.libs.maya.cmds.lighting.hdritypes import arnoldhdri
...
from cgrig.libs.maya.cmds.textures.filetexturetypes import mayafiletexture
...
class ArnoldHdri(hdribase.HdriBase):
    ...
    def __init__(self, name="", genAttrDict=None, node=None, create=False, ingest=False, suffixName=False,
        ...
        super(ArnoldHdri, self).__init__(name=name,
                                         ...
                                         message=message)
        arnoldMajor, arnoldMinor, arnoldPatch = arnoldVersionNumber()
        if arnoldMajor >= 5 and arnoldMinor >= 3:
            self.exposureAttr = "aiExposure"
    ...
    def setIntensity(self, intensity):
        ...
        :param intensity: The intensity of the light in CgRig generic units
    ...
    def intensity(self):
        ...
        :return intensity: The intensity of the light in generic CgRig units
    ...
    def exposure(self):
        ...
        :return intensity: The exposure of the light in generic CgRig units
        :rtype intensity: float
        """
        return self._getAttrScalar(self.intensityAttr, node=self.shapeName())

    def _setHDRIColorSpace(self, forceSRGB=False):
        ...
        if MAYA_VERSION < 2022 or cmds.colorManagementPrefs(query=True, displayName=True) == "legacy":
            if forceSRGB:
                self.txtrInst.setColorSpace("sRGB")
            else:
    ...
    def setImagePath(self, imagePath):
        ...
        :type imagePath: str"""
        forceSRGB = False
        # Check if the image path ends with .png, .jpg or .jpeg
        if ".png" in imagePath.lower() or ".jpg" in imagePath.lower() or ".jpeg" in imagePath.lower():
            forceSRGB = True
        # Autoload or create a texture network
        if self.txtrInst:
            if self.txtrInst.exists():
                self.txtrInst.setPath(imagePath)
                self._setHDRIColorSpace(forceSRGB=forceSRGB)
                return
        ...
        if not self.txtrInst.valid:
            output.displayWarning("The existing texture input to the `{}` attribute "
                                  "is not recognized by CgRig Tools".format(self.imagePathAttr))
            return
        self.txtrInst.setPath(imagePath)
        self._setHDRIColorSpace(forceSRGB=forceSRGB)
