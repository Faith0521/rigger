...
from cgrig.libs.maya.cmds.broadunittests import testrenderers
...
from cgrig.apps.model_assets import assetconstants as ac
from cgrig.apps.light_suite import lightconstants as lc
...
def importReplaceAsset(path, assetName, renderer):
    ...
    # import asset
    fileName = ".".join([assetName, exportabcshaderlights.CGRIGSCENESUFFIX])
    allNodes = assetsimportexport.importCgRigSceneAsAsset(os.path.join(path, fileName),
                                                        renderer,
                                                        replaceAssets=True,
                                                        importAbc=True,
                                                        importShaders=True,
...
def testImportChangeAssetsRenderer(renderer):
    ...
    for assetName in ASSET_LIST:
        ...
        assetName = assetName.replace(" ", "_")
        assetsimportexport.deleteCgRigAssetSelected(uiSelectedName=assetName)
...
def importLightPreset(path, lightPresetName, renderer):
    if not rendererload.getRendererIsLoaded(renderer):
        rendererload.loadRenderer(renderer)
    # import asset
    fileName = ".".join([lightPresetName, exportabcshaderlights.CGRIGSCENESUFFIX])
