# -*- coding: utf-8 -*-
"""This module is the Renderman class for area lights
...
.. code-block:: python
    ...
    from cgrig.libs.maya.cmds.lighting.areatypes import rendermanarea
    areaInst = rendermanarea.RendermanArea("areaLightName", create=True, suffixName=False)  # creates a new area light
    ...
    areaInst = rendermanarea.RendermanArea(name="existingAreaLight", create=False, ingest=True)  # ingest existing light
...
NODE_TYPE = "PxrRectLight"


class RendermanArea(areabase.AreaBase):
    """Main class that manages an area light
    """
    nodetype = NODE_TYPE
    nodetypes = ["PxrRectLight", "PxrSphereLight", "PxrDiskLight"]
    lightSuffix = "PXR"
    intensityAttr = "intensity"
    exposureAttr = "exposure"
    visAttr = "primaryVisibility"
    colorAttr = "lightColor"
    normalizeAttr = "areaNormalize"
    tempBoolAttr = "enableTemperature"
    ...
    def __init__(self, name="", genAttrDict=None, node=None, create=False, ingest=False, suffixName=False,
        ...
        :type message: bool
        """
        super(RendermanArea, self).__init__(name=name,
    ...
    def setShape(self, shape, rebuild=True):
        self.nodetype = self.nodetypes[shape]
        if not rebuild:
            return
        pass
