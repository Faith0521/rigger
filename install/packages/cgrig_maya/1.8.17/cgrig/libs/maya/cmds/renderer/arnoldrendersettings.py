...
from cgrig.libs.maya.cmds.renderer.rendererconstants import ARNOLD, RENDERER_PLUGIN
...
def loadRendererArnold(setCgRigDefaults=True):
...
def setArnoldMatteSel(matte=True):
    selObjs = cmds.ls(selection=True)

    shapes = cmds.listRelatives(selObjs, shapes=True, fullPath=True)
    if shapes:
        for shape in shapes:
            if cmds.nodeType(shape) == "mesh":
                cmds.setAttr("{}.aiMatte".format(shape), matte)

def setArnoldVisibilitySel(visibility=False):
    selObjs = cmds.ls(selection=True)
    ...
    if shapes:
        for shape in shapes:
            if cmds.nodeType(shape) == "mesh":
                cmds.setAttr("{}.primaryVisibility".format(shape), visibility)
