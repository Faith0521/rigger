...
from cgrig.libs.maya.cmds.objutils import display
display.setPrimaryVisibleSelected()
...
# ------------------
# Arnold display attributes
# ------------------

def setAiMatteObject(objList, value=True):
    """Sets all shapes to be matte in Arnold, this is a convenience function for setting all the attributes
    ...
    meshList = objhandling.getTypeTransformsHierarchy(objList, nodeType="mesh")
    if not meshList:
        om2.MGlobal.displayWarning("No Shape Nodes Found, Please Select And Run Again")
        return False
    for shape in meshList:
        cmds.setAttr('{}.aiMatte'.format(shape), value)
        cmds.setAttr('{}.primaryVisibility'.format(shape), value)
        # hide
        cmds.setAttr('{}.castsShadows'.format(shape), not value)
        cmds.setAttr('{}.aiVisibleInDiffuseReflection'.format(shape), not value)
        cmds.setAttr('{}.aiVisibleInSpecularReflection'.format(shape), not value)
        cmds.setAttr('{}.aiVisibleInDiffuseTransmission'.format(shape), not value)
        cmds.setAttr('{}.aiVisibleInSpecularTransmission'.format(shape), not value)
        cmds.setAttr('{}.aiVisibleInVolume'.format(shape), not value)
        cmds.setAttr('{}.aiSelfShadows'.format(shape), not value)

def setAiMatteObjectSelected(value=True):
    """Sets all selected shapes to be matte in Arnold, this is a convenience function for setting all the attributes
    ...
    if not objList:
        om2.MGlobal.displayWarning("No Objects Selected, Please Select And Run Again")
        return False
    if setAiMatteObject(objList, value=value):
        om2.MGlobal.displayInfo("Success: Objects Now Have Matte Attributes set to `{}` - `{}` ".format(value, objList))
        return True
