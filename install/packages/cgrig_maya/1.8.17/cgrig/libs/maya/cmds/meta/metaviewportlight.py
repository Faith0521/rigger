# -*- coding: utf-8 -*-
"""
from cgrig.libs.maya.cmds.meta import metaviewportlight
metaNode = metaviewportlight.CgRigViewportLight()
...
from cgrig.libs.maya.cmds.lighting import viewportlights
...
from cgrig.libs.maya.cmds.objutils import connections
...
class CgRigViewportLight(base.MetaBase):
    """Class that controls the meta network node setup for cgrig viewport lights
    ...
    def metaAttributes(self):
        ...
        defaults = super(CgRigViewportLight, self).metaAttributes()
        defaults.extend(CgRigViewportLight._default_attrs)
        return defaults
    ...
    def deleteLight(self, deleteMeta=True, message=True):
        ...
        if deleteMeta:  # deletes the meta node itself
            self.delete()
        # Delete layers with safety
        for lyr in [viewportlights.LAYER_CTRLS, viewportlights.LAYER_GEO, viewportlights.LAYER_LIGHTS]:
            if cmds.objExists(lyr):
                if cmds.nodeType(lyr) == "displayLayer":
                    cmds.delete(lyr)
        if message:
...
def sceneMetaNodes():
    """Returns all CgRigMotionPathRig meta nodes, will delete any metaNodes that are not deemed valid

    :param validMeta: A list of viewport light meta nodes
    :type validMeta: list(:class:`CgRigViewportLight`)
    """
    metaNodes = base.findMetaNodesByClassType(CgRigViewportLight.__name__)
    validMeta = list()
    if not metaNodes:
        return list()
...
def metaNodesFromCamera(camera):
    ...
    metaNodes = base.findRelatedMetaNodesByClassType([cameraZapi], CgRigViewportLight.__name__)
    return metaNodes
...
def cameraVLightMetaNodes():
    ...
    :rtype metaNodes: :class:`CgRigViewportLight`
    """
...
def createViewportLight(camera, selectLight=True, directionalIntensity=0.55, specularIntensity=0.0,
    ...
    :rtype metaNode: :class:`CgRigViewportLight`
    """
    metaNode = CgRigViewportLight()
...
def switchCamViewportLights(metaNodes, camera):
    ...
    :type metaNodes: list(:class:`CgRigViewportLight`)
