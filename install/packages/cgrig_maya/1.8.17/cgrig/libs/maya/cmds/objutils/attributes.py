...
MAYA_TRANS_ROT_ATTRS = MAYA_TRANSLATE_ATTRS + MAYA_ROTATE_ATTRS
MAYA_DEFAULT_ATTRS = MAYA_TRANSFORM_ATTRS + ["visibility", "rotateOrder"]
...
def createAttribute(node, attr, attributeType="float", channelBox=True, nonKeyable=False, defaultValue=0,
                    minValue=None, maxValue=None, multi=False):
    ...
    if attributeType == "string":
        cmds.addAttr(node, longName=attr, dt='string')
        cmds.setAttr(".".join([node, attr]), defaultValue, type="string")
    elif not multi and attributeType != "matrix" and attributeType != "message":
        cmds.addAttr(node, longName=attr, attributeType=attributeType, defaultValue=defaultValue)
    else:
        cmds.addAttr(node, longName=attr, attributeType=attributeType, multi=multi)
...
def isAProxyAttr(obj, attr_name):
    ...
    """
    from cgrig.libs.maya import zapi
    return zapi.nodeByName(obj).attribute(attr_name).isProxy()


def find_proxy_source(node, attr):
    """
    Finds the source attribute of a proxy attribute in Maya. Returns None if the attribute is not a proxy.

    :param node: str, name of the node with the proxy attribute
    :param attr: str, name of the proxy attribute
    :return: str, source plug in the format 'node.attr', or None
    """
    if not isAProxyAttr(node, attr):
        return None, None
    full_attr = "{}.{}".format(node, attr)

    if not cmds.objExists(full_attr):
        cmds.warning("Attribute does not exist: {}".format(full_attr))
        return None, None
    ...
    if connections:
        nodeAttr = connections[0]  # e.g., 'originalNode.originalAttr'
        # return the node and attribute as two strings
        return nodeAttr.split(".")[0], nodeAttr.split(".")[1]
    return None, None
...
def relatedProxyAttributes(obj, attr):
    """Returns a list of related proxy attributes from an object or node and an attribute.

    :param obj: object or node name
    :type obj: str
    :param attr: attribute name
    :type attr: str
    :return relatedProxies: A list of related proxy attribute names, ["obj.attr", "obj.attr"]
    :rtype relatedProxies: list(str)
    """
    relatedProxies = []
    downstream = cmds.listConnections(".".join([obj, attr]), plugs=True, source=False, destination=True) or []
    if not downstream:
        return []
    for nodeAttr in downstream:
        if not attr:
            continue
        node = nodeAttr.split(".")[0]
        attr = nodeAttr.split(".")[-1]
        proxyAttrs = proxyAttributes(node)  # the name of the node
        if proxyAttrs:
            for proxyAttr in proxyAttrs:
                if attr == proxyAttr:
                    relatedProxies.append(nodeAttr)
                    continue
    return relatedProxies
...
def attributeDefault(node, attr, defaultValue, setValue=True, setProxies=False):
    ...
    :param setValue: Set the value to the default value
    :type setValue: bool
    :param setProxies: Set the proxy attributes to the default value
    :type setProxies: bool
    """
    ...
    if not setProxies:
        return
    # set proxies to default
    relatedProxies = relatedProxyAttributes(node, attr)
    if relatedProxies:
        for nodeProxyAttr in relatedProxies:
            cmds.addAttr(nodeProxyAttr, edit=True, defaultValue=defaultValue)
            if setValue:
                cmds.setAttr(".".join([node, attr]), defaultValue)
...
def labelAttr(labelName, node, checkExists=False):
    ...
    Label, separator, or divider attributes are useful for organizing attributes in the channel box.
...
def convert_str_attrType(node, attr, valueStr, message=True):
    """Converts a string attribute value to the appropriate type based on the attribute's type.

    Only supports float, bool, str and int types. Only converts the value from str to the appropriate type.

    :param node: The name of the Maya node
    :type node: str
    :param attr: The name of the attribute to convert
    :type attr: str
    :param valueStr: The string value to convert
    :type valueStr: str
    :param message: Report messages to the user?
    :type message: bool

    :return: The converted value, or None if the attribute does not exist
    :rtype: float, int, bool, or None
    """
    if not cmds.attributeQuery(attr, node=node, exists=True):
        if message:
            output.displayWarning("Attribute `{}` does not exist on node `{}`.".format(attr, node))
        return None
    if valueStr == "False" or valueStr == "false":
        return False
    elif valueStr == "True" or valueStr == "true":
        return True

    attrType = cmds.getAttr(".".join([node, attr]), type=True)  # get the attribute type
    if (attrType == "doubleLinear") or (attrType == "float") or (attrType == "double") or (attrType == "doubleAngle"):
        return float(valueStr)
    elif (attrType == "enum") or (attrType == "long") or (attrType == "double") or (attrType == "bool"):
        return int(valueStr)
    elif attrType == "string":
        return str(valueStr)
        ...
        if message:
            output.displayWarning(
                "Attribute `{}` on node `{}` is of type `{}`, which is not supported.".format(attr, node, attrType))
        return None
...
def lock_and_hide_default(node, hide=True):
    """
    ...
    """
    channelArray = ["tx", "ty", "tz", "rx", "ry", "rz", "sx", "sy", "sz", "v"]
...
def getChannelBoxAttrs(message=False):
    """Selected Channel box attributes. Returns the long name of the selected attribute from the channel box
...
# -----------------
# STORE SET DEFAULT ATTRS USED IN HIVE
# -----------------

def get_channelbox_visible_attrs(obj):
    """
    Returns all channel box visible attributes (keyable, unlocked, visible) for the object.

    This function combines the keyable and channelBox attributes, ensuring that both are included.

    :param obj: The Maya object or node name to get attributes from.
    :type obj: str
    :return: A list of attribute names that are keyable and visible in the channel box.
    :rtype: list(str)
    """
    attrs = cmds.listAttr(obj, keyable=True, unlocked=True) or []
    cb_attrs = cmds.listAttr(obj, channelBox=True) or []
    return list(set(attrs + cb_attrs))


def get_channelbox_attribute_list(objs):
    """Returns the attributes in the channel box for the given objects. Not selection based.

    Return list of [longName, attr, value] skipping defaults and underscores.

    :param objs: A list of Maya objects to get attributes from.
    :type objs: list(str)
    :return: A list of [obj, attr, value, proxy] where proxy is either "proxyAttr" or "normalAttr".
    :rtype: list(list(str, str, float, str))
    """
    result = []

    for obj in objs:
        attrs = get_channelbox_visible_attrs(obj)
        for attr in attrs:
            if attr in MAYA_DEFAULT_ATTRS:
                continue
            if re.compile(r'^_+$').match(attr):
                continue
            full_attr = ".".join([obj, attr])
            try:
                ...
                if isinstance(value, float):
                    value = round(value, 5)
                if isAProxyAttr(obj, attr):
                    proxy = "proxyAttr"
                else:
                    proxy = "normalAttr"
                result.append([obj, attr, value, proxy])
            except:
                continue

    result.sort(key=lambda x: x[0])  # sort alphabetically
    return result


def get_channelbox_attribute_list_sel(longNames=False):
    """Returns a list of channel box attributes for the selected objects.

    :param longNames: If True returns long names, if False returns short names.
    :type longNames: bool
    :return: A list of [obj, attr, value, proxy] where proxy is either "proxyAttr" or "normalAttr".
    :rtype: list(list(str, str, float, str))
    """
    ...
    result = []
    objs = cmds.ls(selection=True, long=longNames)

    selAttrs = getChannelBoxAttrs(message=False)  # Selected Channel box attributes.
    if not selAttrs:
        return get_channelbox_attribute_list(objs)  # no selected attrs in channel box only objects
    for obj in objs:
...
def get_channelbox_attribute_list_noProxy(data):
    """Converts the data to make sure all node.attributes are root attributes and not proxy attributes.

    :param data: A list of tuples in the format (obj, attr, value, proxy) where proxy is either "proxyAttr" or "normalAttr".
    :type data: list(tuple(str, str, float, str))
    :return: A list of tuples in the format (obj, attr, value, proxy) where proxy is either "proxyAttr" or "normalAttr".
    :rtype: list(tuple(str, str, float, str))
    """
    convertedList = list()
    if not data:
    ...
    if data[0] is None:
        return convertedList
    for obj, attr, value, proxy in data:
        if proxy == "proxyAttr":
            newObj, newAttr = find_proxy_source(obj, attr)
            if obj is not None:
                convertedList.append((newObj, newAttr, value, "normalAttr"))
            else:
                convertedList.append((obj, attr, value, "proxyAttr"))
        else:
            convertedList.append((obj, attr, value, "normalAttr"))
    convertedList.sort(key=lambda x: x[0])  # sort alphabetically
    return convertedList


def get_channelbox_attribute_list_proxy_sel(rootOfProxies=True, longNames=False, removeDefaultAttrs=True):
    """Returns a list of channel box attributes for the selected objects, including proxy attributes.

    :param rootOfProxies: If True converts proxy attributes to root attributes.
    :type rootOfProxies: bool
    :param longNames: If True returns long names, if False returns short names.
    :type longNames: bool
    :param removeDefaultAttrs: If True removes default attributes from the list, such as MAYA_DEFAULT_ATTRS.
    :type removeDefaultAttrs: bool
    :return: A list of [obj, attr, value, proxy] where proxy is either "proxyAttr" or "normalAttr".
    :rtype: list(list(str, str, float, str))
    """
    data = get_channelbox_attribute_list_sel(longNames=longNames)
    if removeDefaultAttrs:  # Remove default attributes MAYA_DEFAULT_ATTRS, they cannot be default set
        data = [item for item in data if item[1] not in MAYA_DEFAULT_ATTRS]
        # remove pure underscore attributes as they are dividers
        data = [item for item in data if not re.compile(r'^_+$').match(item[1])]
    if rootOfProxies:
        # Get the channel box attributes from the selection and convert to root attributes
        data = get_channelbox_attribute_list_noProxy(data)
    return data


def get_channel_attrs_proxy_sel_string(rootOfProxies=True, longNames=False):
    """ Returns a string of channel box attributes for the selected objects.

    The string is formatted as "object attribute value" for each attribute.

    :param rootOfProxies: If True converts proxy attributes to root attributes.
    :type rootOfProxies: bool
    :param longNames: If True returns long names, if False returns short names.
    :type longNames: bool
    :return: A string of channel box attributes formatted as "object attribute value".
    :rtype: str
    """
    attrDataString = ""
    attrDataList = get_channelbox_attribute_list_proxy_sel(rootOfProxies=rootOfProxies, longNames=longNames)
    if not attrDataList:
        return attrDataString

    for obj, attr, value, proxy in attrDataList:
        attrDataString += "{} {} {}\n".format(obj, attr, value)
    return attrDataString
