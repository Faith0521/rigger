...
.. code-block:: python
    ...
    # Create a control
    from cgrig.libs.maya.cmds.rig import controls
    ...
    # Delete CgRig Tracker Attrs from selected controls
    from cgrig.libs.maya.cmds.rig import controls
...
from cgrig.libs.maya.mayacommand.library import createshapefromlib
from cgrig.libs.toolsets import controls
from cgrig.libs.cgrigscene.constants import DEPENDENCY_FOLDER

from cgrig.libs.utils import color as colorutils, output
from cgrig.libs.maya.cmds.objutils import shapenodes, namehandling, objcolor, joints, alignutils, filtertypes, \
    scaleutils, attributes, objhandling
from cgrig.libs.maya.cmds.rig import nodes as nodeCreate
...
def createControlCurve(folderpath="", ctrlName="control", curveScale=(1.0, 1.0, 1.0), designName='circle',
    ...
    if folderpath:  # use the folder path
        if addToUndo:
            parentMObject, curveMObjects = executor.execute("cgrig.maya.curves.createFromFilePath",
        ...
        if addToUndo:
            ...
            parentMObject, curveMObjects = executor.execute("cgrig.maya.curves.createFromLib",
...
def createControlsOnJoints(jntList, folderpath="", curveScale=(1.0, 1.0, 1.0), rotateOffset=(0, -90, 90), grpJnts=True,
    ...
    :type jntList: list
    :param folderpath: Optional folder path of the .shape file. If empty "" then search within the CgRig internal library
    :type folderpath: str
    :param curveScale: The scale of each control in Maya units.  Is measured in radius, not width.
    :type curveScale: list(float)
    :param rotateOffset: rotate offset of the control, is a CV rotation offset not object
...
def createControlsMatch(folderpath="", matchObj="", newName=controls.NEW_CONTROL, curveScale=(1.0, 1.0, 1.0),
    ...
    if grp:  # Then group and match the group
        ...
        driven = cmds.createNode("transform", name="_".join([newName, "driven"]), p=grpName)
        newControl = cmds.parent(newControl, driven)[0]
        if matchObj:
...
def createControlsMatch_(folderpath="", matchObj="", newName=controls.NEW_CONTROL, curveScale=(1.0, 1.0, 1.0),
                        rotateOffset=(0, 0, 0), designName='circle', grp=True,
                        rgbColor=(0, 0, 1), lineWidth=-1, addToUndo=True):
    """Creates a control matching the rot and pos of an object does not parent.
    ...
    if grp:  # Then group and match the group
        grpName = "_".join([newName, filtertypes.GROUP_SX])
        grpName = cmds.group(name=grpName, em=True)
        con = cmds.createNode("transform", name="_".join([newName, "con"]), p=grpName)
        driven = cmds.createNode("transform", name="_".join([newName, "driven"]), p=con)
        newControl = cmds.parent(newControl, driven)[0]
        if matchObj:
            cmds.matchTransform([grpName, matchObj], pos=1, rot=1, scl=0, piv=0)
        grpName = namehandling.getLongNameFromShort(grpName)  # make long name
        grpNames["grp"] = grpName
        grpNames["con"] = con
        grpNames["sdk"] = driven
    elif matchObj:  # Don't create a group just match the control
        cmds.matchTransform([newControl, matchObj], pos=1, rot=1, scl=0, piv=0)
    newControl = namehandling.getLongNameFromShort(newControl)  # make long name
    return newControl, grpNames
...
def createControlsMatchList(objList, overrideName="", folderpath="", curveScale=(1.0, 1.0, 1.0), rotateOffset=(0, 0, 0),
                            designName='circle', grp=True, rgbColor=(0, 0, 1), lineWidth=-1, addToUndo=True, mode=0):
    ...
    for i, obj in enumerate(objList):
        ...
        if mode == 0:
            ...
            newControl, grpName = createControlsMatch_(folderpath=folderpath, matchObj=obj, newName=pureName,
                                                       curveScale=curveScale, rotateOffset=rotateOffset,
                                                       designName=designName, grp=grp,
                                                       rgbColor=rgbColor, lineWidth=lineWidth,
                                                       addToUndo=addToUndo)
...
def addTrackAttrs(transformName, scale=(1.0, 1.0, 1.0), color=None, rotate=(0.0, 0.0, 0.0),
    ...
    CGRIG_SCALE_DEFAULT_ATTR "cgrigScaleDefault" sets the default scale that generally will not change unless frozen (reset)
    ...
    :type designName: str
    """
    if not cmds.attributeQuery(controls.CGRIG_SCALE_TRACK_ATTR, node=transformName, exists=True):
        # trans rot and scale
        for i, attr in enumerate(controls.CGRIG_CONTROLTRACK_VECTOR_LIST):  # add a vector attribute for each trakcer attr
        ...
        # color
        for i, attr in enumerate(controls.CGRIG_CONTROLTRACK_RGB_LIST):  # add a vector attribute for each tracker attr
            cmds.addAttr(transformName, longName=attr, attributeType="double3")
            for letter in controls.RGB_LIST:  # add the "attrName_x" "attrName_y" and "attrName_z"
                cmds.addAttr(transformName, longName="{}_{}".format(attr, letter),
                             attributeType="double", parent=attr)
        # designName "circle"
        cmds.addAttr(transformName, longName=controls.CGRIG_SHAPE_TRACK_ATTR, dataType="string")
        cmds.addAttr(transformName, longName=controls.CGRIG_SHAPE_DEFAULT_ATTR, dataType="string")
    # set the attributes
    cmds.setAttr(".".join([transformName, controls.CGRIG_SCALE_TRACK_ATTR]), scale[0], scale[1], scale[2])
    cmds.setAttr(".".join([transformName, controls.CGRIG_SCALE_DEFAULT_ATTR]), scale[0], scale[1], scale[2])
    cmds.setAttr(".".join([transformName, controls.CGRIG_ROTATE_TRACK_ATTR]), rotate[0], rotate[1], rotate[2])
    cmds.setAttr(".".join([transformName, controls.CGRIG_ROTATE_DEFAULT_ATTR]), rotate[0], rotate[1], rotate[2])
    cmds.setAttr(".".join([transformName, controls.CGRIG_TRANSLATE_TRACK_ATTR]), translate[0], translate[1], translate[2])
    cmds.setAttr(".".join([transformName, controls.CGRIG_TRANSLATE_DEFAULT_ATTR]), translate[0], translate[1],
                 translate[2])
    if color:  # might be None
        cmds.setAttr(".".join([transformName, controls.CGRIG_COLOR_TRACK_ATTR]), color[0], color[1], color[2])
        cmds.setAttr(".".join([transformName, controls.CGRIG_COLOR_DEFAULT_ATTR]), color[0], color[1], color[2])
        ...
        if shapes:
            ...
            cmds.setAttr(".".join([transformName, controls.CGRIG_COLOR_TRACK_ATTR]), color[0], color[1], color[2])
            cmds.setAttr(".".join([transformName, controls.CGRIG_COLOR_DEFAULT_ATTR]), color[0], color[1], color[2])
    cmds.setAttr(".".join([transformName, controls.CGRIG_SHAPE_TRACK_ATTR]), designName, type="string")
    cmds.setAttr(".".join([transformName, controls.CGRIG_SHAPE_DEFAULT_ATTR]), designName, type="string")
...
def freezeScaleTracker(controlCurve):
    ...
    :rtype success: bool
    """
    if not cmds.attributeQuery(controls.CGRIG_SCALE_TRACK_ATTR, node=controlCurve, exists=True):  # tracker attr exists
        return False
    scaleXYZ = cmds.getAttr(".".join([controlCurve, controls.CGRIG_SCALE_TRACK_ATTR]))[0]
    cmds.setAttr(".".join([controlCurve, controls.CGRIG_SCALE_DEFAULT_ATTR]), scaleXYZ[0], scaleXYZ[1], scaleXYZ[2])
    return True
...
def deleteTrackAttrs(controlTransform):
    ...
    Attributes are found in:
        ...
        CGRIG_CONTROLTRACK_VECTOR_LIST
        CGRIG_CONTROLTRACK_RGB_LIST
        CGRIG_CONTROLTRACK_STRING_LIST

    :param controlTransform: The transform node that may contain the tracking attributes
    :type controlTransform: str
    """
    attrList = controls.CGRIG_CONTROLTRACK_VECTOR_LIST + controls.CGRIG_CONTROLTRACK_RGB_LIST + \
               controls.CGRIG_CONTROLTRACK_STRING_LIST + [controls.CGRIG_TEMPBREAK_MASTER_ATTR]
...
def deleteTrackAttrsSel(message=True):
    ...
    if message:
        output.displayInfo("Success: CgRig tracker attributes have been removed on the selected control curves.")
...
def colorUpdateTracker(cntrl, color):
    ...
    :type color: list(float)
    """
    if cmds.attributeQuery(controls.CGRIG_COLOR_TRACK_ATTR, node=cntrl, exists=True):  # then update
        cmds.setAttr(".".join([cntrl, controls.CGRIG_COLOR_TRACK_ATTR]), color[0], color[1], color[2])
...
def offsetHSVControl(cntrl, offset, hsvType="hue", linear=True):
    ...
    if cmds.attributeQuery(controls.CGRIG_COLOR_TRACK_ATTR, node=cntrl, exists=True):
...
def resetScale(controlCurve):
    ...
    if not cmds.attributeQuery(controls.CGRIG_SCALE_TRACK_ATTR, node=controlCurve, exists=True):  # tracker attr exists
        return
    currentScale = cmds.getAttr(".".join([controlCurve, controls.CGRIG_SCALE_TRACK_ATTR]))[0]
    dfltScale = cmds.getAttr(".".join([controlCurve, controls.CGRIG_SCALE_DEFAULT_ATTR]))[0]
    cmds.setAttr(".".join([controlCurve, controls.CGRIG_SCALE_TRACK_ATTR]), dfltScale[0], dfltScale[1], dfltScale[2])
...
def setTrackerScaleAbs(controlCurve, scaleXYZ):
    """While absolute scaling controls through the GUI, this function updates the scale track attr CGRIG_SCALE_TRACK_ATTR
    ...
    """
    if cmds.attributeQuery(controls.CGRIG_SCALE_TRACK_ATTR, node=controlCurve, exists=True):  # tracker attr exists
        cmds.setAttr(".".join([controlCurve, controls.CGRIG_SCALE_TRACK_ATTR]), scaleXYZ[0], scaleXYZ[1], scaleXYZ[2])
...
def setTrackerScaleOffset(controlCurve, scaleXYZ):
    """While offset scaling controls through the UI, this function updates the scale track attr CGRIG_SCALE_TRACK_ATTR
    ...
    cmds.setAttr(".".join([controlCurve, controls.CGRIG_SCALE_TRACK_ATTR]), trackerScale[0], trackerScale[1],
                 trackerScale[2])
...
def scaleAndTrackControlSelected(scaleXYZ):
    """While CV scaling control curves through the UI, this function updates the scale track attr CGRIG_SCALE_TRACK_ATTR
...
def getCreateControlScale(controlCurve):
    """Returns the CGRIG_SCALE_TRACK_ATTR attribute scale if it exists otherwise returns the size of the control \
    ...
    """
    if cmds.attributeQuery(controls.CGRIG_SCALE_TRACK_ATTR, node=controlCurve, exists=True):  # tracker attr exists
        trackerScale = cmds.getAttr(".".join([controlCurve, controls.CGRIG_SCALE_TRACK_ATTR]))[0]
        return trackerScale
    else:  # doesn't exist so measure the size of the control and create the tracker attributes
...
def getControlScaleList(controlCurveList):
    """Returns the CGRIG_SCALE_TRACK_ATTR attribute scale if it exists for objList otherwise returns the size of the \
...
def getAllTrackerInfo(controlCurve):
    """Returns the cgrig tracker attributes, if they don't exist it will create them with default values.
    ...
    rotate = cmds.getAttr(".".join([controlCurve, controls.CGRIG_ROTATE_TRACK_ATTR]))[0]
    translate = cmds.getAttr(".".join([controlCurve, controls.CGRIG_TRANSLATE_TRACK_ATTR]))[0]
    shape = cmds.getAttr(".".join([controlCurve, controls.CGRIG_SHAPE_TRACK_ATTR]))
    # get color properly
    shapes = cmds.listRelatives(controlCurve, shapes=True, fullPath=True, type="nurbsCurve")
    if shapes:
        color = objcolor.getRgbColor(shapes[0], hsv=False, linear=True)
    else:
        color = cmds.getAttr(".".join([controlCurve, controls.CGRIG_COLOR_TRACK_ATTR]))[0]
    return translate, rotate, scale, color, shape


def getControlDefaultScale(controlCurve):
    """Returns the CGRIG_SCALE_DEFAULT_ATTR attribute scale if it exists otherwise returns [1.0, 1.0, 1.0]

    :param controlCurve: The name of the control curve to retrieve the scale tracker values
    :type controlCurve: str
    """
    if cmds.attributeQuery(controls.CGRIG_SCALE_DEFAULT_ATTR, node=controlCurve, exists=True):  # tracker attr exists
        trackerScale = cmds.getAttr(".".join([controlCurve, controls.CGRIG_SCALE_DEFAULT_ATTR]))[0]
        return trackerScale
    return [1.0, 1.0, 1.0]
...
def replaceWithShapeDesign(controlName, folderpath="", designName="circle", autoScale=True,
    ...
    cmds.setAttr(".".join([controlName, controls.CGRIG_SHAPE_TRACK_ATTR]), designName, type="string")
    if message:
...
def replaceDesignBrkCnctCtrl(cntrl, folderpath="", designName="circle", autoScale=True, rotateOffset=(0, 0, 0),
    ...
    Break-off functionality maintains the ctrl's offset picot and scale information by using the cgrig tracker attrs. \
...
def duplicateControl(controlName, useSelectedName=False, copyScaleTracker=True, deleteNodeShapes=False):
    ...
    """
    cgrigScaleTrack = list()
    cgrigScaleDefault = [1.0, 1.0, 1.0]
    duplicatedCtrl = shapenodes.duplicateWithoutChildren(controlName, name='cgrig_tempControlXX',
    ...
    if cmds.nodeType(duplicatedCtrl) == "joint":  # is a joint so transfer shape to a transform node
        if copyScaleTracker:  # get the attribute values as they won't be duplicated
            cgrigScaleTrack = getCreateControlScale(controlName)
            cgrigScaleDefault = getControlDefaultScale(controlName)
        dupGrp = cmds.group(empty=True, name='cgrig_tempMirror_GrpXX')
        ...
        if copyScaleTracker:  # add the scale tracker attributes
            addTrackAttrs(duplicatedCtrl, cgrigScaleTrack)
            cmds.setAttr(".".join([duplicatedCtrl, controls.CGRIG_SCALE_DEFAULT_ATTR]), cgrigScaleDefault[0],
                         cgrigScaleDefault[1],
                         cgrigScaleDefault[2])
...
def breakTrackControl(ctrlName, applyTempColor=True, tempColor=(1.0, 0.0, 1.0), createNetwork=True):
    ...
    """
    if cmds.attributeQuery(controls.CGRIG_TEMPBREAK_CTRL_ATTR, node=ctrlName, exists=True):
        return "", "", ""
    networkName = ""
    ...
    if createNetwork:
        networkName = "_".join([newControlTransform, controls.CGRIG_TEMPBREAK_NETWORK])
        networkName = nodeCreate.messageNodeObjs(networkName, [newControlTransform], controls.CGRIG_TEMPBREAK_CTRL_ATTR)
        nodeCreate.messageNodeObjs(networkName, [grp], controls.CGRIG_TEMPBREAK_GRP_ATTR, createNetworkNode=False)
        if cmds.attributeQuery(controls.CGRIG_TEMPBREAK_MASTER_ATTR, node=ctrlName, exists=True):  # can already exist
            cmds.deleteAttr(ctrlName, attribute=controls.CGRIG_TEMPBREAK_MASTER_ATTR)
        nodeCreate.messageNodeObjs(networkName, [ctrlName], controls.CGRIG_TEMPBREAK_MASTER_ATTR, createNetworkNode=False)
...
def reconnectBreakControl(breakCntrl, masterCntrl, replaceGrp, ignoreScale=False):
    ...
    designName = cmds.getAttr(".".join([breakCntrl, controls.CGRIG_SHAPE_TRACK_ATTR]))
    color = cmds.getAttr(".".join([breakCntrl, controls.CGRIG_COLOR_TRACK_ATTR]))[0]
    ...
    cmds.setAttr(".".join([masterCntrl, controls.CGRIG_TRANSLATE_TRACK_ATTR]), translate[0], translate[1], translate[2])
    cmds.setAttr(".".join([masterCntrl, controls.CGRIG_ROTATE_TRACK_ATTR]), rotate[0], rotate[1], rotate[2])
    if not ignoreScale:  # set scale as always positive for flipping
        cmds.setAttr(".".join([masterCntrl, controls.CGRIG_SCALE_TRACK_ATTR]), scale[0], scale[1], scale[2])
    cmds.setAttr(".".join([masterCntrl, controls.CGRIG_COLOR_TRACK_ATTR]), color[0], color[1], color[2])
    cmds.setAttr(".".join([masterCntrl, controls.CGRIG_SHAPE_TRACK_ATTR]), designName, type="string")
    # Set the color
...
def reconnectBreakTempControlNetwork(breakCntrl):
    ...
    networkNode = nodeCreate.getNodeAttrConnections([breakCntrl], controls.CGRIG_TEMPBREAK_CTRL_ATTR)[0]
    masterCntrl = nodeCreate.getNodeAttrConnections([networkNode], controls.CGRIG_TEMPBREAK_MASTER_ATTR)[0]
    replaceGrp = nodeCreate.getNodeAttrConnections([networkNode], controls.CGRIG_TEMPBREAK_GRP_ATTR)[0]
    # break off the control
    reconnectBreakControl(breakCntrl, masterCntrl, replaceGrp)
    # Cleanup the remaining empty group and network node, and remove the network attr on the master control
    cmds.delete(networkNode)
    cmds.deleteAttr(masterCntrl, attribute=controls.CGRIG_TEMPBREAK_MASTER_ATTR)
...
def reconnectAllBreakCtrls(message=True):
    ...
    networkNodeList = [s for s in allNetworkNodes if
                       controls.CGRIG_TEMPBREAK_NETWORK in s]  # returns all with break name matches
    if not networkNodeList:
        if message:
            output.displayWarning("No break off controls found connected to network nodes "
                                  "containing the name `{}`".format(controls.CGRIG_TEMPBREAK_NETWORK))
        return
    for network in networkNodeList:  # Loop over the network nodes
        # Get the grp, masterCtrl and brokenCtrl add to lists
        breakCtrlTestList = nodeCreate.getNodeAttrConnections([network], controls.CGRIG_TEMPBREAK_CTRL_ATTR)
        ...
        masterCntrl = nodeCreate.getNodeAttrConnections([network], controls.CGRIG_TEMPBREAK_MASTER_ATTR)[0]
        replaceGrp = nodeCreate.getNodeAttrConnections([network], controls.CGRIG_TEMPBREAK_GRP_ATTR)[0]
    ...
    if not masterCtrlList:
        if message:
            output.displayWarning("No break off controls found in the scene connected to network "
                                  "nodes named".format(controls.CGRIG_TEMPBREAK_NETWORK))
        return
    if message:
...
def scaleBreakConnectCtrl(cntrl, scaleXYZ, relative=True):
    ...
    Break-off uses the cgrig tracker information to build the object space.
    ...
    :type relative: bool
    """
    # Check if cgrigTranslateTrack_x attribute exists
    if cmds.attributeQuery("cgrigTranslateTrack", node=cntrl, exists=True):
        translateOffset = cmds.getAttr("{}.cgrigTranslateTrack".format(cntrl))[0]
        rotateOffset = cmds.getAttr("{}.cgrigRotateTrack".format(cntrl))[0]
        # If no translate or rotation then scale with CV scale
        if translateOffset == (0.0, 0.0, 0.0) and rotateOffset == (0.0, 0.0, 0.0):
            oldScale = cmds.getAttr("{}.cgrigScaleTrack".format(cntrl))[0]
            if relative:
            ...
            # Update scale meta
            cmds.setAttr("{}.cgrigScaleTrack".format(cntrl), newScale[0], newScale[1], newScale[2])
            return
...
def scaleBreakConnectCtrlList(cntrlList, scaleXYZ, relative=True):
    ...
    """
    for ctrl in cntrlList:
        if not cmds.attributeQuery(controls.CGRIG_TEMPBREAK_CTRL_ATTR, node=ctrl, exists=True):  # Regular control
...
def rotateBreakConnectCtrlList(cntrlList, rotateXYZ, relative=True):
    ...
    reconnects. Break-off uses the cgrig tracker information to build the object space.
    ...
    """
    for ctrl in cntrlList:
        if not cmds.attributeQuery(controls.CGRIG_TEMPBREAK_CTRL_ATTR, node=ctrl, exists=True):  # not already broken off
...
def scaleResetBrkCnctCtrl(cntrl):
    """Resets a control scales with breakTrackControl(); as per the cgrig tracker stored default scale attribute. \
    Performs the scale in the shape's object space and then reconnects.  Break-off uses the cgrig tracker information to \
    ...
    brokenOffCtrl, replaceGrp, networkNode = breakTrackControl(cntrl, applyTempColor=False, createNetwork=False)
    # do the scale
    dfltScale = cmds.getAttr(".".join([cntrl, controls.CGRIG_SCALE_DEFAULT_ATTR]))[0]
...
def scaleResetBrkCnctCtrlList(cntrlList):
    """Resets a list of controls with breakTrackControl(); as per the cgrig tracker stored default scale attribute. \
    Performs the scale in the shape's object space and then reconnects.  Break-off uses the cgrig tracker information to \
    build the object space.
    ...
    for ctrl in cntrlList:
        if not cmds.attributeQuery(controls.CGRIG_TEMPBREAK_CTRL_ATTR, node=ctrl, exists=True):  # Regular control
            ...
            dfltScale = cmds.getAttr(".".join([ctrl, controls.CGRIG_SCALE_DEFAULT_ATTR]))[0]
            cmds.scale(dfltScale[0], dfltScale[1], dfltScale[2], ctrl, relative=False, absolute=True)


def rotateResetBrkCnctCtrl(cntrl):
    """Resets a control with breakTrackControl(); as per the cgrig tracker stored default rotate attribute. \
    Performs the rotate in the shape's object space and then reconnects.  Break-off uses the cgrig tracker information to \
    build the object space.
    ...
    brokenOffCtrl, replaceGrp, networkNode = breakTrackControl(cntrl, applyTempColor=False, createNetwork=False)
    # do the scale
    dfltRotate = cmds.getAttr(".".join([cntrl, controls.CGRIG_ROTATE_DEFAULT_ATTR]))[0]
...
def rotateResetBrkCnctCtrlList(cntrlList):
    """Resets a list of controls with breakTrackControl(); as per the cgrig tracker stored default rotate attribute. \
    Performs the rotate in the shape's object space and then reconnects.  Break-off uses the cgrig tracker information to \
    build the object space.
    ...
    for ctrl in cntrlList:
        if not cmds.attributeQuery(controls.CGRIG_TEMPBREAK_CTRL_ATTR, node=ctrl, exists=True):  # Regular control
            ...
            dfltRotate = cmds.getAttr(".".join([ctrl, controls.CGRIG_ROTATE_DEFAULT_ATTR]))[0]
...
def mirrorControl(currentCntrl, destinationCntrl, mirrorAxis="X", keepColor=True):
    ...
    mirrorPivotGrp = cmds.group(empty=True, name='cgrig_tempMirrorPivotXX')
...
def copyControl(ctrl):
    """Copies the cgrig tracker information from a control. Returns:
    ...
    :type ctrl: str
    :return translate: The cgrig tracker translation value as [X, Y, Z]
    :rtype translate: list(float)
    :return rotate: The cgrig tracker rotation value as [X, Y, Z]
    :rtype rotate: list(float)
    :return scale: The cgrig tracker scale value as [X, Y, Z]
    :rtype scale: list(float)
    :return color: The cgrig tracker color value as [R, G, B], color is linear float 0-1
    :rtype color: list(float)
    :return shape: The name of the cgrig tracker design shape of the control ie "circle"
    :rtype shape: str
    """
...
def pasteControl(ctrlList, pasteCtrl, translate, rotate, scale, color, shape):
    """Pastes the curve shape nodes and cgrig tracker information onto a list of controls.
    ...
    :type pasteCtrl: str
    :param translate: The cgrig tracker translation value as [X, Y, Z]
    :type translate: list(float)
    :param rotate: The cgrig tracker rotation value as [X, Y, Z]
    :type rotate: list(float)
    :param scale: The cgrig tracker scale value as [X, Y, Z]
    :type scale: list(float)
    :param color: The cgrig tracker color value as [R, G, B], color is linear float 0-1
    :type color: list(float)
    :param shape: The name of the cgrig tracker design shape of the control ie "circle"
    :type shape: str
    """
    dupCtrlList = list()
...
# ---------------------------
# CONTROLLER TAG HELPER FUNCTIONS
# ---------------------------

def find_controller_nodes(transform_name):
    """
    Given a transform node name, return a list of child nodes
    of type 'controller'.

    :param transform_name: Name of the transform node to search for controller nodes.
    :type transform_name: str
    """
    # listRelatives returns child nodes of the specified type if found
    controllers = cmds.listConnections(transform_name, connections=True, type='controller', plugs=False)
    for controller in controllers:
        if cmds.nodeType(controller) != 'controller':
            controllers.remove(controller)
    return controllers if controllers else []


def tag_selected_transforms_as_controllers(transforms, message=True):
    """Tags each selected transform as a controller by creating/connecting
    a 'controller' node. If a transform is already tagged, it will skip it.

    :param transforms: Names of the transforms to tag as controllers.
    :type transforms: list(str)
    :param message: Report the message to the user?
    :type message: bool
    """
    for transform_name in transforms:
        # Check if it's already tagged as a controller
        existing_controller = find_controller_nodes(transform_name)
        if existing_controller:
            if message:
                output.displayWarning(
                    "Transform '{}' is already tagged with a controller node: '{}'".format(transform_name,
                                                                                           existing_controller[0]))
            continue
        try:
            # Tag the transform as a controller. This creates/attaches
            # a 'controller' node to the transform.
            controller_node = cmds.controller(transform_name)
            if message:
                output.displayInfo(
                    "Tagged '{}' as a controller. Created node: '{}'".format(transform_name, controller_node))
        except Exception as e:
            output.displayInfo("Failed to tag '{}' as controller. Reason: {}".format(transform_name, e))


def set_controller_visibility_mode(transform_names, visibility_mode, message=False):
    """
    Finds the 'controller' node of the given transform and sets its 'visibilityMode'
    attribute to the specified value (0, 1, or 2).

    :param transform_names: Names of the transforms to check for a controller node.
    :type transform_names: str
    :param visibility_mode: Desired visibility mode (0, 1, or 2).
    :type visibility_mode: int
    :return: True if successful, False otherwise.
    :rtype: bool
    """
    if visibility_mode not in [0, 1, 2]:
        return False

    # Find child controller nodes
    controller_nodes = list()
    for transform_name in transform_names:
        controller_nodes.extend(find_controller_nodes(transform_name))

    if not controller_nodes:
        output.displayWarning("No controller node/s found under transform/s: {}".format(transform_names))
        return False

    for controller in controller_nodes:
        try:
            cmds.setAttr("{}.visibilityMode".format(controller), visibility_mode)
        except Exception as e:
            if message:
                output.displayWarning("Failed to set visibility mode: {}".format(e))
            return False
    return True


def connect_masterObj_controller_sel(masterObj, masterAttr="controlMode", message=True):
    """Connects the master object to the controller nodes of the selected objects.
    Good for batch controlling controller visibility modes.
    Note no error checks

    :param masterObj: The master object to connect to the controller nodes
    :type masterObj: str
    :param masterAttr: The master attribute
    :type masterAttr: str
    :param message: Report the message to the user?
    :type message: bool
    """
    ...
    for obj in sel:
        controllerNodes = find_controller_nodes(obj)
        if controllerNodes:
            control_attr = "{}.visibilityMode".format(controllerNodes[0])
            cmds.connectAttr(".".join([masterObj, masterAttr]), control_attr, force=True)
            if message:
                output.displayInfo("Connected {} to {}".format(masterObj, controllerNodes[0]))
