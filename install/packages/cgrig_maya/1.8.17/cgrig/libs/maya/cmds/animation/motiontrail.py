...
from cgrig.libs.maya.cmds.objutils import namehandling, filtertypes
...
def createKeyframeTrackAttrs(mTShape):
    """On a single motion trail create the cgrig tracker attributes for keyframe dot scale and visibility
    ...
    :rtype: bool
    """
    if cmds.attributeQuery("cgrigKeyframeSize", node=mTShape, exists=True):
        return False
    # Attributes don't exist so create and return defaults. ----------------------
    cmds.addAttr(mTShape, longName="cgrigKeyframeSize", attributeType='float')
    cmds.addAttr(mTShape, longName="cgrigKeyframeVis", attributeType='bool')
    ...
    if val < 0.5:
        ...
        size = 3.0
        vis = True
    cmds.setAttr("{}.cgrigKeyframeSize".format(mTShape), size)
    cmds.setAttr("{}.cgrigKeyframeVis".format(mTShape), vis)
    return True


def keyframeTrackAttrs(mTShape):
    """On a single motion trail return the cgrig tracker attributes for keyframe dot scale and visibility.
    ...
    size = cmds.getAttr("{}.cgrigKeyframeSize".format(mTShape))
    vis = cmds.getAttr("{}.cgrigKeyframeVis".format(mTShape))
    return size, vis
...
def setKeyDotsVis(moTShapes, visible):
    """Sets the visibility of the key frame dots, annoying function that also takes into consideration the cgrig scale \
    toggle small/large.

    The keyframeSize attribute doubles as visiblity (0.1) and scale larger sizes.  So its difficult for UI toggles.
    CgRig creates trackable attributes for monitoring keyframe dot scale and size independently.
    ...
    for mTShape in moTShapes:
        ...
        cmds.setAttr("{}.cgrigKeyframeVis".format(mTShape), visible)  # set the cgrig tracker is attr
        if not visible:
            size = 0.1  # invisible
        else:
            size = cmds.getAttr("{}.cgrigKeyframeSize".format(mTShape))
...
def setKeyDotsSize(moTShapes, sizeBool):
    """Sets the scale of the keyframe dots, annoying function that also takes into consideration the cgrig visibility \
    toggle on/off.
    ...
    for mTShape in moTShapes:
        createKeyframeTrackAttrs(mTShape)  # creates attrs if they don't exist
        cmds.setAttr("{}.cgrigKeyframeSize".format(mTShape), size)  # set the cgrig tracker is attr
        vis = cmds.getAttr("{}.cgrigKeyframeVis".format(mTShape))
        if not vis:
            size = 0.1  # invisible
...
class CgRigMotionTrailTrackerSingleton(object):
...
def cvCurveFromObjAnimation(obj, startTime, endTime, cvEveryFrame=1.0):
    ...
    cmds.rebuildCurve(cvCurve,
                      ...
                      degree=3)
    # Rename the curve -------------------------------------------
    cvCurve = cmds.rename(cvCurve, "{}_crv".format(obj))
