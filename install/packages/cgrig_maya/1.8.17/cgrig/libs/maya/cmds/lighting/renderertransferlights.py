...
.. code-block:: python
    ...
    from cgrig.libs.maya.cmds.lighting import renderertransferlights
...
.. code-block:: python
    ...
    lightApplyAttrDict['gIblTexture'] = r"C:/Users/Andrew Silke/Documents/cgrig_preferences/assets/light_suite_ibl_skydomes/sky_clear/HDRI-SKIES_Sky215.hdr"
...
from cgrig.libs.maya.cmds.objutils import attributes, namehandling, shapenodes, objhandling, matching, scaleutils
from cgrig.libs.maya.cmds.lighting import redshiftlights, arnoldlights, rendermanlights, lightingutils
from cgrig.libs.maya.cmds.cameras import cameras
from cgrig.libs.maya.cmds.renderer.rendererconstants import RENDERER_SUFFIX, RENDERER_SUFFIX_DICT
from cgrig.libs.maya.cmds.renderer import rendererload, multirenderersettings
from cgrig.libs.maya.cmds.textures import textures
from cgrig.libs.general import exportglobals
from cgrig.libs.utils import color, output
...
ARNOLDSKYDOMEATTRDICT = {INTENSITY: "intensity",
                         ...
                         IBLTEXTURE: "color"}  # in 2024 and above exposure is "aiExposure"
...
def getRendererAttrDictSkydome(rendererNiceName):
    ...
    if rendererNiceName == REDSHIFT:
        ...
        arnoldMajorVersion, arnoldMinorVersion, _ = arnoldVersionNumber()
        if arnoldMajorVersion >= 5 and arnoldMinorVersion >= 3:
            attrDict[EXPOSURE] = "aiExposure"
    elif rendererNiceName == RENDERMAN:
...
def scaleAllLightsInScene(scalePercentage, rendererNiceName, scalePivot=(0.0, 0.0, 0.0), ignoreNormalization=False,
                          ignoreIbl=False, ignoreDirectionals=False, importUnitAdjust=False, message=True,
...
def cgrigHdriPaths():
    """Returns all the HDRI paths that is being used by cgrig tools.

    :return hdriPaths: A list of all the folder paths in the skydome cgrig preferences
    :rtype hdriPaths: list(str)
    """
    hdriPaths = list()
    try:
        from cgrig.preferences.interfaces import lightsuiteinterfaces
...
def checkIBLTextureFound(iblTexturePath):
    """Checks if the IBL path exists, if it doesn't check to see if the file exists in the HDRI CgRig Prefs folders.
    ...
    hdriPaths = cgrigHdriPaths()  # all HDRI paths used by cgrig tools
    if not hdriPaths:
        return
    ...
    output.displayWarning("The HDRI texture file does not exist and CgRig could find a replacement. "
...
def createAreaLight(renderer="Auto", name="areaLight", exposure=16.0, intensity=1.0, addSuffix=True,
    ...
    :param renderer: "Auto" uses the CgRig primary renderer, "Redshift", "Arnold" or "Renderman"
    :type renderer: str
    ...
    """
    if renderer == "Auto":
        renderer = rendererload.currentCgRigRenderer()

    if renderer == "VRay":
        output.displayWarning("VRay directional lights `{}` are not yet supported by CgRig Tools.".format(renderer))
        return "", ""

    if renderer == "Maya":
        output.displayWarning("Maya directional lights `{}` are not yet supported by CgRig Tools.".format(renderer))
        return "", ""
...
def createDirectionalLight(renderer="Auto", name="directionalLight", addSuffix=True, position="selected", intensity=1.0,
    ...
    if renderer == "VRay":
        output.displayWarning("VRay directional lights are not yet supported by CgRig Tools.")
        return "", ""

    if renderer == "Maya":  # Then use Arnold
        renderer = "Arnold"
        if not rendererload.loadRenderer("Arnold", setCgRigDefaults=False):
...
def createHdriLight(renderer="Auto", name="hdriSkydomeLight", addSuffix=True, intensity=1.0, exposure=0.0,
    ...
    :param renderer: "Auto" uses the CgRig primary renderer, "Redshift", "Arnold" "VRay", or "Renderman".
    :type renderer: str
    :param name: The name of the light to be created
    :type name: str
    :param intensity: The intensity value
...
def createAreaLightMatchPos(name, renderer, warningState=False, addSuffix=False, replaceLight=False, position="world"):
    ...
    if position == "selected":  # create by matching to the first selected object
        ...
        if cam:
            matching.matchCgRigAlSimpErrConstrain(cam, lightTransformName)
        else:
