...
from cgrig.libs.maya.cmds.objutils import filtertypes, attributes, namehandling
from cgrig.libs.maya.cmds.rig import nodes
from cgrig.libs.maya.cmds.shaders import rendermandisplacement, redshiftdisplacement, arnolddisplacement, shaderutils
from cgrig.libs.maya.cmds.shaders.shdmultconstants import DISP_ATTR_TYPE, DISP_ATTR_DIVISIONS, DISP_ATTR_SCALE, \
...
def displaceMessageNodeSetup(meshList, shadingGroupList, fileNodeList, place2dTextureNodeList, displacementNode,
                             renderer, autoBump):
    """Connects displacement nodes to a network node named "cgrigDisplacement_*" (NW_DISPLACE_NODE)
...
def deleteDisplacementNetworkNodes(nodeList, message=True):
    ...
    if not networkNodeList:
        if message:
            om2.MGlobal.displayWarning("No CgRig displacment found on the selected nodes")
        return False
