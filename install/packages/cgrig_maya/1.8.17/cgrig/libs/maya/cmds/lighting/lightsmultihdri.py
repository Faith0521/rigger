...
.. code-block:: python
    ...
    from cgrig.libs.maya.cmds.lighting import lightsmultihdri
...
.. code-block:: python
    ...
    # Default HDRI dictionary example from cgrig.libs.maya.cmds.lighting.lightconstants
...
from cgrig.libs.maya.cmds.lighting import lightingutils, lightconstants as lgtcn
from cgrig.libs.maya.cmds.lighting.hdritypes import hdribase, arnoldhdri, redshifthdri, rendermanhdri, vrayhdri
from cgrig.libs.maya.cmds.renderer import multirenderersettings
...
def hdriInstancer(hdriType, hdriName="", create=False, ingest=True, suffixName=False, message=False):
    ...
    :type ingest: bool
    :return: A cgrig HDRI instance of the specified light.
...
def hdriInstanceFromHdri(hdriShape):
    ...
    :type hdriShape: str
    :return: A cgrig hdri instance of the specified hdri, None if the hdri does not exist
    :rtype: :class:`hdribase.HdriBase`
    """
...
def createHdriInstanceType(hdriType, hdriName="", imagePath="", suffixName=False, message=True):
    ...
    :type hdriName: str
    :param imagePath: Adds the image path to a HDRI texture image, if "auto" will use the default cgrig lores HDRI
    :type imagePath: str
    :param message: Report a message to the user
    :type message: bool

    :return: A cgrig hdri instance of the specified hdri type
    :rtype: :class:`hdribase.HdriBase`
    """
...
def createDefaultHdriRenderer(renderer, hdriName="", imagePath="", lightGroup=True, select=True, suffixName=False,
    ...
    if lightGroup:
        hdriInstance.parentCgRigLightGroup()
    if select:
...
def createDefaultHdriAutoRenderer(hdriName="", imagePath="", lightGroup=True, select=True, suffixName=False,
                                  message=True):
    """Creates a default hdri skydome from the renderer that's currently loaded in CgRig Tools Pro.
...
def hdriInstancesFromNodes(nodeTransforms, hdriTypes=lgtcn.SKYDOME_TYPES, message=True):
    ...
    :return: A list of cgrig hdri instances
...
def hdriInstancesFromSelected(hdriTypes=lgtcn.SKYDOME_TYPES, message=True):
    """From selected HDRI lights return a list of associated cgrig HDRI instances
...
def hdriInstanceRenderer(renderer, message=True):
    ...
    :param message: Report warnings to the user?
    :type message: bool
    :return: A cgrig hdri instance or None if none found.
    :rtype: :class:`hdribase.HdriBase`
    """
...
def hdriDependencyPath(imageFilePath):
    ...
    :return: Fullpath to dependency Folder and cgriginfo file.
    :rtype: tuple[str, str]
    """
    ...
    cgrigInfoFile = ".".join([nameOnly, constants.CGRIG_INFO_EXT])
    return os.path.join(path, directoryName), os.path.join(path, directoryName, cgrigInfoFile)


def saveExtraDataJson(intensityMultiplier, rotationOffset, imageFilePath, message=True):
    """Saves a cgrigInfoFilePath JSON file with the intensity multiplier and rotation offset data given a HDRI image path.
    ...
    :return cgrigInfoFilePath: The file path of the file saved
    :rtype cgrigInfoFilePath: str
    """
    dependencyPath, cgrigInfoFilePath = hdriDependencyPath(imageFilePath)
    if not os.path.exists(dependencyPath):
        os.mkdir(dependencyPath)
    if not os.path.exists(cgrigInfoFilePath):
        cgrigInfoDict = dict()
        filesystem.saveJson(cgrigInfoDict, cgrigInfoFilePath)  # save empty file
    else:
        cgrigInfoDict = filesystem.loadJson(cgrigInfoFilePath)
    cgrigInfoDict[HDRI_INTENSITY_MULTIPLIER] = intensityMultiplier
    cgrigInfoDict[HDRI_ROTATE_OFFSET] = rotationOffset
    filesystem.saveJson(cgrigInfoDict, cgrigInfoFilePath, indent=4, separators=(",", ":"), message=message)
    return cgrigInfoDict, cgrigInfoFilePath
...
def loadExtraDataJson(imageFilePath):
    ...
    if not os.path.exists(cgrigInfoFilePath):
        return None, None
    cgrigInfoDict = filesystem.loadJson(cgrigInfoFilePath)
    if HDRI_INTENSITY_MULTIPLIER not in cgrigInfoDict:
        return None, None
    return cgrigInfoDict[HDRI_INTENSITY_MULTIPLIER], cgrigInfoDict[HDRI_ROTATE_OFFSET]
