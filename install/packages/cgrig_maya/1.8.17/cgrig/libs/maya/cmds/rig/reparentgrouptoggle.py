# -*- coding: utf-8 -*-
"""
Creates and toggles parenting of groups containing '_cgrigParentToRig' into and out of _cgrigParentToRig
...
from cgrig.libs.maya.cmds.objutils import namewildcards, namehandling

REPARENT_SUFFIX = "_cgrigParentToRig"
...
def createGroupToRig(parentName, wildcardSuffix=REPARENT_SUFFIX, matchObject=True, message=True):
    ...
    Creates and matches new group:
        ...
        "parentName_cgrigParentToRig"

    :param parentName: The short name name of the parent obj, will create "parentName_cgrigParentToRig"
    :type parentName: str
    ...
    for parent in parentList:
        ...
        if namehandling.nodeListIsUniqueName(parentList):  # Check short name is unique
            om2.MGlobal.displayWarning("The name `{}` is not unique to the scene. Please rename.  "
                                       "Can use CgRig Renamer > Make Unique".format(parentName))
            errorState = True
            continue
...
def parentReparentGrpScene(parent=True, wildcardSuffix=REPARENT_SUFFIX, matchObject=True, message=True):
    ...
    grpParentList = list()
    errorState = False
    # Find all objects in scene with suffix '_cgrigParentToRig'
