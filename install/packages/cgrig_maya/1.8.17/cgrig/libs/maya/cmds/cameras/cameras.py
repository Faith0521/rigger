...
from cgrig.libs.maya.cmds.objutils import objhandling
...
CGRIG_APERTURE_INCH_WIDTH_ATTR = "cgrigApertureWidthI"
CGRIG_APERTURE_INCH_HEIGHT_ATTR = "cgrigApertureHeightI"
CGRIG_APERTURE_BOOL = "cgrigApertureBool"
CGRIG_APERTURE_PREVIOUS_FIT = "cgrigPreviousFit"
CGRIG_FIT_LIST = "Fill:Horizontal:Vertical:Overscan"
...
def allCamTransformsDag(camType=CAM_TYPE_ALL):
    ...
    :type camType:
    :return:
    :rtype: list[:class:`cgrig.libs.maya.zapi.DagNode`]
    """
...
def setCamResGate(camShape, on=True, resGateOnly=True, message=True):
    ...
    :type on: bool
    :param resGateOnly: Only sets the resGate, does not set the CgRig defaults
    :type resGateOnly: bool
    :param message: Report a message to the user?
    :type message: bool
    """
    ...
    if resGateOnly:  # Only toggle the resGate, skip CgRig settings
        setCamResolutionGate(camShape, int(on))
        if message:
            om2.MGlobal.displayInfo('Success: `{}` resolution gate {}'.format(camShape, out))
        return

    setCgRigResolutionGate(camShape, resolutionGate=on)
...
def createCgRigApertureAttrs(camShape):
    """Adds the custom attributes "cgrigApertureWidthInches" and "cgrigApertureHeightInches" and set their values from the \
    Maya camera settings.

    The attrs are used to store and measure the film gate size (camera sensor size) by cgrig tools which will override \
    Maya's settings.
    ...
    if not cmds.attributeQuery(CGRIG_APERTURE_INCH_HEIGHT_ATTR, node=camShape, exists=True):  # may exist earlier version
        cmds.addAttr(camShape, longName=CGRIG_APERTURE_INCH_WIDTH_ATTR, attributeType='float')
        cmds.addAttr(camShape, longName=CGRIG_APERTURE_INCH_HEIGHT_ATTR, attributeType='float')
        cmds.setAttr("{}.{}".format(camShape, CGRIG_APERTURE_INCH_WIDTH_ATTR), inchesWidth)
        cmds.setAttr("{}.{}".format(camShape, CGRIG_APERTURE_INCH_HEIGHT_ATTR), inchesHeight)
    # add the boolean and the remember combo
    cmds.addAttr(camShape, longName=CGRIG_APERTURE_BOOL, attributeType='bool')
    cmds.setAttr("{}.{}".format(camShape, CGRIG_APERTURE_BOOL), False)
    cmds.addAttr(camShape, longName=CGRIG_APERTURE_PREVIOUS_FIT, attributeType='enum', enumName=CGRIG_FIT_LIST)
    return inchesWidth, inchesHeight


def matchResolutionFilmGate(camShape, fitResGate=CAM_FIT_RES_OVERSCAN, getCgRigGate=True, matchGates=True):
    ...
    :type fitResGate: int
    """
    gateWidth, gateHeight, gateRatio = sensorSizeInches(camShape, getCgRigGate=getCgRigGate)
...
def setCgRigResolutionGate(camShape, resolutionGate=True, overscan=1.05, matchResGateRatios=True,
                         gateMaskOpacity=0.5, affectGateMask=True, fitResGate=CAM_FIT_RES_OVERSCAN):
    """Sets resolution gate with default CgRig settings:
...
def matchCamFilmBackRatio(camShape, ratio, keepVerticalSensorSize=False, useCgRigGate=True):
    ...
    If useCgRigGate:
        ...
        Will use the CgRig Tools stored attributes instead of Maya's default to do all the calculations
        CgRig messes with Maya's sensor size, so it needs to stores and query it separately
    ...
    :param useCgRigGate: The vertical height sensor size of the camera is kept, otherwise horizontal
    :type useCgRigGate: float
    """
    # If querying the cgrig attributes and they don't exist then create them
    if useCgRigGate and not cmds.attributeQuery(CGRIG_APERTURE_BOOL, node=camShape, exists=True):
        createCgRigApertureAttrs(camShape)
    ...
    if keepVerticalSensorSize:  # Adjust the vertical film aperture to match the resolution ratio
        if useCgRigGate:  # Use CgRig's stored settings
            verticalFilmAperture = cmds.getAttr("{}.{}".format(camShape, CGRIG_APERTURE_INCH_HEIGHT_ATTR))
        ...
        if useCgRigGate:  # Use CgRig's stored settings
            horizontalFilmAperture = cmds.getAttr("{}.{}".format(camShape, CGRIG_APERTURE_INCH_WIDTH_ATTR))
    ...
    if useCgRigGate:  # set tracker Attr to True
        cmds.setAttr('.'.join([camShape, CGRIG_APERTURE_BOOL]), True)
        cmds.setAttr('.'.join([camShape, CGRIG_APERTURE_PREVIOUS_FIT]), cmds.getAttr('{}.filmFit'.format(camShape)))


def unmatchCamFilmBackRatio(camShape):
    """Unmatches the film gate only after it has been set by cgrig tools. Requires CgRig custom attrs on cameras.

    :param camShape: A maya camShape node
    :type camShape: str
    """
    if not cmds.attributeQuery(CGRIG_APERTURE_BOOL, node=camShape, exists=True):
        createCgRigApertureAttrs(camShape)
        return
    horizontalFilmAperture = cmds.getAttr("{}.{}".format(camShape, CGRIG_APERTURE_INCH_WIDTH_ATTR))
    verticalFilmAperture = cmds.getAttr("{}.{}".format(camShape, CGRIG_APERTURE_INCH_HEIGHT_ATTR))
    cmds.setAttr('{}.verticalFilmAperture'.format(camShape), verticalFilmAperture)
    cmds.setAttr('{}.horizontalFilmAperture'.format(camShape), horizontalFilmAperture)
    cmds.setAttr('{}.{}'.format(camShape, CGRIG_APERTURE_BOOL), False)  # Set tracker Attr to False
    cmds.setAttr('{}.filmFit'.format(camShape), cmds.getAttr('.'.join([camShape, CGRIG_APERTURE_PREVIOUS_FIT])))


def cgrigMatchGateBoolValue(camShape):
    """Returns the value of the CgRig Aperture Boolean, a custom attribute that tracks the Match Gate state.

    :param camShape: A maya camShape node
    :type camShape: str
    :return cgrigMatchBoolValue: True if the cams are currently matched, False if not.
    :rtype cgrigMatchBoolValue: bool
    """
    if not cmds.attributeQuery(CGRIG_APERTURE_BOOL, node=camShape, exists=True):
        try:  # not sure may error?
            ...
            pass
        return False
    return cmds.getAttr('{}.{}'.format(camShape, CGRIG_APERTURE_BOOL))
...
def setSensorSizeInches(camShape, width, height, setCgRigGate=True):
    """Sets the sensor size (film gate) aperture width and height settings,

    If setCgRigGate True:

        Checks for the cgrig custom attribute CGRIG_APERTURE_BOOL if on then sets with
    ...
    :type height: float
    :param setCgRigGate: If True will set with the Match Gate CgRig custom attributes
    :type setCgRigGate: bool
    """
    if setCgRigGate:
        if not cmds.attributeQuery(CGRIG_APERTURE_BOOL, node=camShape, exists=True):
            createCgRigApertureAttrs(camShape)
        cmds.setAttr("{}.{}".format(camShape, CGRIG_APERTURE_INCH_WIDTH_ATTR), width)
        cmds.setAttr("{}.{}".format(camShape, CGRIG_APERTURE_INCH_HEIGHT_ATTR), height)
        if cmds.getAttr('{}.{}'.format(camShape, CGRIG_APERTURE_BOOL)):  # if boolean is True
...
def setSensorSizeMm(camShape, width, height, setCgRigGate=True):
    """Sets the sensor size (film gate) aperture width and height settings,
    ...
    :type setCgRigGate: bool
    """
    setSensorSizeInches(camShape, mmToInches(width), mmToInches(height), setCgRigGate=setCgRigGate)
...
def sensorSizeInches(camShape, getCgRigGate=True):
    """Returns the camera shape setting "Camera Aperture (mm)" height and width

    If getCgRigGate:

        Use cgrigTools custom attributes to query the camera's sensor size (film back), and does not use Maya's
        Also creates the custom attributes if they don't exist

    :param camShape: The name of the camera shape node
    :type camShape: str
    :param getCgRigGate: If the CgRig Tools custom attributes exist will use those values, if not will create/set attrs
    :type getCgRigGate: str
    ...
    if getCgRigGate:
        if not cmds.attributeQuery(CGRIG_APERTURE_BOOL, node=camShape, exists=True):
            createCgRigApertureAttrs(camShape)
        if cmds.getAttr('{}.{}'.format(camShape, CGRIG_APERTURE_BOOL)):  # if boolean is True
            inchesWidth = cmds.getAttr("{}.{}".format(camShape, CGRIG_APERTURE_INCH_WIDTH_ATTR))
            inchesHeight = cmds.getAttr("{}.{}".format(camShape, CGRIG_APERTURE_INCH_HEIGHT_ATTR))
        else:  # If boolean is False, go with the Maya settings
            inchesWidth = cmds.getAttr("{}.horizontalFilmAperture".format(camShape))
            inchesHeight = cmds.getAttr("{}.verticalFilmAperture".format(camShape))
    else:  # No CgRig check go with defaults
        inchesWidth = cmds.getAttr("{}.horizontalFilmAperture".format(camShape))
        inchesHeight = cmds.getAttr("{}.verticalFilmAperture".format(camShape))
...
def sensorSizeMm(camShape, getCgRigGate=True):
    """See documentation for sensorSizeInches() only returns in mm instead of inches
    """
    inchesWidth, inchesHeight, ratio = sensorSizeInches(camShape, getCgRigGate=True)
...
def resolutionSizeInches(camShape, getCgRigGate=True):
    ...
    resolutionWidth, resolutionHeight, resRatio = renderGlobalsWidthHeight()
    sensorWidth, sensorHeight, sensorRatio = sensorSizeInches(camShape, getCgRigGate=False)
...
def resolutionSizeMm(camShape, getCgRigGate=True):
    """See documentation for resolutionSizeInches() only returns in mm instead of inches
    """
    widthInches, heightInches = resolutionSizeInches(camShape, getCgRigGate=True)
...
def setGlobalsWidthHeight(width, height, matchCurrentCam=False, fitResGate=CAM_FIT_RES_OVERSCAN,
    ...
    if updateAllMatchedCams:
        ...
        for camShape in allCamShapes:
            if cgrigMatchGateBoolValue(camShape):
                matchResolutionFilmGate(camShape)
    if matchCurrentCam:  # Set the focus camera's film gate to match
        ...
        if not camera:
            return
        camShape = cameraShape(camera)
        setCgRigResolutionGate(camShape,
                             resolutionGate=True,
                             overscan=1.05,
