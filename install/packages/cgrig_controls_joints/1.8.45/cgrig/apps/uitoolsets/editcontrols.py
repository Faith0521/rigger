...
from cgrig.apps.controlsjoints.mixins import ControlsJointsMixin
from cgrigvendor.Qt import QtWidgets, QtCore

from cgrig.libs.cgrigscene import cgrigscenefiles

from cgrig.preferences.interfaces import coreinterfaces, controljointsinterfaces

import maya.cmds as cmds
from cgrig.libs.maya.cmds.objutils import shapenodes, objcolor, filtertypes, matching, namehandling, curves

from cgrig.libs.maya.cmds.rig import controls
from cgrig.libs.toolsets.controls import controlsdirectories
...
from cgrig.libs import iconlib, shapelib
...
class EditControls(toolsetwidget.ToolsetWidget, ControlsJointsMixin):
    ...
    def postContentSetup(self):
        ...
        if DEFAULT_SHAPE_STR not in self.controlShapeList:  # "circle" can't be found in the list
            if not self.controlShapeList:  # probably there is an issue with the shapes library, if empty list.
                output.displayWarning("No Control Shapes found in the CgRig Shapes Library.  "
    ...
    def deleteCurvesPopupWindow(self, shapeName):
        ...
        :rtype okState: bool
        """
        message = "Are you sure you want to delete `{0}` from the CgRig Curve Library?\n" \
    ...
    def renameCurvesPopupWindow(self, existingControlName):
        ...
        :rtype newControlName: str
        """
        message = "Rename `{}` in the CgRig library to a new name.\n" \
    ...
    @toolsetwidget.ToolsetWidget.undoDecorator
    def delCgRigTrackerAttrs(self):
        """Deletes/Cleans the cgrig tracker attribtutes on the selected control curves. """
    ...
    def saveControlsToLibrary(self):
        """Saves the selected control to disk, currently CgRig internally and not to cgrig_preferences"""
        curveTransformList = filtertypes.filterByNiceTypeKeepOrder(filtertypes.CURVE,
                                                                   searchHierarchy=False,
                                                                   selectionOnly=True,
                                                                   dag=False,
    ...
    def deleteShapeDesignFromDisk(self):
        """Deletes specified shape/design from disk, currently CgRig internally and not to cgrig_preferences"""
        designName = self.controlShapeList[self.properties.shapeCombo.value]
        ...
        filesFullPathDeleted = cgrigscenefiles.deleteCgRigSceneFiles(self.fullPath, message=True)
    ...
    def renameShapeDesignOnDisk(self):
        ...
        if not renameText:
            return
        fileRenameList = cgrigscenefiles.renameCgRigSceneOnDisk(renameText, self.fullPath,
    ...
    def uiConnections(self):
        ...
        self.advancedWidget.delCgRigTrackerBtn.clicked.connect(self.delCgRigTrackerAttrs)
...
class GuiWidgets(QtWidgets.QWidget):
    def __init__(self, parent=None, properties=None, uiMode=None, toolsetWidget=None):
        ...
        # Del CgRig Tracker Attrs ------------------------------------
        toolTip = "Deletes/cleans tracker attributes that CgRig Tools creates while using control creator."
        self.delCgRigTrackerBtn = elements.styledButton(text="Delete CgRig Tracker Attrs",
                                                      icon="xCircleMark2",
                                                      toolTip=toolTip,
                                                      style=uic.BTN_LABEL_SML)
        # Mirror Text Search ------------------------------------
...
class AdvancedLayout(GuiWidgets):
    def __init__(self, parent=None, properties=None, uiMode=UI_MODE_ADVANCED, toolsetWidget=None):
        ...
        gridLayout.addWidget(self.delCgRigTrackerBtn, 10, 1)
