...
class SplineRig(toolsetwidget.ToolsetWidget):
    ...
    def buildRig(self):
        ...
        with scaleutils.sceneUnitsContext("cm"):
            self._metaNode = executor.execute("cgrig.maya.splinerig.build", metaAttrs=metaAttrs,
    ...
    def bakeRig(self):
        ...
        with scaleutils.sceneUnitsContext("cm"):
            for m in metaNodes:
                executor.execute("cgrig.maya.splinerig.bake", meta=m)
    ...
    def rebuildRig(self, event=None, message=False):
        ...
        for m in metaNodes:
            executor.execute("cgrig.maya.splinerig.rebuild", meta=m, metaAttrs=metaAttrs, bake=bake)
        self.blockCallbacks(False)
    ...
    def deleteRig(self):
        ...
        for m in metaNodes:
            executor.execute("cgrig.maya.splinerig.delete", meta=m)
    ...
    def updateRig(self):
        ...
        with scaleutils.sceneUnitsContext("cm"):
            for m in metaNodes:
                executor.execute("cgrig.maya.splinerig.edit", meta=m, metaAttrs=metaAttrs)
    ...
    def publishAction(self):
        ...
        :return:
        :rtype: cgrig.libs.pyqt.extended.searchablemenu.action.TaggedAction
        """
    ...
    def splineControlSwitch(self, event=None):
        ...
        with scaleutils.sceneUnitsContext("cm"):
            for m in metaNodes:
                switched = executor.execute("cgrig.maya.splinerig.switch", meta=m,
    ...
    def splineRenamed(self, event):
        ...
        :param event:
        :type event:  cgrig.libs.pyqt.extended.combobox.comboeditwidget.EditChangedEvent
        :return:
        :rtype:
        """
        ...
        for m in allMeta:
            executor.execute("cgrig.maya.splinerig.rename", meta=m, name=text)
    ...
    def dotsMenuActionTriggered(self, action):
        ...
        :param action:
        :type action: cgrig.libs.pyqt.extended.searchablemenu.action.TaggedAction
        :return:
        :rtype:
        """
        ...
        if action.text() == DotsItems.SelectJoints:
            ...
            self.blockCallbacks(True)
            executor.execute("cgrig.maya.splinerig.duplicate", meta=meta)
            self.blockCallbacks(False)

        elif action.text() == DotsItems.RebuildBaked:
            metaNodes = self.currentMetaNodes()
            for m in metaNodes:
                executor.execute("cgrig.maya.splinerig.rebuild", meta=m, bake=True)

        elif action.text() == DotsItems.DeleteAll:
            executor.execute("cgrig.maya.splinerig.delete", meta=meta, deleteAll=True)
    ...
    def rebuildAdditiveFk(self, data=None):
        ...
        if splineMeta:  # Spline Rig exists, and checkbox is on so rebuild
            ...
            addFkRig = executor.execute("cgrig.maya.splinerig.buildAdditiveFk",
                                        meta=splineMeta,
            ...
            executor.execute("cgrig.maya.additiveFk.delete",
                             meta=addFkRig)
            addFkRig = executor.execute("cgrig.maya.additiveFk.build",
    ...
    def additiveFkStateChanged(self, checked):
        ...
        if checked:  # Build Add FK
            ...
            if splineMeta:
                ...
                executor.execute("cgrig.maya.additiveFk.delete", meta=addFkRig)
            else:
                return
            # Force UI Update
    ...
    def selectedAdditiveFkRig(self):
        ...
        :rtype addFkNode: cgrig.libs.maya.cmds.meta.metaadditivefk.CgRigMetaAdditiveFk
        """
        ...
        addFkNode = metaadditivefk.CgRigMetaAdditiveFk.connectedMetaNodes(selected)
        if addFkNode:
            return addFkNode

    def meshMarkingMenuChecked(self, state):
        for m in self.currentMetaNodes():
            executor.execute("cgrig.maya.splinerig.meshMarkingMenuActive", meta=m, active=state)
