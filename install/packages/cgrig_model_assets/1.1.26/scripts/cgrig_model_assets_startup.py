...
logger = logging.getLogger("CgRig Model Assets Startup")
...
def updatePrefsKeys(assetConstants, compareDictionariesFunc, preference):
    ...
    modelAssetsDefaultData = preference.defaultPreferenceSettings("cgrig_model_assets", "maya/cgrig_model_assets")
...
def updateMayaPrefsKeys(assetConstants, compareDictionariesFunc, preference):
    ...
    modelAssetsDefaultData = preference.defaultPreferenceSettings("cgrig_model_assets", "maya/cgrig_maya_scenes")
    # Force upgrade if needed
...
def checkUpgradePrefs(localData, defaultData, assetConstants):
    """Will upgrade the cgrig_model_assets.prefs file by overwriting if the "settings" key does not exist

    Will only be needed if upgrading from in CgRig 2.2.3 or lower

    :param localData: The preference object for local model_assets data in cgrig_preferences
    :type localData:
    :return camToolsLocalDict: The settings dictionary locally in cgrig_preferences, now potentially updated
