...
if env.isMaya():
    ...
    from cgrig.libs.maya.cmds.assets import assetsimportexport
    from cgrig.libs.maya.cmds.objutils import namehandling
    from cgrig.libs.maya.cmds.renderer import rendererload, exportabcshaderlights
...
from cgrig.apps.toolsetsui.mixins import MiniBrowserMixin
from cgrig.apps.toolsetsui.ext.renderers import RendererMixin
from cgrig.libs.cgrigscene import cgrigscenefiles
from cgrig.libs.pyqt.extended.imageview.models import cgrigscenemodel
from cgrig.libs.pyqt import keyboardmouse
...
from cgrig.preferences.interfaces import assetinterfaces, coreinterfaces
...
class AlembicAssets(toolsetwidget.ToolsetWidget, RendererMixin, MiniBrowserMixin):
    ...
    def exportGenericAlembic(self):
        """Exports all geo, cams and lights in the scene/selected as generic type .cgrigScene:

            - Objects.geo and cameras are saved as alembic
            - Lights saved as generic .cgrigLights
            - Shaders saved as generic .cgrigShaders
        ...
        fullFilePath = os.path.join(modelAssetsDirectory,
                                    os.path.extsep.join((name, exportabcshaderlights.CGRIGSCENESUFFIX))
                                    )
        # Save
    ...
    def renameAsset(self):
        ...
        cgrigScenePath = miniBrowser.itemFilePath()
        # Do the rename
        fileRenameList = cgrigscenefiles.renameCgRigSceneOnDisk(renameText, cgrigScenePath)
    ...
    def deletePresetPopup(self):
        ...
        filenameWithSuffix = "{}.{}".format(fileNameNoSuffix, exportabcshaderlights.CGRIGSCENESUFFIX)
        cgrigScenePath = os.path.join(os.path.dirname(self.currentWidget().miniBrowser.itemFilePath()),
                                    filenameWithSuffix)
        # Build popup window
        ...
        if result:  # Ok was pressed
            filesFullPathDeleted = cgrigscenefiles.deleteCgRigSceneFiles(cgrigScenePath, message=True)
            self.refreshThumbs()
    ...
    def selectPackageRootGrpsInScene(self):
        ...
        assetsimportexport.selectCgRigAssetGrps(uiSelectedName=uiSelectedName)
    ...
    def deleteAssetInScene(self):
        ...
        * needs undo support
        ...
        assetsimportexport.deleteCgRigAssetSelected(uiSelectedName=uiSelectedName)
    ...
    def importAsset(self, assetName=""):
        """ Imports the cgrigScene asset given the GUI settings
        ...
        if replaceIndex == 0:
            ...
            replaceByType = False
        # Do the import
        allNodes = assetsimportexport.importCgRigSceneAsAsset(item.filePath,
                                                            rendererNiceName,
...
class AllWidgets(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None, properties=None, uiMode=None, toolsetWidget=None):
        ...
        self.browserModel = cgrigscenemodel.CgRigSceneModel(self.miniBrowser,
