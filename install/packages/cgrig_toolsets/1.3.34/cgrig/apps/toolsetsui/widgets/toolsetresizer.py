...
class ToolsetResizer(QtWidgets.QFrame):
    def __init__(
        ...
        mainLayout.setContentsMargins(
            utils.dpiScale(margins[0]),
            utils.dpiScale(margins[1]),
            utils.dpiScale(margins[2]),
            utils.dpiScale(margins[3]),
        )
    ...
    def mousePressEvent(self, event):
        self._activeMove = True
        event.accept()
        super(ToolsetResizer, self).mousePressEvent(event)
    ...
    def mouseMoveEvent(self, event):
        ...
        :rtype:
        """
        if not self._activeMove:
            super(ToolsetResizer, self).mouseMoveEvent(event)
            return
    ...
    def mouseReleaseEvent(self, event):
        ...
        self._activeMove = False
        event.accept()
        super(ToolsetResizer, self).mouseReleaseEvent(event)
