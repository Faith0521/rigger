...
if env.isMaya():
    from cgrig.apps.toolsetsui.widgets import toolsetwidgetmaya
...
class ToolsetRegistry(QtCore.QObject):
    ...
    .. code-block:: python
        ...
        os.environ["CGRIG_TOOLSET_UI_PATHS"] = "../examples/widgets"
        os.environ["CGRIG_TOOLSET_LAYOUTS"] = "../examples/cgrigtoolsets"
        reg = ToolsetRegistry()
        self.logger.debug("{}".format(reg.toolsetDefs))
        {'ConvertAllShadersRenderer': <class cgrig.apps.generictoolsetui.cgrigtoolsets.convertallshaders_renderer.ConvertAllShadersRendererToolset>,
        'importExportShaders': <class cgrig.apps.generictoolsetui.cgrigtoolsets.importexportshaders.ImportExportShadersToolset}

    """
    registryEnv = "CGRIG_TOOLSET_UI_PATHS"
    toolsetJsons = "CGRIG_TOOLSET_LAYOUTS"
    ...
    def discoverToolsets(self):
        """Searches the component library specified by the environment variable "CGRIG_TOOLSET_UI_PATHS"
        """
        self.readJsons()
        self._toolsetDefs = {}
    ...
    def toolset(self, toolsetID):
        ...
        if ret is None:
            logger.warning(" \"{}\" toolset not found. Make sure the toolset ID name is correct in the toolsetgroups, "
                           "or the toolset folder has been added to 'CGRIG_TOOLSET_UI_PATHS' in 'cgrig_package.json'.".format(
                toolsetID))
...
def toolset(toolsetID):
    ...
    .. code-block:: python
        ...
        toolsetCls = registry.toolset("cgrigVertSkinning")

        cgrigVertSkinning = toolsetCls()
