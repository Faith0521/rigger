# -*- coding: utf-8 -*-
from cgrigvendor.six.moves import cPickle
import time

from cgrigvendor import six
from cgrigvendor.Qt import QtCore, QtWidgets

from cgrig.apps.toolsetsui.widgets import toolsettree, toolsetwidgetitem
...
from cgrig.libs.pyqt.widgets.frameless.widgets import TitleBar
from cgrig.core.util import zlogging
from cgrig.preferences.interfaces import coreinterfaces
from cgrig.apps.toolsetsui import registry
...
class ToolsetsUI(elements.CgRigWindow):
    windowSettingsPath = "cgrig/toolsetsui"
    ...
    def maxWindowHeight(self):
        ...
        sceneGeometry = utils.screenFromWidget(self).geometry()

        relativePos = pos - QtCore.QPoint(sceneGeometry.left(), sceneGeometry.top())

        return sceneGeometry.height() - relativePos.y() - WINDOWHEIGHT_PADDING
    ...
    def resizeEvent(self, event):
        super(ToolsetsUI, self).resizeEvent(event)
        self.resizedHeight = event.size().height()
...
class ToolsetTitleBar(TitleBar):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.toolsetUi = self.cgrigWindow  # type: ToolsetsUI

    def setFramelessEnabled(self, action=None):
        """ Get the settings and transfer to new CgRigToolset
...
def toolsetsByAttr(attr, visibleOnly=False):
    ...
    :type attr:
    :return:
    :rtype: list[:class:`cgrig.apps.toolsetsui.widgets.toolsetwidget.ToolsetWidgetBase`]
    """
