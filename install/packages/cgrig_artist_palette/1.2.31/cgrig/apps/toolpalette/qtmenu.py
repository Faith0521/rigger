...
from cgrig.libs.pyqt.extended import searchablemenu
from cgrig.core.util import zlogging
from cgrig.apps.toolpalette import utils

from cgrig.apps.toolpalette import layouts


logger = zlogging.getLogger(__name__)


def generateMenuTree(
    parentItem, qtParentMenu, triggeredFunc, typeRegistry, iconSize, createdItems_=None
):
    ...
    :type typeRegistry: :class:`cgrig.core.type.TypeRegistry`
    ...
    for childItem in parentItem.children:
        ...
        if childItem.isMenu():
            ...
            nativeItem = createMenuAction(
                childItem, qtParentMenu, triggeredFunc, iconSize
            )
            createdItems_[childItem.id] = nativeItem
            continue
        elif childItem.isVariant():
            childId = childItem.parent.id
            childPluginType = "action"
        ...
        if typePlugin is None:
            ...
            logger.warning(
                "Failed to find Plugin: {}, type: {}".format(childId, childPluginType)
            )
            return
...
def createMenuAction(menuItem, nativeParentMenu, triggeredFunc, iconSize):
    ...
    menuItem.arguments["item"] = menuItem
    ...
    if isCheckable:
        ...
        taggedAction.toggled.connect(
            lambda: triggeredFunc(pluginId, state=taggedAction.isChecked())
        )
    else:
        taggedAction.triggered.connect(
            partial(triggeredFunc, pluginId, **menuItem.arguments)
        )
