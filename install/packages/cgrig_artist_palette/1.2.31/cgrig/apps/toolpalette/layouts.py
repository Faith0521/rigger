...
class Item(HierarchyItem):
    ...
    def __init__(self, data, parent=None):
        ...
        self.type = data.get("type", "action")
        ...
        self.tags = []
        self.metaData = {}
        self.widgetInfo = data.get("widgetInfo", {})
    ...
    def update(self, data):
        ...
        self.widgetInfo = data.get("widgetInfo", self.widgetInfo)
...
class Shelf(HierarchyItem):
    ...
    def createFromDict(cls, data):
        ...
        newShelf.showInHostShelf = data.get("showInHostShelf", True)
    ...
    def loadFromEnv(self, envVar):
        ...
        for sortedLayout in sorted(layouts, key=lambda x: x.get("sortOrder", 0)):
            for shelf in sortedLayout.get("shelves", []):
                ...
                shelf["showInHostShelf"] = sortedLayout.get("showInHostShelf", True)
...
class MenuManager(object):
    """Management class for all CgRig tools Main Menus.
    """
    PRIMARY_MENU_TOKEN = "{cgrigToolsPro}"
    ...
    def loadFromEnv(self, envVar):
        ...
        topLevelCgRigMenu = self.menuById(self.menuObjectName)

        devMenu = topLevelCgRigMenu.childById("Developer")
        devMenu.updateChildren([{"type": "action", "id": "cgrig.logging"}])

        topLevelCgRigMenu.updateChildren([{"type": MenuItem.kSeparatorType},
                                        {"type": MenuItem.kLabelType,
                                         "label": "CgRig Tools PRO - {}".format(api.currentConfig().buildVersion()),
                                         "id": "cgrigToolsVersion"}
                                        ]
                                       )
