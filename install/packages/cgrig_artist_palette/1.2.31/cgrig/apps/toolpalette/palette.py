# -*- coding: utf-8 -*-
"""This Module handles the base class for the cgrigtools main menu.
...
from cgrig.apps.toolpalette import layouts, qtmenu
...
class ToolPalette(object):
    TOOLSENV = "CGRIG_ACTIONPLUGIN_MODULES"
    LAYOUTENV = "CGRIG_MENU_LAYOUTS"
    TYPE_ENV = "CGRIG_TOOLPALETTE_PLUGINTYPE_PATH"
    SHELFLAYOUTENV = "CGRIG_SHELF_LAYOUTS"
    ...
    def __init__(self, parent=None):
        ...
        self.typeRegistry.registerPlugin(SliderType)
        for p in self.typeRegistry.plugins.keys():
            self.typeRegistry.loadPlugin(p, toolPalette=self)
        self._pluginIdMap = {}  # pluginId, pluginType ie. action
        ...
        # only valid while build the menu, this temporarily stores the native host menu instance by the cgrig id
        self._menuCache = {}
        self._shelfCache = {}

    def removePreviousMenus(self):
        """Removes Any cgrig plugin menu from maya by iterating through the children of the main window looking for any
        widget with the objectname CgRig Tools.
        """
    ...
    def removeShelves(self, clearLayoutOnly=False):
        """Removes any cgrig shelves from the host application. If clearLayoutOnly is True then each shelf will be
...
class ActionType(PluginTypeBase):
    id = "action"
    TOOLSENV = "CGRIG_ACTIONPLUGIN_MODULES"
    commandString = """
from cgrig.apps.toolpalette import run as _paletterun
_cgrigPalette = _paletterun.currentInstance()
_cgrigPalette.executePluginById(**{})
"""

    def __init__(self, toolPalette):
        super(ActionType, self).__init__(toolPalette)
        self._typeRegistry = pluginmanager.PluginManager(interface=[ActionPlugin],
                                                         variableName="id",
                                                         name="toolPaletteActionType")
    ...
    def pluginUiData(self, pluginId, overrides=None):
        ...
        try:
            ...
            logger.error("Missing Action plugin: {}".format(pluginId), exc_info=True)
        finally:
            return uiData
...
class SliderType(PluginTypeBase):
    id = "slider"
    TOOLSENV = "CGRIG_ACTIONPLUGIN_MODULES"
    commandString = """
from cgrig.apps.toolpalette import run as _paletterun
...
"""
    ...
    def __init__(self, toolPalette):
        super(SliderType, self).__init__(toolPalette)
        self._typeRegistry = pluginmanager.PluginManager(interface=[SliderActionPlugin],
                                                         variableName="id",
                                                         name="toolPaletteActionType")
        self._typeRegistry.registerByEnv(self.TOOLSENV)
        self._typeRegistry.loadAllPlugins(toolPalette=toolPalette)
...
class ActionPlugin(plugin.Plugin):
    tags= []
    uiData = {"icon": "",
              "tooltip": "",
              "label": "",
    ...
    def __init__(self, manager=None, toolPalette=None):
        super(ActionPlugin, self).__init__(manager=manager)
        self.toolPalette = toolPalette
    ...
    def _execute(self, *args, **kwargs):
        """ Modified version from ActionPlugin to allow multiple tools.
...
class SliderActionPlugin(ActionPlugin):

    def __init__(self, manager=None, toolPalette=None):
        super(SliderActionPlugin, self).__init__(manager=manager, toolPalette=toolPalette)
