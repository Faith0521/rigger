...
def currentInstance():
    ...
    :return:
    :rtype: :class:`cgrig.apps.toolpalette.palette.ToolPalette` or None
    """
    global PALETTE_UI
    return PALETTE_UI
...
def load(parent=None, applicationName=None):
    ...
    if _REGISTRY_INSTANCE is None:
        _REGISTRY_INSTANCE = pluginmanager.PluginManager(interface=[palette.ToolPalette],
                                                         ...
                                                         name="ArtistPalette")
        _REGISTRY_INSTANCE.registerByEnv("CGRIG_ARTIST_PALETTE_PATH")
...
def close(deleteMenu=True, deleteShelves=True):
    ...
    try:
        ...
        # happens when cgrigtoolsUi global is none
        raise
    finally:
