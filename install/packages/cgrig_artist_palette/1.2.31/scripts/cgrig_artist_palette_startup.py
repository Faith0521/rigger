...
def updatePrefsKeys():
    ...
    Will only be needed if upgrading from CgRig 2.2.5 or lower"""
    artistPrefsLocalData = preference.findSetting("prefs/maya/artistpalette.pref", None)  # artistpalette .prefs info
    artistPrefsDefaultData = preference.defaultPreferenceSettings("cgrig_artist_palette", "maya/artistpalette")
...
def checkUpgradePrefs(localData, defaultData):
    ...
    :param localData: The preference object for local camera_tools data in cgrig_preferences
    :type localData:
    :return controlsLocalDict: The settings dictionary locally in cgrig_preferences, now potentially updated
