...
from cgrig.apps.hiveartistui.views import validationview
...
from cgrig.libs.maya.utils import fbx as fbxutils
...
from cgrig.libs.utils import general as libgeneral
...
PRESET_DICT['Unity 2021'] = {FBX_WORLD_UP_KEY: "Y",
                             FBX_VERSION_KEY: "FBX 2020"}
PRESET_DICT['Unity 2022'] = {FBX_WORLD_UP_KEY: "Y",
                             FBX_VERSION_KEY: "FBX 2020"}
...
SAVE_SCENE_MSG = "WARNING: This scene will be destroyed in the process of FBX exporting. \n\n" \
                 "CgRig Tools will close and reload this Maya scene file after the FBX export completes.\n\n" \
                 "Save the current file to avoid losing changes?"


class HiveRigFbxExportToolset(toolsetwidget.ToolsetWidget):
    id = "hiveFbxRigExport"
    ...
    def postContentSetup(self):
        ...
        self.validationView = validationview.ValidationView(parent=self)
        self.validationView.hide()
        self.uiFolderPath = ""
    ...
    def initializeProperties(self):
        ...
        return [
            ...
            {"name": "tangentsCheckBox", "value": 1},
            {"name": "smoothingGroups", "value": 1}
        ]
    ...
    def widgets(self):
        ...
        """
        return super(HiveRigFbxExportToolset, self).widgets()
    ...
    def uiSettingsDict(self):
        ...
        uiSettingsDict["smoothingGroups"] = self.smoothingGroups
        uiSettingsDict["tangentsCheckBox"] = self.tangentsCheckBox
    ...
    def setUiSettings(self, uiSettingsDict):
        ...
        self.properties.tangentsCheckBox.value = uiSettingsDict["tangentsCheckBox"]
        self.properties.smoothingGroups.value = uiSettingsDict["smoothingGroups"]
        ...
        self.compactWidget.tangentsCheckBox.setEnabled(shapeState)
        self.compactWidget.smoothingGroups.setEnabled(shapeState)
    ...
    def saveUiSettingsDisk(self):
        ...
        filePath, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save the UI Settings File",
                                                            self.defaultBrowserPath, "*.cgrigFbxUi")
    ...
    def updateUISettings(self):
        ...
        self.tangentsCheckBox = self.properties.tangentsCheckBox.value and self.meshes
        self.smoothingGroups = self.properties.smoothingGroups.value and self.meshes
    ...
    def applyPreset(self):
        ...
        self.properties.filePathEdit.value = self.outputFilePath
        ...
        # filePathEdit should already, but isn't so update hack fix.
        self.compactWidget.filePathEdit.setPathText(self.properties.filePathEdit.value)
    ...
    def exportSingleFile(self):
        ...
        settings.tangents = self.tangentsCheckBox
        validationInfo = exporter.validate(self.rigInstance, settings)
        self.validationView.reload(validationInfo)
        if validationInfo.status != 0:
            for widget in self.widgets():
                widget.validationInfoBtn.setValidationState(validationInfo.status)
                widget.validationInfoBtn.show()
    ...
    def _onValidationInfoBtn(self):
        if not self.validationView.shouldDisplay:
            return
        cursorPosition = QtGui.QCursor.pos()

        self.validationView.move(cursorPosition)

        self.validationView.show()

    def _onValidationInfoRecheck(self):
        exporter = hiveapi.Configuration().exportPluginForId("fbxExport")()
        exporter.onProgressCallbackFunc = self.progressCallback
        settings = exporter.exportSettings()  # type: fbxexporter.ExportSettings
        # Export settings ----------------------
        ...
        for widget in self.widgets():
            widget.validationInfoBtn.setValidationState(validationInfo.status)
            if validationInfo.status == hiveapi.VALIDATION_SUCCESS:
                widget.validationInfoBtn.hide()
    ...
    def batchExportFrameRanges(self):
        ...
        fbxBatch.tangents = self.tangentsCheckBox
        fbxBatch.smoothingGroups = self.smoothingGroups
        # Export ------------------------------------
    ...
    def uiConnections(self):
        ...
        for widget in self.widgets():
            ...
            widget.validationInfoBtn.showUISignal.connect(self._onValidationInfoBtn)
            widget.validationInfoBtn.recheckSignal.connect(self._onValidationInfoRecheck)
...
class GuiWidgets(QtWidgets.QWidget):
    def __init__(self, parent=None, properties=None, uiMode=None, toolsetWidget=None):
        ...
        toolTip = "Specify optional name suffix. \n" \
                  " - eg. shot05\n" \
                  " - eg. cgrig_mannequin_shot05.fbx, robot_shot05.fbx \n"
        self.suffixNameStr = elements.StringEdit(label="Name Suffix",
                                                 parent=parent,
                                                 editPlaceholder="shot05 (optional, eg. cgrig_mannequin_shot05.fbx)",
                                                 toolTip=toolTip)
        # Prefix Rig Name Checkbox -----------------------------
        toolTip = "Check to prefix the rig name to the file name. \n" \
                  "On: \n" \
                  " - cgrig_mannequin_walk.fbx, cgrig_mannequin_run.fbx \n" \
                  "Off: \n" \
                  " - walk.fbx, run.fbx"
        ...
        toolTip = "Override frame range suffix naming. \n" \
                  " - eg. walk, run, jump \n\n" \
                  "Not Used: \n" \
                  " - eg. cgrig_mannequin_1_100.fbx, cgrig_mannequin_101-200.fbx, natalie \n" \
                  "Overridden: walk, run: \n" \
                  " - eg. cgrig_mannequin_walk.fbx, cgrig_mannequin_run.fbx, natalie\n\n" \
        ...
        self.skinningCheckBox = elements.CheckBox("Skinning", enableMenu=False, toolTip=toolTip,
                                                  ...
                                                  parent=parent)
        toolTip = "Exports Geometry with tangents and binormals."
        self.tangentsCheckBox = elements.CheckBox("Tangents And BiNormals", enableMenu=False, toolTip=toolTip,
                                                  checked=self.properties.tangentsCheckBox.value,
                                                  parent=parent)
        toolTip = "Exports Geometry with smoothing groups"
        self.smoothingGroups = elements.CheckBox("Smoothing Groups", enableMenu=False, toolTip=toolTip,
                                                 checked=self.properties.smoothingGroups.value,
                                                 parent=parent)
        # Presets Combo -----------------------------
        ...
        tooltip = "Save the UI settings to either the `scene` or to `disk`. \n" \
                  " - Scene: Saves the UI settings to a node named `cgrigHiveExportFbxUiSettings`in the scene. \n" \
                  " - Disk: Saves the UI settings to a file *.cgrigFbxUi on disk. "
        ...
        tooltip = "Loads previously saved `Export FBX` settings into the UI. \n" \
                  "Loads a *.cgrigFbxUi file."
        self.uiLoadBtn = elements.styledButton("",
                                               parent=parent,
                                               icon="openFolder01",
                                               toolTip=tooltip,
                                               style=uic.BTN_DEFAULT)
        self.validationInfoBtn = validationview.ValidationButton(parent=parent)
        self.validationInfoBtn.hide()
    ...
    def onMeshCheckChanged(self, state):
        ...
        self.tangentsCheckBox.setEnabled(state)
        self.smoothingGroups.setEnabled(state)
...
class GuiCompact(GuiWidgets):
    def __init__(self, parent=None, properties=None, uiMode=UI_MODE_COMPACT, toolsetWidget=None):
        ...
        geometryLayout3 = elements.hBoxLayout(spacing=uic.SLRG, margins=(uic.REGPAD, 0, uic.REGPAD, 0))
        geometryLayout3.addWidget(self.tangentsCheckBox, 1)
        geometryLayout3.addWidget(self.smoothingGroups, 1)
        # Time Range Layout -----------------------
        ...
        mainLayout.addLayout(geometryLayout3)
        ...
        mainLayout.addWidget(self.validationInfoBtn)
