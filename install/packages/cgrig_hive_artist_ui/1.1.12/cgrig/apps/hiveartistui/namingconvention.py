...
from cgrig.apps.hiveartistui.hivenaming import fieldwidget, presetwidget, rulewidget
...
from cgrig.libs.pyqt.models import constants as modelconstants
...
class NamingConventionUI(elements.CgRigWindow):
    helpUrl = "https://create3dcharacters.com/maya-tool-hive-naming-convention/"
    windowSettingsPath = "cgrig/hivenamingconventionui"
    ...
    def _initUi(self):
        ...
        self.conventionCombobox, self.conventionClearBtn, self.conventionLayout = (
            self._createConventionLayout()
        )
        self.rulesCombobox, self.ruleClearBtn, self.ruleLayout = (
            self._createRulesLayout()
        )

        self.rulePreviewLabel = elements.Label(text="Rule Preview", parent=self)
        self.ruleExpressionWidget = rulewidget.CompleterStringEdit(
            "Rule", labelRatio=8, editRatio=100, parent=self
        )
        self.ruleExpressionWidget.layout().setAlignment(
            self.ruleExpressionWidget.label, QtCore.Qt.AlignRight
        )
    ...
    def _createConventionLayout(self):
        layout = elements.hBoxLayout()
        conventionCombobox = elements.ComboBoxRegular(
            "Hive Type",
            items=[],
            boxRatio=100,
            labelRatio=8,
        ...
        conventionCombobox.layout().setAlignment(
            conventionCombobox.label, QtCore.Qt.AlignRight
        )
        clearBtn = elements.IconMenuButton(iconName="reload2", parent=self)
        clearBtn.setMaximumWidth(20)
        clearBtn.setToolTip("Clear the Convention Override(s).")
        toolTip = (
            "Clear the selected Convention Overrides.\n"
            "Includes All Rule and field overrides"
        )
        clearBtn.addAction(
            "Clear Current Overrides",
            connect=self._onClearSelectedConvention,
            toolTip=toolTip,
        )
        toolTip = (
            "Clear All Overrides on current Preset.\n"
            "Include all Hive Types for rules and fields."
        )
        clearBtn.addAction(
            "Clear All Overrides", connect=self._onClearAllConventions, toolTip=toolTip
        )
        layout.addWidget(conventionCombobox)
        layout.addWidget(clearBtn)

        return conventionCombobox, clearBtn, layout

    def _createRulesLayout(self):
        layout = elements.hBoxLayout()
        rulesCombobox = elements.ComboBoxRegular(
            label="Rule Type",
            labelRatio=8,
            boxRatio=100,
            items=[],
        ...
        rulesCombobox.layout().setAlignment(rulesCombobox.label, QtCore.Qt.AlignRight)
        clearBtn = elements.IconMenuButton(iconName="reload2", parent=self)
        clearBtn.setMaximumWidth(20)
        clearBtn.setToolTip("Clear the Rule Override(s).")
        toolTip = "Clear the selected Rule Override."
        clearBtn.addAction(
            "Clear Current Overrides",
            connect=self._onClearSelectedRule,
            toolTip=toolTip,
        )
        toolTip = "Clear All Overrides on current Hive Type."
        clearBtn.addAction(
            "Clear All Overrides", connect=self._onClearAllRules, toolTip=toolTip
        )
        layout.addWidget(rulesCombobox)
        layout.addWidget(clearBtn)
        return rulesCombobox, clearBtn, layout

    def _onClearSelectedConvention(self):
        # todo: add confirmation dialog
        uiState = self.uiState
        changeConfig = uiState.currentPreset.findNamingConfigForType(
            self.conventionCombobox.currentText(), recursive=False
        )
        if changeConfig is not None:
            uiState.currentConfig.clearFields()
            uiState.currentConfig.clearRules()
            self.uiState.requiresSave = True
            self._updateRuleTypes()
            self.updatePreviewRuleLabel()
            self.fieldsWidget.reloadFromConfig(uiState.currentConfig)

    def _onClearAllConventions(self):
        # todo: add confirmation dialog
        uiState = self.uiState
        uiState.presetsOps.clearPresetOps(uiState.currentPreset)
        for cfg in uiState.currentPreset.configs:
            uiState.presetsOps.createDeleteConfigOp(uiState.currentPreset, cfg.hiveType)
    ...
    def _onClearSelectedRule(self):
        ...
        if changeConfig is not None:
            ...
            rule = config.rule(currentRule, recursive=False)
            if rule is not None:
    ...
    def _setupLayout(self):
        mainLayout = elements.vBoxLayout(
        ...
        conventionLayout.addLayout(self.conventionLayout)
        conventionLayout.addLayout(self.ruleLayout)
    ...
    def _setupConnections(self):
        ...
        self.conventionCombobox.currentIndexChanged.connect(
            self._onConventionTypeChanged
        )
    ...
    def _initializePresetTree(self):
        ...
        if root is not None:
            self.presetView.loadPreset(
                root, includeRoot=self.uiState.isAdmin, parent=None
            )

    def enabledState(self, preset=True):
        if preset:
            self.conventionCombobox.setEnabled(True)
            self.conventionClearBtn.setEnabled(True)
            self.ruleClearBtn.setEnabled(True)
        else:
    ...
    def disableState(self):
        ...
        self.conventionClearBtn.setEnabled(False)
        self.ruleClearBtn.setEnabled(False)
    ...
    def _onPresetSelectionChanged(self, event):
        ...
        :param event:
        :type event: :class:`cgrig.libs.pyqt.extended.treeviewplus.TreeViewPlusSelectionChangedEvent`
        """
    ...
    def _onConventionTypeChanged(self, index):
        ...
        if not isLocal:
            ...
            config = self.uiState.currentPreset.createConfig(
                name=None, hiveType=requestedConvention, rules=rules, fields=fields
            ).config
            ...
            self.uiState.presetsOps.createConfigOp(
                self.uiState.currentPreset, requestedConvention
            )
            self.uiState.requiresSave = True
        else:
            self.uiState.presetsOps.createConfigUpdateOp(
                self.uiState.currentPreset, requestedConvention
            )
            self.uiState.requiresSave = True
    ...
    def _onCreatePreset(self):
        ...
        label = elements.InputDialogQt(
            windowName="Preset Name",
        ...
        if existingPreset is not None:
            elements.MessageBox.showWarning(
                title="Non-Unique Name",
                parent=self,
        ...
        newPreset = self.uiState.presetManager.createPreset(
            label, outputFolder, parent=parentPreset
        )
    ...
    def _onRemovePreset(self):
        ...
        for item in items:
            ...
            toDelete.append((label, preset, item.uid))

        for presetLabel, preset, itemId in toDelete:
            if self.uiState.presetManager.removePreset(presetLabel):
                self.uiState.presetsOps.createDeletePresetOp(preset)
                self.uiState.requiresSave = True
                indexList = model.match(model.index(0, 0), modelconstants.uidRole, itemId, 1)
                if not indexList:
                    logger.debug("Unable to find item with uid: {}, Preset: {}".format(itemId, presetLabel))
                    continue
                index = indexList[0]
    ...
    def _onRuleExpressionChanged(self, text):
        ...
        self.uiState.currentConfig.rule(currentRuleName).expression = "_".join(
            expression
        )
        self.updatePreviewRuleLabel()
    ...
    def close(self):
        if self.uiState.requiresSave:
            result = elements.MessageBox.showWarning(
                parent=self,
    ...
    def _updateRuleTypes(self):
        logger.debug(
            "Updating Rule Widget for config: {}".format(
                self.uiState.currentConfig.name
            )
        )
        globalRules = list(
            sorted(
                self.uiState.currentConfig.rules(recursive=True), key=lambda x: x.name
            )
        )
...
class PresetOperations(object):
    ...
    def clearPresetOps(self, preset):
    ...
    def createRenameOp(self, preset, oldName, newName):
        ...
        try:
            ...
            pass
        self.operations.setdefault(preset, {})["rename"] = {
            "oldName": oldName,
            "newName": newName,
            "preset": preset,
        }
    ...
    def createConfigUpdateOp(self, preset, hiveType):
        ...
        try:
            ...
            pass
        self.operations.setdefault(preset, {}).setdefault("updateConfig", []).append(
            hiveType
        )

    def createConfigOp(self, preset, hiveType):
        self.operations.setdefault(preset, {}).setdefault("createConfig", []).append(
            hiveType
        )

    def createDeleteConfigOp(self, preset, hiveType):
        self.operations.setdefault(preset, {}).setdefault("deleteConfig", []).append(
            hiveType
        )
...
def save(preferences, uiState):
    ...
    for preset, ops in uiState.presetsOps.operations.items():
        ...
        requiresPreferencesSave = (
            _handleSaveAddOps(ops, savePreset, preset, uiState)
            or requiresPreferencesSave
        )
        requiresPreferencesSave = (
            _handleSaveDeleteOps(ops, preset, uiState) or requiresPreferencesSave
        )
        requiresPreferencesSave = (
            _handleSaveRenameOps(ops, preset, uiState) or requiresPreferencesSave
        )
        requiresPreferencesSave = (
            _handleDeleteConfig(ops, preset, uiState) or requiresPreferencesSave
        )
...
def _handleSaveRenameOps(ops, preset, uiState):
    ...
    if renameOp:
        ...
        uiState.presetManager.savePreset(preset)
        uiState.presetManager.printHierarchy()
        return True
    return False
...
def _handleSaveDeleteOps(ops, preset, uiState):
    if "delete" in ops:
        ...
        while stack:
            p = stack.pop()
            uiState.presetManager.deletePreset(p, configs=True)
...
def _handleSaveConfigsOps(ops, preset, uiState):
    ...
    for hiveType in ops.get("createConfig", []):
        config = preset.findNamingConfigForType(hiveType, recursive=False)
    ...
    savePreset = (
        _handleSaveUpdateConfig(ops, preset, originalPreset, uiState) or savePreset
    )
    return savePreset
...
def _handleDeleteConfig(ops, preset, uiState):
    savePreset = False
    for hiveType in ops.get("deleteConfig", []):
        config = preset.findNamingConfigForType(hiveType, recursive=False)
        if config is not None:
            preset.deleteConfig(config)
            savePreset = True
    if savePreset:
        uiState.presetManager.savePreset(preset)
