...
from cgrig.apps.hiveartistui import artistuicore, uiinterface
from cgrig.apps.hiveartistui.registries import toolregistry
from cgrig.apps.hiveartistui.views import createview, verticaltoolbar
from cgrig.apps.hiveartistui.views import rigbox
from cgrig.apps.hiveartistui.views.loadingwidget import LoadingWidget
from cgrig.apps.hiveartistui.views.visibilitybutton import VisibilityButton
from cgrig.apps.toolsetsui import toolsetui
from cgrig.apps.uitoolsets import hiveartistsettings
...
from cgrig.libs.pyqt.widgets import resizerwidget
from cgrig.libs.utils import profiling
from cgrig.preferences.interfaces import coreinterfaces
from cgrig.preferences.interfaces import hiveartistuiinterfaces
...
class HiveArtistUI(elements.CgRigWindow):
    TOOLSET_GROUP = "hiveArtistUi"
    helpUrl = "https://create3dcharacters.com/maya-hive-autorigger"
    windowSettingsPath = "cgrig/hiveartistui"

    def __init__(self, title="Hive",
                 width=580,
    ...
    def initUi(self):
        ...
        self.validationBar = validationview.ValidationButton(parent=self)
        self.validationBar.hide()
    ...
    def initBottomBar(self):
        ...
        :return:
        """
        bottomBarLayout = elements.vBoxLayout(margins=(4, 4, 4, 4), spacing=0)
        ...
        guideToolTip = "Place the pivots for the rig, modify or add components"
        controlToolTip = "Scale and change the design shapes and colors of the rig's controls."
        skeletonToolTip = "Skin the rig, add correctives, check the skeleton."
        rigToolTip = "Build the rig."
        polishToolTip = "The rig will be built and animator right-click functionality is also added."
        self.rigModeButtons = elements.JoinedRadioButton(
            texts=["GUIDES", "CONTROLS", "SKELETON", "PRE RIG", "POLISH RIG"],
            toolTips=[guideToolTip, controlToolTip, skeletonToolTip,
                      rigToolTip, polishToolTip],
            parent=self)
        self.viewButton = VisibilityButton(parent=self)
        self.rebuildIcon = elements.styledButton(
            "",
            icon="hammer",
            parent=self,
            toolTip="Rebuilds the current Rig.",
            themeUpdates=False,
        ...
        self.rigModeLayout.addWidget(self.rebuildIcon)
        self.rigModeLayout.addWidget(self.viewButton)

        bottomBarLayout.addWidget(self.rigMode)
        bottomBarLayout.addWidget(self.validationBar)
    ...
    def connections(self):
        ...
        self.rebuildIcon.clicked.connect(self.onRebuildRig)
        ...
        self.validationBar.showUISignal.connect(self._onValidationClicked)
        self.validationBar.recheckSignal.connect(self._onValidationRecheckClick)
    ...
    def componentTreeView(self):
        ...
        :return:
        :rtype: :class:`cgrig.apps.hiveartistui.views.componenttree.ComponentTreeView`
        """
    ...
    def onRebuildRig(self):
        print("rebuilding rig")
        hiveapi.commands.rebuildRig(self.core.currentRigContainer.rigModel.rig)

    def rigModeToggled(self, buttonEvent):

        index = buttonEvent.index
        if index == 0:  # guides
            self.displayValidationResults(None)
            self.core.buildGuides()
        elif index == 1:
            self.core.toggleGuideControls()
        elif index == 2:
            if self.core.rigMode() == hiveapi.constants.GUIDES_STATE:
                if self.displayValidationResults(self.core.validateGuides()) > 0:
                    self.updateRigMode()
                    return
    ...
    def refreshUi(self):
        ...
        try:
            ...
            if rig is None:
                ...
                self.displayValidationResults(None)
                return
            if self.core.currentRigContainer.rigModel.rig != rig:
                self.displayValidationResults(None)
    ...
    def enable(self):
        ...
        self.createUi.templateLibraryWgt.enableTree(True)
        self.createUi.componentLibraryWgt.enableTree(True)
    ...
    def updateRigMode(self):
        ...
        if rigMode == hiveapi.constants.GUIDES_STATE:
            ...
            self.rebuildIcon.setEnabled(True)
            self.rigModeButtons.buttons()[-2].setEnabled(True)
            self.createUi.templateLibraryWgt.enableLoadTemplate(False)
            self.createUi.componentLibraryWgt.enableTree(False)
    ...
    def displayValidationResults(self, result):
        """

        :param result:
        :type result: :class:`hiveapi.ValidationRigInfo`

        """
        highestStatus = result.status if result is not None else 0
        if highestStatus == 0:
            self.validationView.reload(None)
        else:
            self.validationView.reload(result)
        self.validationBar.setValidationState(highestStatus)
        return highestStatus

    def _onValidationClicked(self):
    ...
    def _onValidationRecheckClick(self):
        self.displayValidationResults(self.core.validateGuides())
