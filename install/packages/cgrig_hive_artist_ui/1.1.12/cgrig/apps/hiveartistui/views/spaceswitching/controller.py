...
from cgrig.libs.pyqt.models import modelutils, constants
...
class HiveSpaceSwitchController(QtCore.QObject):
    ...
    .. note::
        ...
        See :mod:`cgrig.apps.hiveartistui.views.spaceswitching.model` for how the tablemodel handles the
    ...
    def bindView(self, view):
        ...
        :param view:
        :type view: :class:`cgrig.apps.hiveartistui.views.spaceswitching.view.SpaceSwitchWidget`
        """
        self.view = view
        ...
        container.spaceDriverView.tableView.copyRequested.connect(self._onCopyRequested)
        container.spaceDriverView.tableView.pasteRequested.connect(self._onPasteRequested)
    ...
    def spaceLabels(self):
        yield ""
        for space in self.componentModel.component.definition.spaceSwitching:
            yield space.label
    ...
    def _onCopyRequested(self, rows):
        data = {
            "fromComponent": self.componentModel.component.serializedTokenKey()
        }
        for row in rows:
            modelIndex, baseModel = modelutils.dataModelIndexFromIndex(row)
            spaceSwitch = self.currentSpaceSwitch()
            driver = spaceSwitch.drivers[modelIndex.row()]
            data.setdefault("items", []).append(driver)
        if data:
            outData = "HIVE_SPACESWITCH_DRIVERS:" + json.dumps(data)
            clipboard = QtWidgets.QApplication.clipboard()
            clipboard.setText(outData)

    def _onPasteRequested(self, rows):
        clipText = QtWidgets.QApplication.clipboard().text()
        if not clipText:
            return
        data = clipText.split("HIVE_SPACESWITCH_DRIVERS:")
        if len(data) != 2:
            return
        try:
            jsonStr = json.loads(data[1])
        ...
        for item in jsonStr["items"]:
            if currentSpaceSwitch.hasDriver(item["label"]):
                continue
            newDriver = api.SpaceSwitchDriverDefinition(item)
            driverLabelOrig = newDriver["driver"]
            # in the case we're pasting with "self" where the driver is self but doesn't exist
            # we need to remap to the component we copied from
            if driverLabelOrig not in driverIds:
                exprParts = api.splitAttrExpression(driverLabelOrig) if driverLabelOrig else []
                if exprParts:
                    if exprParts[0] == api.constants.ATTR_EXPR_SELF:
                        exprParts[0] = jsonStr["fromComponent"]
                        newDriver["driver"] = api.pathAsDefExpression(exprParts)
                    # in the case we're pasting a driver which == current component we need to remap to self
                    elif exprParts[0] == self.componentModel.component.serializedTokenKey():
                        exprParts[0] = api.constants.ATTR_EXPR_SELF
                        newDriver["driver"] = api.pathAsDefExpression(exprParts)
            currentSpaceSwitch.createDriver(newDriver)
            created = True

        if created:
            self.componentModel.component.saveDefinition(self.componentModel.component.definition)
            self.reloadTableModel()
    ...
    def onSpaceChanged(self, event):
        ...
        :param event:
        :type event: :class:`cgrig.libs.pyqt.extended.combobox.comboeditwidget.IndexChangedEvent`
        """
    ...
    def onDefaultDriverChanged(self, event):
        ...
        :param event:
        :type event: :class:`cgrig.libs.pyqt.extended.combobox.combobox.ComboItemChangedEvent`
        """
        text = event.text
        spaceSwitch = self.currentSpaceSwitch()
    ...
    def driversForComponent(self, component, driverExpression):
        ...
        if component == self.componentModel.component:
            ...
            return self._driversForComponent(component)

        driverData = [HiveSpaceSwitchController._emptyDriver]
        driverData += newDrivers
        return driverData

    def _driversForComponent(self, component):
        componentNameToken = component.serializedTokenKey()
        ...
        driverData += newDrivers
        return driverData
    def setDriverForRow(self, driverId):
        pass
