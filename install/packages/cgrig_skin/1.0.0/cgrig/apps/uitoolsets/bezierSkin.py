...
from cgrigvendor.Qt.QtCore import *
from cgrigvendor.Qt.QtWidgets import *
from cgrig.libs.weights import explorer
from cgrig.libs.weights import weight_tool
from cgrig.libs.weights import sculpt_weight
from cgrig.libs.weights import bezier_tool
from cgrig.libs.weights import weight_pickle
from cgrig.libs.weights import npTools
...
class BezierWeight(QDialog):
    ...
    def __init__(self):
        ...
        tool_bar.addAction(u"拷贝权重", weight_tool.copy_weights)
        tool_bar.addAction(u"重置蒙皮", weight_tool.re_skin)
        tool_bar.addAction(u"缓存权重", weight_tool.copy_joint_weight)
        tool_bar.addAction(u"加载权重", weight_tool.paste_joint_weight)
        tool_bar.addAction(u"限制影响", weight_tool.limit_max_influence)
        tool_bar.addAction(u"平滑权重", weight_tool.lock_influence_smooth)
        tool_bar.addAction(u"雕刻权重", sculpt_weight.auto_apply)

        self.max_influence = MaxInfluence(self)
        self.lock_smooth = LockInfluenceSmooth(self)
        menu = menu_bar.addMenu(u"工具")
        vx_menu = menu.addMenu(u"创建定位器")

        vx_menu.addAction(u"x", weight_tool.create_vx)
        vx_menu.addAction(u"y", weight_tool.create_y_vx)
        vx_menu.addAction(u"z", weight_tool.create_z_vx)
        ...
        menu.addAction(u"保存权重和模型", weight_pickle.save_weight_ui)
        menu.addAction(u"导入权重和模型并拷贝", weight_pickle.load_weight_ui)

        menu = menu_bar.addMenu(u"numpy")
        self.face_point = npTools.ui.FacePointWeight(self)
        menu.addAction(u"表面点", self.face_point.show)
...
class BezierSkinGUI(toolsetwidget.ToolsetWidget):
    id = "bezierSkin"
    info = "Create Helper Joint Settings GUI."
    uiData = {"label": "Bezier Skin Tool",
              "icon": "stream",
              "tooltip": "Bezier Split Skin GUI.",
    ...
    def widgetsAll(self, parent):
        ...
        :type parent: obj
        """
        self.bezier_widget = BezierWeight()
    ...
    def allLayouts(self, parent):
        ...
        mainLayout = elements.vBoxLayout(parent, margins=(uic.WINSIDEPAD, uic.WINBOTPAD, uic.WINSIDEPAD, uic.WINBOTPAD),
                                         spacing=uic.SREG)
        mainLayout.addWidget(self.bezier_widget)
