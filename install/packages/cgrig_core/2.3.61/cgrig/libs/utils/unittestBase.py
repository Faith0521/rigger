...
class CgRigTestResult(TextTestResult):
    def __init__(self, *args, **kwargs):
        super(CgRigTestResult, self).__init__(*args, **kwargs)
        self.testTimings = []
        self._startedAt = 0.0

    def startTest(self, test):
        self._startedAt = timeit.default_timer()
        super(CgRigTestResult, self).startTest(test)

    def addSuccess(self, test):
        super(CgRigTestResult, self).addSuccess(test)
        elapsed = timeit.default_timer() - self._startedAt
        name = self.getDescription(test)
        self.testTimings.append((name, elapsed))


class CgRigTestRunner(TextTestRunner):

    def __init__(self, slowTestThreshold=0.3, *args, **kwargs):
        self.slowTestThreshold = slowTestThreshold
        super(CgRigTestRunner, self).__init__(
            *args,
            **kwargs
        )

    def run(self, test):
        result = super(CgRigTestRunner, self).run(test)
        slowTimings = []
...
def runTests(directories=None,
    ...
    runner = CgRigTestRunner(stream=stream,
                           verbosity=2,
                           resultclass=resultClass if resultClass is not None else CgRigTestResult)
