...
import functools
import sys
from cgrigvendor.six import string_types, StringIO
from cgrig.libs.utils import commandline
...
def renameMultipleFilesNoExt(newNameNoExtension, fileFullPathToRenameList, skipThumbnails=True):
    """Rename multiple files/dependency folders useful for .cgrigScene files and check if all files can be renamed first.
...
def set_tool(context):
    try:
        ...
        cmds.warning('Was unable to set context to %s' % context)
...
class ProgressBarContext(object):
    ...
    >>>
    >>> progressBar=ProgressBarContext(maxValue=1000, step=1)
    >>>
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        if not self.disable:
            ...
            if exc_type:
                cmds.error('%s : %s' % (exc_type, exc_value))
            del(self)
