...
def offsetColor(col, offset=0):
    ...
    """
    return tuple([cgrigmath.clamp((c + offset), 0, 255) for c in col])
...
class RGBRotate(object):
    ...
    def apply(self, r, g, b):
        ...
        return cgrigmath.clamp(rx, 0, 255), cgrigmath.clamp(gx, 0, 255), cgrigmath.clamp(bx, 0, 255)
...
def hslColourOffsetFloat(rgb, hueOffset=0, saturationOffset=0, lightnessOffset=0):
    ...
    if saturationOffset:
        ...
        hsv = (hsv[0], cgrigmath.clamp(hsv[1] + saturationOffset), hsv[2])
        rgb = convertHsvToRgb(list(hsv))
    if lightnessOffset:
        rgb = (cgrigmath.clamp(rgb[0] + lightnessOffset),
               cgrigmath.clamp(rgb[1] + lightnessOffset),
               cgrigmath.clamp(rgb[2] + lightnessOffset))
    return rgb
