...
def iterShapeRootPaths():
    ...
    environment variable "CGRIG_LIB_PATH"
...
def iterAvailableShapesNames():
    ...
    "CGRIG_LIB_PATH" environment variable
...
def findShapePathByName(shapeName):
    ...
    ..code-block: python
        ...
        # result: rootFolder\cgrig\libs\shapelib\__init__.py
...
def loadAndCreateFromLib(shapeName, parent=None, shapeType=0, mod=None):
    """Load's and create's the nurbscurve from the shapelib.  If parent will shape node parent to the given object

    TODO: should combine cgrig_preferences and cgrig internal directories
    ...
    return curves.createCurveSurface(parent, newData, mod=mod)
...
def loadAndCreateFromPath(shapeName, folderPath, parent=None):
    ...
    shapeType = newData.get("shapeType", 0)
    if shapeType == 0:
        return curves.createCurveShape(parent,
                                       newData)  # todo move these out of cgrig_core or get createCurveShape app agnostic
    return curves.createCurveSurface(parent, newData)


def saveToLib(node, name, override=True, saveMatrix=False, shapeType=0):
    """Save's the current transform node shapes to the cgrig library
    ...
    return saveToDirectory(node, name, directory, override=override, saveMatrix=saveMatrix,shapeType=shapeType)


def saveToDirectory(node, name, directory, override=True, saveMatrix=False, shapeType=0):
    ...
    if name is None:
        from cgrig.libs.maya.api import nodes  # todo app agnostic
    ...
    if shapeType == 0:
        ...
        data = curves.serializeSurface(node)
...
def deleteShapeFromLib(shapeName, message=True):
    """Deletes a shape from the internal cgrig library.  Deletes the file on disk.
    ...
    if message:
        output.displayInfo("The Curve `{}` has been deleted from the CgRig Library".format(shapeName))
    return True


def renameLibraryShape(shapeName, newName, message=True):
    """Renames a shape from the internal cgrig library.  Renames the file on disk.
