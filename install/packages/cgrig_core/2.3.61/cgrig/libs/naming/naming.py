...
class NameManager(object):
    ...
    def __init__(self, config=None, configPath=None):
        ...
        if config is not None:
            self.copyFromConfigData(config)

    def __repr__(self):
        return "<NameManager {}>(name={}, path={})".format(
            id(self), self.name, self.configPath
        )

    def __eq__(self, other):
        return other.name == self.name and other.configPath == self.configPath

    def __ne__(self, other):
        return other.name != self.name or other.configPath != self.configPath

    def findParentAndChild(self, parentConfig):
        """For the given parent return the child within this current tree branch

        :param parentConfig:
        :type parentConfig: :class:`NameManager`
        :return:
        :rtype: :class:`NameManager`
        """
        currentParent = self.parentConfig
        childConfig = self
        if currentParent == parentConfig or currentParent is None:
            return childConfig

        while currentParent != parentConfig and currentParent is not None:
            childConfig = currentParent
            currentParent = currentParent.parentConfig
        return childConfig
    ...
    def addRule(
            self, name, expression, description, exampleFields, creator=None, recursive=True
    ):
        ...
        if not self.hasRule(name, recursive=recursive):
            ...
            newRule = Rule(
                name, creator, description, expression, exampleFields=exampleFields
            )
    ...
    def expressionFromString(self, name):
        ...
        if not possibles:
            raise ValueError(
                "Could not resolve name: {} to an existing expression".format(name)
            )
        ...
        if len(truePossibles) > 1:
            raise ValueError(
                "Could not Resolve name: {}, due too many possible expressions".format(
                    name
                )
            )
    ...
    def resolve(self, rule, fields):
        ...
        newStr = expression
        missingKeys = set()
        for field in expressionFields:  # type: str
            tokenValue = fields.get(field)  # type: Field or None
            resolvedField = self.field(field, recursive=True)
            if tokenValue is None:
                # if the field is strict then we want to fail otherwise we remove the field from the
                # string with an empty string.
                if resolvedField.strict:
                    missingKeys.add(field)
                    continue
                remappedValue = "_"
                pattern = NameManager._reFieldResolver.format(
                    field=surroundFieldExpr(field)
                )
            else:
                try:
                    remappedValue = resolvedField.valueForKey(tokenValue, tokenValue)
                except (
                        KeyError, AttributeError
                ):  # hit when the field doesn't exist, so we use the provided value
                    remappedValue = tokenValue
                if not remappedValue:
                    # basically empty string so removes the field and makes sure only
                    # one underscore exists
                    pattern = NameManager._reFieldResolver.format(
                        field=surroundFieldExpr(field)
                    )
                    remappedValue = "_"
                else:
                    pattern = surroundFieldExpr(field)
            newStr = re.sub(pattern, remappedValue, newStr)
        if missingKeys:
            raise ValueError(
                "Missing Expression fields, rule: {}, fields:{}".format(
                    rule, missingKeys
                )
            )
        return newStr
    ...
    def copyFromConfigData(self, config):
        """Applies the provide config local
        :param config:
        :type config:
        :return:
        :rtype:
        """
        self._fields = [
            Field.fromDict(fieldMap) for fieldMap in config.get("fields", [])
        ]
    ...
    def _load(self, configPath):
        ...
        self.copyFromConfigData(self.originalConfig)
...
class Field(object):
    ...
    def fromDict(cls, data):
        ...
        keyValues = [
            KeyValue(k, v, protected=k in perms)
    ...
    def valueForKey(self, key, defaultValue=None):
...
def surroundFieldExpr(fieldValue):
    """Given a field value, returns the expression format for the field.

    :param fieldValue:  The field value to surround with the expression format.
    :type fieldValue: Str
    :return: The field value surrounded by "{}" e.g. {fieldValue}
    :rtype: str
    """
    return "{" + fieldValue + "}"
