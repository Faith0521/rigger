...
Logic License: MIT, see cgrig.libs.tweenmachine.LICENSE
"""
...
class TweenPopup(elements.CgRigWindowThin):
    ...
    def tweenTextChanged(self):
        """Slider text changed"""
        if self.count == 1:  # hack as is triggering twice, second time is correct
            cmds.undoInfo(openChunk=True)
            # restart as the mouse may be out of the ui
            self.tweenInstance.clearCache()
            self.tweenInstance.tweenSel(self.tweenFloatSlider.value(),
                                        start=False,
                                        message=False)
            # do the tween
            self.tweenInstance.tweenCachedDataOnce(self.tweenFloatSlider.value())
            # Add the counter for double press
            self.count = 0  # Hack as is triggering twice, will trigger on second go.
            cmds.undoInfo(closeChunk=True)
        self.count += 1  # add one as is 0 and first time round with incorrect value
...
def main(cursorOnSlider=False, clickOffClose=False, closeOnRelease=False):
    ...
    win = eng.showDialog(TweenPopup, name="tweenPopup", allowsMultiple=False, qtPopup=clickOffClose,
                         closeOnRelease=closeOnRelease)  # CgRig open and manage window.
    ...
    win.show(point)
    output.displayInfo("Tween Machine Popup Opened")
