...
from cgrig.libs.pyqt.widgets import extendedmenu, treeview, frame, searchwidget
from cgrig.libs.pyqt.extended import clippedlabel
from cgrig.libs.pyqt.models import sortmodel, modelutils
from cgrigvendor.Qt import QtWidgets, QtCore, QtCompat
from cgrig.libs.pyqt.widgets import slidingwidget
...
class TreeViewPlus(frame.QFrame):
    ...
    def setSortingEnabled(self, enabled):
        ...
        self.treeView.sortByColumn(0 if enabled else -1, QtCore.Qt.AscendingOrder)
        self.proxySearch.setDynamicSortFilter(enabled)
        if enabled:
            self.proxySearch.setFilterKeyColumn(0)
        self.sorting = enabled

    def selectedItems(self, rowIndices=True, columnIndices=True):
        modelIndices = []
        selected = []
        if rowIndices and columnIndices:
            selected = self.selectionModel().selection().indexes()
        elif rowIndices and not columnIndices:
            selected = self.selectionModel().selectedRows()
        elif columnIndices and not rowIndices:
    ...
    def connections(self):
        self.searchEdit.textChanged.connect(self.proxySearch.setFilterRegularExpression)
