...
from cgrig.libs.pyqt.widgets import dialog, layouts
    ...
    def addItem(self, text, data=None):
        ...
        for i, d in enumerate(data):
            if self.parentWidget().comboRounding is not None and type(text[0]) is float:
                ...
                item = ComboStandardItem(formatText.format(round(text[0], self.parentWidget().comboRounding)),
                                         parent=self)
            else:
...
class ComboMenuPopup(dialog.Dialog):
    ...
    returnedPressed = QtCore.Signal(ComboCustomEvent)
    comboRounding = -1
    ...
    def __init__(self, parent=None, maxHeight=300, showOnInitialize=False, comboEdit=None, rounding=None):
        ...
        :type comboEdit: cgrig.libs.pyqt.extended.combobox.comboeditwidget.ComboEditWidget
        """
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == QtCore.Qt.Key_Up:
            ...
            self.returnedPressed.emit(self.eventFromIndex(self.currentIndex()))
        event.accept()
