...
from cgrig.libs.pyqt.widgets.popups import MessageBox
from cgrigvendor.Qt import QtCore, QtWidgets, QtGui

from cgrig.libs.pyqt import utils, uiconstants as uic
from cgrig.libs.pyqt.extended.imageview.thumbnail import directorypopup
...
from cgrig.libs.pyqt.extended.imageview.thumbnail.infoembedwindow import InfoEmbedWindow
from cgrig.libs.pyqt.extended.searchablemenu import TaggedAction
from cgrig.libs.pyqt.extended.snapshotui import SnapshotUi
from cgrig.libs.pyqt.widgets import searchwidget, layouts, buttons
...
from cgrig.libs.pyqt.widgets.layouts import hBoxLayout, vBoxLayout

from cgrig.libs import iconlib
from cgrig.libs.cgrigscene import cgrigscenefiles, constants
from cgrig.libs.utils import general, output, filesystem


if general.TYPE_CHECKING:
    from cgrig.libs.pyqt.extended.imageview import model as thumbnailmodel
...
class MiniBrowser(QtWidgets.QWidget):
    ...
    def __init__(self,
        ...
        :type toolsetWidget: cgrig.apps.toolsetsui.widgets.toolsetwidget.ToolsetWidget
    ...
    def newSnapshotWidget(self):
        ...
        self.snapshotWgt = SnapshotUi(engine.currentEngine().host.qtMainWindow, onSave=self.screenshotSaved.emit)
    ...
    def connections(self):
        ...
        cgrigWindow = CgRigWindow.getCgRigWindow(self)

        if cgrigWindow:
            cgrigWindow.minimized.connect(self.directoryPopup.close)
    ...
    def _updatePathThumbnail(self):
        ...
        if self.snapshotType == MiniBrowser.SNAPTYPE_EDIT:
            if not os.path.dirname(item.thumbnail).endswith(constants.DEPENDENCY_FOLDER):
                dep = cgrigscenefiles.getDependencyFolder(item.fullPath(), create=False)[0]
    ...
    def itemFilePath(self):
        ...
        return self.infoEmbedWindow.metadata['cgrigFilePath']
    ...
    def itemDependencyPath(self, create=False):
        ...
        try:
            if self.infoEmbedWindow.metadata is None:
                return None
            ret = self.infoEmbedWindow.metadata.get('cgrigFilePath')
            if ret is None:
                return None
        except TypeError:
            return None
        return cgrigscenefiles.getDependencyFolder(ret, create=create)[0]
    ...
    def setModel(self, model):
        ...
        :param model:
        :type model: cgrig.libs.pyqt.extended.imageview.model.FileModel
        :return:
        :rtype:
        """
    ...
    def _setModelDirectories(self, directories):
        ...
        :param directories:
        :type directories: list[cgrig.core.tooldata.tooldata.DirectoryPath]
        :return:
        """
    ...
    def itemSelected(self, name, item):
        ...
        :param item:
        :type item:  :class:`cgrig.libs.pyqt.extended.imageview.items.BaseItem`
        :return:
        :rtype:
        """
    ...
    def currentItem(self):
        ...
        :return:
        :rtype: :class:`cgrig.libs.pyqt.extended.imageview.items.BaseItem`
        """
    ...
    def model(self):
        ...
        :rtype: :class:`cgrig.libs.pyqt.extended.imageview.model.MiniBrowserFileModel`
        """
...
class ThumbnailSearchWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None, themePref=None):
        ...
        :type parent:
        :param themePref:
        :type themePref: preferences.interface.preference_interface.CgRigToolsPreference
        """
