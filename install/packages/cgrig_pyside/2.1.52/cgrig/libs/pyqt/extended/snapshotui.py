...
from cgrig.libs.pyqt.extended.lineedit import LineEdit, IntLineEdit
from cgrig.libs.pyqt.widgets.frameless import window
from cgrig.libs.pyqt.widgets.buttons import styledButton

from cgrig.libs.pyqt.widgets.label import Label
from cgrig.libs.pyqt.widgets.layouts import vBoxLayout, hBoxLayout
...
class SnapshotUi(window.CgRigWindowThin):
    saved = QtCore.Signal(object)  # QPixmap

    def __init__(self, parent=None, onSave=None, width=512, height=512):
        """Snapshot ui
    ...
    def initUi(self):
        ...
        self.lockedBtn = styledButton(
            icon="unlock", style=uiconstants.BTN_TRANSPARENT_BG
        )
        ...
        self.cancelBtn = styledButton(
            text="Cancel", icon="xMark", style=uiconstants.BTN_DEFAULT_QT
        )
    ...
    def setupTitleBar(self):
        ...
        self.titleBar.titleLayout.setContentsMargins(
            *utils.marginsDpiScale(0, 3, 15, 7)
        )
        self.titleBar._mainRightLayout.setContentsMargins(
            *utils.marginsDpiScale(0, 0, 6, 0)
        )
    ...
    def snapshot(self, hide=False):
        ...
        self.snapshotPx = utils.desktopPixmapFromRect(rect, self)
    ...
    def sizeEditChanged(self):
        ...
        if self.locked or (
            self.sender() is not None and self.sender().signalsBlocked()
        ):
            return
    ...
    def toggleAspectClicked(self):
        ...
        if self.keepAspect:
            ...
            self.aspectLinkBtn.setIconColor(
                (192, 192, 192)
            )  # todo: this needs to be fixed at styledbutton, and not done here
            self.window().resize(
                self.window().height() + self.widthOffset(),
    ...
    def bottomSize(self):
        return (
            ...
            + self.windowResizer.botResize.height()
            + utils.dpiScale(2)
        )
    ...
    def windowResize(self):
        ...
        if self.keepAspect:
            self.window().resize(
                self.window().width(), self.window().width() + self.bottomSize()
            )
        self.updateWidgets()
    ...
    def verticalResize(self):
        ...
        if self.keepAspect:
            self.window().resize(
                self.window().height() - self.bottomSize(), self.window().height()
            )
        self.updateWidgets()
