...
from cgrig.libs.pyqt.extended.combobox import combomenupopup
from cgrig.libs.pyqt.widgets import layouts, label as lbl, iconmenu, stringedit, buttons
import math
...
class ComboEditWidget(QtWidgets.QWidget):
    ...
    # emitted when either item is clicked or return is pressed while item is selected
    # note: this is was created to avoid itemChanged being changed when listView selected is updated eg. up/down keys
    # this helps to delay data changes(scene eg. hive) being changed N amount of times instead of once.
    itemClickedOrReturned = QtCore.Signal(object)
    ...
    def __init__(self, parent=None, label="", text=None, items=None, data=None, editCount=1, primaryIcon="sortDown",
        ...
        self.supportMiddleMouseScroll = supportMiddleMouseScroll
    ...
    def connections(self):
        ...
        self.comboPopup.itemClicked.connect(self.itemClickedOrReturned.emit)
        self.comboPopup.returnedPressed.connect(self.itemClickedOrReturned.emit)
    ...
    def wheelEvent(self, event):
        ...
        :rtype:
        """
        if self.supportMiddleMouseScroll:
    ...
    def indexChanged(self, event):
        ...
        :param event:
        :type event: cgrig.libs.pyqt.extended.combobox.combomenupopup.ComboCustomEvent
        :return:
        :rtype:
        """
        if event is None:
    ...
    # backwards compatibility, use setCurrentIndex() instead
    setIndexInt = setCurrentIndex
    ...
    def currentIndex(self):
        ...
        return index.row()
    # backwards compatibility, use currentIndex() instead
