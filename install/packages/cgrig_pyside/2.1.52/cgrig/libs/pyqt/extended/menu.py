# -*- coding: utf-8 -*-
from cgrigvendor.Qt import QtWidgets, QtCore, QtCompat
...
class MenuCreateClickMethods(object):
    ...
    .. code-block:: python
        ...
        cgrigQtWidget.setMenu(myQMenu, mouseButton=QtCore.Qt.RightButton)  # add menu to the UI with a mouse click

    Or if you are using ExtendedMenu in cgrigcore_pro, you can easily automatically build the menu items/icons:

    .. code-block:: python

        myExtMenu =  extendedmenu.ExtendedMenu()  # create an empty cgrigcore_pro menu (an extended QMenu)
        theMenuModeList = [("menuIcon1", "Menu Item Name 1"),
                            (" menuIcon1  ", " Menu Item Name 2")]  # create the menu names and icons
        cgrigQtWidget.setMenu(myExtMenu, modeList=theMenuModeList, mouseButton=QtCore.Qt.RightButton)  # add menu

    ExtendedMenu in cgrigcore_pro can connect when a menu is changed with
    ...
    (must be a cgrigcore_pro ExtendedMenu)
    ...
    def addActionList(self, modes, mouseButton=QtCore.Qt.RightButton):
        ...
        Note: Use this method only if the menu is an ExtendedMenu from cgrigcore_pro
        ...
        menu = self.getMenu(mouseButton=mouseButton)
        if menu is not None:
            menu.actionConnectList(modes)  # this only exists in ExtendedMenu which is not in cgrigcore
