...
from cgrigvendor.Qt import QtGui, QtCore, QtWidgets
from cgrig.libs.pyqt import thread, utils
...
class BaseItem(object):
    ...
    def __init__(self, name=None, description=None, iconPath=None, filePath="", toolTip="", thumbnail="",
        ...
        self.ext = ""
        self.imageExt = "jpg"
        self.autoThumbFromImage = autoThumbFromImage  # will try to create thumbnail from image if asset is image
    ...
    def setFilePath(self, path):
        ...
        if self.ext in QT_SUPPORTED_EXTENSIONS + [i.upper() for i in QT_SUPPORTED_EXTENSIONS]:
    ...
    def thumbnail(self):
        ...
        :return:
        """
        # if self.autoThumbFromImage try to auto find the thumbnail from the image
        thumbnail = self.thumbnailLookPath(autoFind=self.autoThumbFromImage)
    ...
    def thumbnailLookPath(self, autoFind=True):
        ...
        if not self._thumbnail:
            if autoFind:  # tries to find the thumbnail from the fileName if it is already and image?
                return os.path.join(self.directory, self.fileName + "." + self.imageExt)
            return self._thumbnail  # type: str or None
