...
from cgrig.libs.pyqt.uiconstants import QT_SUPPORTED_EXTENSIONS
...
from cgrig.core.util import zlogging, env

from cgrigvendor import six
from cgrigvendor.Qt import QtGui, QtCore, QtWidgets, QtCompat

from cgrig.libs.pyqt.extended.imageview import items
from cgrig.libs.pyqt.extended.imageview.items import TreeItem
    ...
    def filterAcceptsRow(self, sourceRow, sourceParent):
        ...
        for role in (QtCore.Qt.DisplayRole, tagRole,
            ...
            if requestedRole == role:
                ...
                consolidatedData += str(data)
        return filterRegExp.match(consolidatedData).capturedStart() != -1
...
class FileModel(ItemModel):
    ...
    def __init__(self, view, extensions, directories=None, activeDirs=None, chunkCount=0, uniformIcons=False,
        ...
        :type chunkCount: int
        :param extensions: The file extensions to find for each item eg. ["ma", "cgrigScene", "shape", etc]
        ...
        if not env.isWindows():
            self.extensions = list(set([i.upper() for i in extensions] + [i.lower() for i in extensions])) + extensions
        else:
...
class MiniBrowserFileModel(FileModel):
    ...
    def __init__(self, view, extensions, directories=None, activeDirs=None, chunkCount=0, uniformIcons=False,
        ...
        :param includeSubdir:
        :param assetPref:
        :type assetPref: cgrig.preferences.assets.BrowserPreference
        """
    ...
    def createItemFromFileAndDirectory(self, directory, filePath, loadImage=False):
        ...
        item.metadata = cgrigscenefiles.infoDictionary(item.fileNameExt(), item.directory)
