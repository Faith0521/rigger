...
from cgrig.libs.pyqt.widgets import layouts, buttons
from cgrig.libs.pyqt.models import treemodel, datasources, delegates, constants as modelconstants
...
class DirectoryPopup(CgRigWindow):
    ...
    def deleteButtonClicked(self):
        ...
        result = popups.MessageBox.showWarning(title="Remove Directory",
                                               message="Are you sure you want to remove the following path from CgRig Tools?  "
                                                       "Files will remain on disk, only the cgrig alias/s will be removed. \n\n- {}".
    ...
    def show(self, reattach=True):
        ...
        if reattach:
            self._attached = True
        newPos = self.moveAttached()
        super(DirectoryPopup, self).show(move=newPos)
        if self._anchorWidget is not None:
            CgRigWindow.getCgRigWindow(self._anchorWidget).titleBar.moving.connect(self._parentMoved)
    ...
    def _parentMoved(self, newPos, delta):
        ...
        if self._attachToParent and self._attached:
            ...
            self.moveAttached(newPos, offset=(0, delta.y()))
