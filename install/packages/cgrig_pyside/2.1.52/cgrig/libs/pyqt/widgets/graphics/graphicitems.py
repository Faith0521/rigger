...
class ConnectionEdge(QtWidgets.QGraphicsPathItem):
    ...
    def __init__(self, source, destination=None, curveType=CUBIC, colour=defaultColor):
        ...
        self.defaultPen = QtGui.QPen(colour, 1.25, QtCore.Qt.DashLine)
        self.defaultPen.setDashPattern([1, 2, 2, 1])
        self.selectedPen = QtGui.QPen(self.selectedColor, 1.7, QtCore.Qt.DashLine)
        self.selectedPen.setDashPattern([1, 2, 2, 1])

        self.hoverPen = QtGui.QPen(self.hoverColor, 1.7, QtCore.Qt.DashLine)
        self.selectedPen.setDashPattern([1, 2, 2, 1])
        self.hovering = False
...
class GraphicsText(QtWidgets.QGraphicsWidget):
    ...
    def onResize(self, width):
        fmWidth = self._fontMetrics.horizontalAdvance(self.item.toPlainText())
