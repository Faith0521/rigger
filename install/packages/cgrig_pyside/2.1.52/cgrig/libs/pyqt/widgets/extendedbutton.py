...
from cgrigvendor.Qt import QtWidgets, QtCore, QtGui, QtCompat

from cgrig.libs import iconlib
from cgrig.libs.pyqt import utils, uiconstants
from cgrig.libs.pyqt.extended import searchablemenu, menu
from cgrig.libs.pyqt.widgets import expandedtooltip, dpiscaling
from cgrig.libs.pyqt.extended.searchablemenu import action as taggedAction
...
class ButtonIcons(QtWidgets.QAbstractButton):
    ...
    def enterEvent(self, event):
        ...
        :return:
        """
        super(ButtonIcons, self).enterEvent(event)
    ...
    def leaveEvent(self, event):
        ...
        :return:
        """
        super(ButtonIcons, self).leaveEvent(event)
...
class ExtendedButton(QtWidgets.QPushButton, ButtonIcons):
    ...
    def __init__(self, icon=None, iconHover=None, text=None, parent=None, doubleClickEnabled=False, iconColorTheme=None,
        ...
        self.middleClicked.connect(partial(self.contextMenu, QtCompat.Qt.MidButton))
    ...
    def setMaximumHeight(self, maxh):
        ...
        :param maxh: float
        """
        super(ExtendedButton, self).setMaximumHeight(utils.dpiScale(maxh))
    ...
    def setMaximumWidth(self, maxw):
        ...
        :param maxw: float
        """
        super(ExtendedButton, self).setMaximumWidth(utils.dpiScale(maxw))
    ...
    def isSearchable(self, mouseMenu=QtCore.Qt.LeftButton):
        ...
        :param mouseMenu: QtCore.Qt.Button
        :return: bool
        """
        if self.clickMenu[mouseMenu] is not None:
    ...
    def clearMenu(self, mouseMenu=QtCore.Qt.LeftButton):
        ...
        :param mouseMenu: QtCore.Qt.LeftButton, QtCompat.Qt.MidButton, QtCore.Qt.RightButton
        :return:
        """
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == QtCore.Qt.LeftButton:
            super(ExtendedButton, self).mousePressEvent(event)
            self.setDown(True)
        elif event.button() == QtCompat.Qt.MidButton:
            self.setDown(False)
    ...
    def mouseReleaseEvent(self, event):
        ...
        :return:
        """
        super(ExtendedButton, self).mouseReleaseEvent(event)
        button = event.button()
        self.setDown(False)
    ...
    def mouseMoveEvent(self, event):
        """if mouse is moved out of button ,while Left-mouse-pressed and hold
        """
        super(ExtendedButton, self).mouseMoveEvent(event)
    ...
    def mouseSingleClickAction(self, button):
        ...
        if self.lastClick == self.SINGLE_CLICK or self.doubleClickEnabled is False:
            if button == QtCore.Qt.LeftButton:
                if not self.mouseOutOfButton:  # if mouse is over the Button
                    self.leftClicked.emit()  # emit signal
                    return True
            elif button == QtCompat.Qt.MidButton:
        ...
        self.mouseOutOfButton = False  # set back to False
    ...
    def addAction(self, name, mouseMenu=QtCore.Qt.LeftButton,
        ...
        :param mouseMenu: Expects QtCore.Qt.LeftButton, QtCompat.Qt.MidButton, or QtCore.Qt.RightButton
...
class ExtendedButtonSimpleMenu(QtWidgets.QPushButton, menu.MenuCreateClickMethods):
    ...
    def __init__(self, parent=None, menuVOffset=20):
        ...
        Menus are not added by default they must be set in the ui code. QMenu's can be passed in via setMenu():
            ...
            cgrigQtWidget.setMenu(myQMenu, mouseButton=QtCore.Qt.RightButton)

        Recommended to use cgrigcore_pro's extendedmenu.ExtendedMenu(). Pass that in instead of a QMenu for extra
        functionality
    ...
    def mousePressEvent(self, event):
        ...
        for mouseButton, menu in iter(self.clickMenu.items()):
            if menu and event.button() == mouseButton:  # if a menu exists and that mouse has been pressed
                ...
                if mouseButton == QtCompat.Qt.MidButton:
