...
class GraphicsView(QtWidgets.QGraphicsView):
    ...
    def _setCgRigm(self, value, sensitivity=None, position=None):
        if position:
        ...
        cgrigm = self.get_cgrigm()
        if CGRIGM_MIN >= cgrigm:
            if scale == 0.9:
                return
        if CGRIGM_MAX <= cgrigm:
            if scale == 1.1:
                return
        self.scale(scale, scale, position)
