...
from cgrig.libs.pyqt.widgets import roundbutton, label, layouts, dpiscaling
from cgrig.libs.pyqt.widgets.extendedbutton import ExtendedPushButton, ExtendedButton, ShadowedButton
...
def buttonRound(**kwargs):
    ...
    :keyword icon (str): the icon name from the cgrig tools iconlib.
...
def styledButton(text=None, icon=None, parent=None, toolTip="", textCaps=False, iconColor=None, iconHoverColor=None,
    ...
    """ Create a button with text or an icon in various styles and options.::
        ...
        Style - 0 - uic.BTN_DEFAULT - Default cgrig extended button with optional text or an icon.
        Style - 1 - uic.BTN_TRANSPARENT_BG - Default cgrig extended button w transparent bg.
        Style - 2 - uic.BTN_ICON_SHADOW - Main cgrig IconPushButton button (icon in a colored box) with shadow underline
        Style - 3 - uic.BTN_DEFAULT_QT - Default style uses vanilla QPushButton and not cgrig's extended button
...
class AlignedButton(ExtendedButton, dpiscaling.DPIScaling):
    ...
    def setIcon(self, icon):
        ...
        :param icon:
        :type  icon: cgrigvendor.Qt.QtGui.QIcon
        :return:
        """
        if not self._icon:
            return
    ...
    def mousePressEvent(self, event):
        super(AlignedButton, self).mousePressEvent(event)
        self._icon.setPixmap(self.iconPressedPixmap)

    def mouseReleaseEvent(self, event):
        super(AlignedButton, self).mouseReleaseEvent(event)
        self._icon.setPixmap(self.iconPixmap)
...
def leftAlignedButton(text, icon=None, toolTip="", parent=None, transparentBg=False,
    ...
    CgRig icons are specified without a colon in the name.

        icon = "save"

    :param text: The button label text
    :type text: str
    :param icon: The icon name, ":iconName" is resource icon, "save" is from CgRig icon library
    :type icon: :class:`QtGui.QIcon`
