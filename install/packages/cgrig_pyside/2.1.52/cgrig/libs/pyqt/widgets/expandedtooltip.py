...
from cgrigvendor.six.moves.html_parser import HTMLParser

from cgrigvendor.Qt import QtCore, QtWidgets, QtGui

from cgrig.libs import iconlib
from cgrig.libs.pyqt.widgets import dialog
    ...
    .. code-block:: python
        ...
        {"expanded":
            ''' Expand / Collapse Widget
                ...
                <p>Gifs can be used as well. <cgrig gif=\"selectAllAnimCurvesInTheScene\" /></p>
                <p> <a href=\"http://create3dcharacters.com\">Click for full documentation</a>"'''}

    Gifs folders are set up in the package.json:
    "HIVE_UI_GIFS": "{self}/cgrig/apps/hiveartistui/gifs"
...
class WidgetsFromTextParser(HTMLParser):
    ...
    Once it reaches a <cgrig> tag, it creates a new widget for each cgrig tag.
    """
    ...
    def handle_starttag(self, tag, attrs):
        ...
        Here we add our own tags eg. <cgrig gif="example" />.
        ...
        # if we find our own tag eg <cgrig gif="example" />
        if tag == "cgrig":
            if attrs[0][0] == "gif":
                rem = len(startTag)
...
def installTooltips(widget, tooltipDict):
    ...
    Works in conjunction with artistui.keyPressEvent() to display a popup (cgrig.apps.hiveartistui.views.expandedtooltip.ExpandedTooltipPopup)
