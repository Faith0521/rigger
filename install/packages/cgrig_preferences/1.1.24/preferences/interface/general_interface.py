...
class GeneralPreferences(prefinterface.PreferenceInterface):
    ...
    def autoLoadPlugin(self):
        ...
        :rtype: bool
        """
        # local import because cgrig_preferences gets loaded outside of maya at times.
        ...
        try:
            ...
            output.displayError("Unable to load cgrigtools startup file")
        return True
...
def pluginPath():
    ...
    pluginFolder = os.path.dirname(cmds.pluginInfo("cgrigtools", path=True, q=True))
    return os.path.join(os.path.dirname(pluginFolder), "cgrigtools_maya_startup.json")
