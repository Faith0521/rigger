...
from cgrig.libs.pyqt import stylesheet, utils
from cgrig.libs.utils import color, filesystem, path
from cgrig.core.util import zlogging
from cgrig.preferences import prefinterface, preferencesconstants
from cgrig.core import api
from cgrigvendor.six import string_types

from cgrig.preferences.themedict import ThemeDict
...
class UpdateThemeEvent(object):
    def __init__(self, stylesheet, themeDict, preference):
        ...
        :type stylesheet: str
        :param themeDict:
        :type themeDict: :class:`cgrig.preferences.themedict.ThemeDict`
        :param preference:
        :type preference: :class:`CgRigToolsPreference`
        """
        self.theme = None
...
class CgRigToolsPreference(prefinterface.PreferenceInterface):
    ...
    class ThemeUpdater(QtCore.QObject):
        ...
        update = QtCore.Signal(
            UpdateThemeEvent
        )  # stylesheet, themeDict, CgRigToolsPreference
    ...
    # internal use only, cgrig preference package name
    _packageName = "cgrig_preferences"
    _settings = None
    _themeUpdate = ThemeUpdater()

    def __init__(self, preference):
        """Main interface for cgrig preferences. This class shouldn't be
        instanced directly but through the PreferenceManager instance.
        """
        super(CgRigToolsPreference, self).__init__(preference=preference)
    ...
    def currentTheme(self):
        """Returns the current theme name for cgrigtools.

        :return: The style sheet theme name
        :rtype: str
        """
        return self.preference.findSetting(
            self._relativePath, root=None, name="current_theme"
        )

    def defaultPreferencesPath(self):
        return "~/cgrig_preferences"
    ...
    def themes(self):
        ...
        """
        return self.preference.findSetting(
            self._relativePath, root=None, name="themes"
        ).keys()
    ...
    def stylesheet(self, theme=None):
        ...
        :rtype: :class:`cgrig.libs.pyqt.stylesheet.StyleSheet`
        """
        if theme is None:
    ...
    def stylesheetSetting(self, key, theme=None):
        ...
        try:
            ...
            logger.error(
                "Incorrectly formatted stylesheet: {}".format(self._relativePath)
            )
            raise

        if ret is None:
            self.preference.defaultPreferenceSettings(
                self._packageName, self._relativePath
            )
    ...
    def revertThemeToDefault(self, theme=None, save=True):
        ...
        :rtype:
        """
        default = self.preference.defaultPreferenceSettings(
            "cgrig_preferences", "global/stylesheet"
        )
    ...
    def __getattr__(self, item):
        ...
        if setting is None:
            return super(CgRigToolsPreference, self).__getattribute__(item)
    ...
    def stylesheetForTheme(self, theme):
        ...
        if data is None:
            raise ValueError(
                "Current styleSheet theme doesn't exist, Theme:{}".format(theme)
            )
    ...
    def themeDict(self, theme):
        ...
        if not stylePrefs:
            stylePrefs = self.preference.defaultPreferenceSettings(
                self._packageName, self._relativePath
            )
    ...
    def styleSheetPath(self):
        ...
        return os.path.join(
            os.path.dirname(preferenceLocation), "cgrigtools_style.qss"
        )
    ...
    def stylesheetFromData(self, data):
        ...
        :type data: dict
        :return: :class:`cgrig.libs.pyqt.stylesheet.StyleSheet`
        """
        stylePath = self.styleSheetPath()
        return stylesheet.StyleSheet.fromPath(stylePath, **data)

    def bakePreferenceRoots(self):
        """Bake Preference Roots
        Bakes the Preferences root paths and names to cgrigtoolspro/config/env/preferences_root.config

        """
        rootConfig = api.currentConfig().preferenceRootsPath()
        rootPaths = {
            rootName: str(rootObject)
            for rootName, rootObject in self.preference.roots.items()
        }
    ...
    def assetFolder(self, folder=""):
        ...
        Example::
            ...
            # Output: userPath/cgrig_preferences/assets/model_assets
    ...
    def userPreferences(self):
        ...
        Retrieves the path to the CgRig Tools User Preferences,
        if '~/cgrig_preferences' then displays full path
