...
from cgrig.libs.utils import filesystem, output
...
from cgrig.preferences import preferencesconstants
from cgrig.preferences import constants as coreprefconstants
...
class GeneralPrefsWidget(prefmodel.SettingWidget):
    ...
    def uiWidgets(self):
        ...
        # Light Presets Path -----------------------------------------
        toolTip = "The location of the CgRig User Preferences folder. \n" \
                  "All CgRig Asset and Prefs folder locations are found in this folder unless \n" \
                  "otherwise specified. \n" \
                  "Moving this folder will require a Maya restart."
        self.userPrefPathLbl = elements.Label("CgRig Preferences Folder", parent=self, toolTip=toolTip)
        self.userPrefPathTxt = elements.StringEdit(label="",
                                                   editText=self.initialUserPrefsPath,
                                                   toolTip=toolTip)
        toolTip = "Browse to change the CgRig User Preferences folder."
        ...
        resetToolTip = "Reset the CgRig User Preferences folder to it's default location."
        ...
        self.dotsMenu.addAction("Reset Preferences to default",
                                icon="arrowBack",
                                connect=self.setCgRigPrefsDefault,
        ...
        if env.isMaya():
            toolTip = "Auto-loads CgRig Tools Pro when Maya starts.\n" \
                      "If not checked CgRig Tools will need to be loaded with\n" \
                      "Maya's Plugin Manager after starting Maya. \n\n" \
                      " - Windows > Settings/Preferences > Plug-in Manager"
            self.autoLoadCheckBox = elements.CheckBox(label="Load CgRigTools on Start Up",
                                                      checked=self.generalInterface.autoLoadPlugin(),
                                                      parent=self,
                                                      toolTip=toolTip)
            toolTip = "Sets the CgRigToolsPro shelf as the the primary active shelf after loading Maya.\n" \
                      " - Note: Plugins that also have shelves that load after CgRig will override this setting."
            self.shelfActiveCheckBox = elements.CheckBox(label="Shelf Active On Start Up",
                                                         ...
                                                         parent=self,
                                                         toolTip=toolTip)
            toolTip = "Auto-loads the CgRigToolsPro shelf when Maya starts.\n" \
                       "If not checked then you will need to load the shelf with\n" \
                       " - CgRigTools Menu > Preferences > Build CgRig Shelves"
            self.autoLoadShelfCheckBox = elements.CheckBox(label="Load CgRigTools Shelf on Start Up",
                                                           checked=self.toolPaletteInterface.loadShelfAtStartup(),
                                                           parent=self,
                                                           toolTip=toolTip)
    ...
    def uiLayout(self):
        ...
        if env.isMaya():
            ...
            startupCheckboxLayout.addWidget(self.autoLoadShelfCheckBox)
    ...
    def ui_changePrefsPopup(self):
        ...
        """
        message = "Are you sure you want to change the CgRig User Preferences Folder? \n\n" \
        ...
        okPressed = elements.MessageBox.showOK(title="Change CgRig User Preferences "
    ...
    def ui_resetPopup(self, prefsDefaultPath):
        ...
        :rtype okPressed: bool
        """
        message = "Are you sure you want to return to the DefaultCgRig User Preferences Folder? \n\n" \
                  "{} \n\n" \
                  "This will move the entire preferences directory with all files and assets.\n" \
    ...
    def ui_restartPopup(self, newPrefsPath):
        """Popup window dialog asking the user to restart Maya after the CgRig User Preferences have changed.

        :param newPrefsPath: The new path to the cgrig_preferences folder
        :type newPrefsPath: str
        :return okPressed: True if the ok button was pressed, else False if cancelled
        :rtype okPressed: bool
        """
        message = "The CgRig User Preferences path has been moved to \n\n  " \
                  "{}\n\n" \
    ...
    def checkRenderLoaded(self):
        ...
        """
        try:  # cgrig_core_pro may not be loaded (though unlikely
            from cgrig.libs.maya.cmds.renderer import rendererload
            ...
            output.displayWarning("Check renderer loaded failed, the package cgrig_core_pro may "
                                  "not be loaded")
    ...
    def global_changeRenderer(self, event=None):
        ...
        try:  # toolsets may not be loaded
            ...
            output.displayWarning("Change renderer did not send to Toolsets, the package cgrig_toolsets may "
                                  "not be loaded")
    ...
    def setCgRigPrefsDefault(self):
        """Sets the UI widget path text to the default CgRig User Preferences path"""
        cgrigDefaultPrefsPath = os.path.normpath(os.path.expanduser("~/cgrig_preferences"))  # check this
        if os.path.normpath(self.userPrefPathTxt.text()) == cgrigDefaultPrefsPath:
            return  # hasn't changed from the saved version
        if self.coreInterface.userPreferences() == cgrigDefaultPrefsPath:
            self.userPrefPathTxt.setText(cgrigDefaultPrefsPath)  # matches the saved path
            return
        okPressed = self.ui_resetPopup(cgrigDefaultPrefsPath)
        if not okPressed:
            return
        self.userPrefPathTxt.setText(cgrigDefaultPrefsPath)
    ...
    def setPrefsFolderClicked(self):
        ...
        newDirPath = QtWidgets.QFileDialog.getExistingDirectory(self, "Set the CgRig User Preferences "
    ...
    def openPrefsFolder(self):
        ...
        output.displayInfo("OS window opened to the `CgRig User Preferences` folder location")
    ...
    def _handlePreferencePathChange(self):
        ...
        if previousPath == newDirPath:  # no change
            return False
        # should we really force the folder name to be cgrig_preferences?
        index, resText = elements.MessageBox.showMultiChoice("CgRig Preference", parent=self.window(),
        ...
        engine.currentEngine().host.quit()
    ...
    def serialize(self):
        ...
        self.toolPaletteInterface.setLoadShelfAtStartup(self.autoLoadShelfCheckBox.isChecked())
    ...
    def uiConnections(self):
        ...
        # cgrig_preferences path changes
