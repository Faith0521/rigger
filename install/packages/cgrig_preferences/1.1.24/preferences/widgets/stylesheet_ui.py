...
from preferences.interface.preference_interface import CgRigToolsPreference
from cgrig.libs.pyqt import uiconstants as uic
from cgrig.apps.preferencesui import prefmodel
from cgrig.apps.preferencesui.example import examplewidget
from cgrig.libs.pyqt import utils, stylesheet
from cgrig.libs.utils import color, filesystem, output
...
from cgrig.apps.preferencesui.stylesheetui import stylesheetui_constants as sc

if env.isMaya():
    from cgrig.libs.maya.qt import cmdswidgets
...
class InterfaceWidget(prefmodel.SettingWidget):
    ...
    def __init__(self, parent=None, setting=None):
        ...
        self.currentCgRigInstance = api.currentConfig()
        self.prefInterface = preference.interface("core_interface")  # type: CgRigToolsPreference
        self.prefs = {}
        self.sheets = {}
    ...
    def _initUI(self):
        ...
        toolTip = "Admin save to {} for setting CgRig Defaults".format(InterfaceWidget.prefDataRelativePath)
        ...
        if self.currentCgRigInstance.isAdmin:
    ...
    def ui_adminSave(self):
        """Saves all themes to the stylesheet within cgrigtools_pro
    ...
    def buildGlobalColorsUI(self, themeDict):
        ...
        for i, (colorGroup, value) in enumerate(iter(sc.COLORGROUP_ODICT.items())):
            ...
            # Color picker
            colorPicker = elements.ColorBtn(text=colorGroup, parent=self,
                                            colorWidth=100, btnRatio=35, labelRatio=75)
            colorPicker.setColorSrgbInt(colorSrgbInt)
    ...
    def updateGlobalUIColorSwatches(self):
        ...
        for i, (colorGroup, value) in enumerate(iter(sc.COLORGROUP_ODICT.items())):
            ...
            self.globalColorWidgets[i].setColorSrgbInt(colorSrgbInt)  # set the widget color
    ...
    def adminSave(self):
        """ Admin save to /Preferences/prefs/global/stylesheet.pref for setting CgRig Defaults

        Will set all current settings as the CgRig Defaults for everyone
        ...
        data = self.prefInterface.preference.findSetting(InterfaceWidget.prefDataRelativePath, root=None)
        if data.isValid():
            cgrig_preferences_package_root = self.prefInterface.preference.packagePreferenceRootLocation(
                "cgrig_preferences")
            stylesheetAdminPath = cgrig_preferences_package_root / InterfaceWidget.prefDataRelativePath
    ...
    def buildSection(self, themeDict):
        ...
        for i, key in enumerate(themeDict.keys()):
            ...
            # Color
                ...
                colWidget = elements.ColorBtn(text=themeKeyToName(key), key=key,
                                              parent=self, colorWidth=100, labelRatio=65, btnRatio=35)
                colWidget.setColorSrgbInt(colorSrgbInt)
...
def applyStyleSheet(styleSheet, themeDict):
    ...
    themePref = coreinterfaces.coreInterface()
    for w in reversed(window.getCgRigWindows()):
...
def divideThemeDictToSections(themeDict):
    ...
    if copyThemeDict:  # then there are leftover keys, add them to the last list - otherSettingsDict
        logger.debug("CgRig Admin: Stylesheets 'stylesheetui_constants.py' is missing the key/s {}".format(copyThemeDict))
