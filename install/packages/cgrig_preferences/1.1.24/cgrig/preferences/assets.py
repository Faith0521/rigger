...
from cgrig.core import manager
from cgrig.core.tooldata.tooldata import DirectoryPath
from cgrig.libs.utils import filesystem
from cgrig.core.util import pathutils
from cgrig.preferences import core as prefcore
...
class AssetPreference(object):
    ...
    def cgrigPrefsPath(self):
        """ The assets folder specific to this user and for this preference.

        .. code-block::
            eg.
            /cgrig_preferences/assets/shaders
            /cgrig_preferences/assets/maya_scenes

        :return: Path to folder asset folder eg. /<..>/cgrig_preferences/assets/shaders
        """

        return pathutils.normpath(os.path.join(self.userAssetRoot(), self._assetFolder))

    def cgrigPrefsDefaultsPath(self):
        """ Path to default asset in cgrig_preference

        :return:
        :rtype:
        """
        return os.path.join(self.cgrigPrefsPath(), "defaults")

    def userAssetRoot(self):
        """ Full Path to the user assets folder  eg. <..>/cgrig_preferences/assets
    ...
    def defaultAssetsPath(self):
        """ The default assets saved in this repository (cgrig package)
    ...
    def currentPackagePath(self):
        ...
        For instance:
            "D:\\Code\\cgrigtoolspro\\cgrig_preferences"
            "D:\\Code\\cgrigtoolspro\\cgrig_pyside"
        ...
        for directory in filesystem.iterParentPath(classFile):
            cgrigPackage = os.path.join(directory, "cgrig_package.json")
            if os.path.exists(cgrigPackage):
                pkg = manager.currentConfig().resolver.packageFromPath(cgrigPackage)
                return pkg.root
...
class BrowserPreference(AssetPreference):
    ...
    def __init__(self, assetFolder, preferenceInterface, buildAssets=True, fileTypes=None, autoFillFolders=True,
        ...
        self._copyDefaults = copyDefaults and prefcore.currentInstance().shouldCopyDefaultAssets
        if buildAssets:
    ...
    def buildAssetDirectories(self):
        ...
        cgrigPrefs = self.cgrigPrefsPath()
        logger.debug("Building asset directories for: {}".format(cgrigPrefs))

        # If using old settings just add that to the preferences and call it a day
        if os.path.exists(cgrigPrefs) and \
                (self.hasOldAssets() or not self.autoFillFolders or self.assetFolderEmpty()) \
                and len(folderPaths) == 0:
            filesystem.ensureFolderExists(cgrigPrefs)
            self._initDefaultDirectory()
            return

        if not os.path.exists(cgrigPrefs):
            filesystem.ensureFolderExists(cgrigPrefs)
            self._initDefaultDirectory()
        ...
        if not self.hasOldAssets() and len(self.browserFolderPaths()) > 1:
            logger.debug("Removing base folder: {}".format(self.cgrigPrefsPath()))
            self.removeBrowserFolder(self.cgrigPrefsPath())
    ...
    def refreshAssetFolders(self, setActive=True, save=True):
        ...
        if self._copyDefaults:
            path = self.copyDefaultAssets(self.cgrigPrefsDefaultsPath())
            if path:
                dirs.append(path)
        ...
        if self.autoFillFolders:
            newFolders = [os.path.join(cgrigPrefs, f) for f in os.listdir(cgrigPrefs) if
                          os.path.isdir(
                              os.path.join(cgrigPrefs, f)) and "_fileDependencies" not in f and not f.startswith(".")]
            dirs += newFolders
        ...
        if os.path.exists(cgrigPrefs) and not len(dirs) > 0:
            self._initDefaultDirectory()
    ...
    def assetFolderEmpty(self):
        ...
        """
        cgrigPrefs = self.cgrigPrefsPath()
        return not len(os.listdir(cgrigPrefs)) > 0
    ...
    def _initDefaultDirectory(self, setActive=True):
        ...
        :return:
        """
        logger.debug("Initialize Default Directory: {}".format(self.cgrigPrefsPath()))
        d = DirectoryPath(path=self.cgrigPrefsPath())
    ...
    def hasOldAssets(self):
        ...
        onlyfiles = [f for f in os.listdir(cgrigPrefs) if os.path.isfile(os.path.join(cgrigPrefs, f))]
