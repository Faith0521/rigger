...
def attributeRefToSceneNode(rig, component, referenceExpr):
    ...
    :rtype: tuple[:class:`cgrig.libs.hive.base.component.Component`, :class:`zapi.DagNode`]
    """
    ...
    if len(parts) == 2:
        ...
        try:
            ...
            raise errors.InvalidDefinitionAttrExpression(referenceExpr)
    funcMap = {constants.INPUTLAYER_DEF_KEY: component.inputLayer,
               constants.OUTPUTLAYER_DEF_KEY: component.outputLayer,
               constants.RIGLAYER_DEF_KEY: component.rigLayer,
               constants.DEFORMLAYER_DEF_KEY: component.deformLayer,
               constants.GUIDELAYER_DEF_KEY: component.guideLayer}
    layerFunc = funcMap.get(layerType)
    if not layerFunc:
        raise errors.MissingMetaNode(layerType)
    sceneLayer = layerFunc()
    if returnLayer:
        return component, sceneLayer
    try:
        if layerType == constants.INPUTLAYER_DEF_KEY:
            try:
                outNode = sceneLayer.inputNode(nodeId)
            except errors.MissingControlError:
                outNode = sceneLayer.taggedNode(nodeId)
                if not outNode:
                    raise
            return component, outNode

        elif layerType == constants.OUTPUTLAYER_DEF_KEY:
            try:
                outNode = sceneLayer.outputNode(nodeId)
            except errors.MissingControlError:
                outNode = sceneLayer.taggedNode(nodeId)
                if not outNode:
                    raise
            ...
            try:
                outNode = sceneLayer.control(nodeId)
            except errors.MissingControlError:
                outNode = sceneLayer.taggedNode(nodeId)
                if not outNode:
                    raise
            return component, outNode
        elif layerType == constants.DEFORMLAYER_DEF_KEY:
            try:
                outNode = sceneLayer.joint(nodeId)
            except errors.MissingControlError:
                outNode = sceneLayer.taggedNode(nodeId)
                if not outNode:
                    raise
            ...
            try:
                outNode = sceneLayer.guide(nodeId)
            except errors.MissingControlError:
                outNode = sceneLayer.taggedNode(nodeId)
                if not outNode:
                    raise
...
def attributeRefToLayerNode(rig, component, referenceExpr):
    """Returns the scene node or the component from an attribute expression.
    ...
    try:
        ...
        if layerType == constants.INPUTLAYER_DEF_KEY:
            ...
            sceneLayer = component.deformLayer()
        elif layerType == constants.GUIDE_LAYER_TYPE:
            sceneLayer = component.guideLayer()
        return component, sceneLayer
    except (errors.MissingControlError, errors.InvalidInputNodeMetaData,
            errors.InvalidOutputNodeMetaData):
        logger.warning("Missing Node '{}' for expression {}".format(layerType, referenceExpr))
        return component, None
