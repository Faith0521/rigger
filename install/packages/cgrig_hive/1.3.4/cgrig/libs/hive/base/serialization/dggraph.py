...
from cgrig.libs.maya.api import plugs as apiplugs
...
class NamedDGGraph(object):
        ...
        for name, paths in inputs.items():
            for path in paths:
                try:
                    ...
                    logger.error("Incorrect path expression: {}".format(path))
                    raise
    ...
    @classmethod
    def create(cls, namedGraphData, inputConnections, outputConnections):
        ...
        inData = inputConnections or {}
        inData.update(namedGraphData.inputs())
        outData = outputConnections or {}
        outData.update(namedGraphData.outputs())
        inputs = NamedDGGraph.inputDataToGraph(createdNodes, inData)
        ...
        outputs = NamedDGGraph.outputDataToGraph(createdNodes, outData)

        return NamedDGGraph(
            namedGraphData.id,
            namedGraphData.name,
            namedGraphData.metaData,
            createdNodes,
            inputs,
            outputs,
        )
    ...
    def __init__(
        ...
        inputs,
        outputs,
        primaryInputNode=None,
        primaryOutputNode=None,
    ):
    ...
    def _connectInputsToNode(self):
        ...
        for inputName, plugs in self._inputs.items():
            ...
            try:
                ...
                maxValue = apiplugs.getPlugMin(plug.plug())
                minValue = apiplugs.getPlugMin(plug.plug())
                dataType = plug.apiType()
                if default:
                    default = apiplugs.getPythonTypeFromValue(dataType, default)
                if minValue:
                    minValue = apiplugs.getPythonTypeFromValue(dataType, minValue)
                if maxValue:
                    maxValue = apiplugs.getPythonTypeFromValue(dataType, maxValue)
                newAttr = self._primaryInputNode.addAttribute(
                    inputName,
                    Type=plug.apiType(),
                    value=plug.value(),
                    min=minValue,
                    max=maxValue,
                    default=default,
                    enums=apiplugs.enumNames(plug.plug()),
                )
            except:
                logger.error(
                    "Failed to add input node attribute for: {}".format(inputName, plug)
                )
                raise
            for plug in plugs:
    ...
    def _connectOutputsToNode(self):
        ...
        for outputName, plug in self._outputs.items():
            try:
                ...
                if maxValue:
                    maxValue = apiplugs.getPythonTypeFromValue(dataType, maxValue)
                newAttr = self._primaryOutputNode.addAttribute(
                    outputName,
                    Type=plug.apiType(),
                    value=plug.value(),
                    min=minValue,
                ...
                logger.error(
                    "Failed to add input node attribute for: {}".format(outputName, plug)
                )
                raise
    ...
    def createIONodes(self):
        ...
        if not self.hasPrimaryInputNode():
            self._primaryInputNode = zapi.createDG(
                "{}Inputs".format(self.name()), "network"
            )

        if not self.hasPrimaryOutputNode():
            self._primaryOutputNode = zapi.createDG(
                "{}Outputs".format(self.name()), "network"
            )
    ...
    def _reconnectInputsToScene(self, modifier):
        ...
        for inputName, internalNodePlugs in self._inputs.items():
            for plug in internalNodePlugs:
                ...
                if (
                    inputNodePlug is None
                    or inputNodePlug.node() != self._primaryInputNode
                ):
                    continue
    ...
    def delete(self, modifier=None):
        ...
        for n in self._nodes.values():
            if n is None or not n.exists():
                continue
            for source, destination in n.iterConnections(
                source=True, destination=False
            ):
                if destination.isLocked:
                    destination.lock(False)
    ...
    def addNode(self, nodeId, node):
        ...
        if nodeId in self._nodes:
            raise NamedNodeAlreadyExists(
                "Node by id: {} already exists!".format(nodeId)
            )
    ...
    def addInput(self, inputId, plug):
        ...
        if any(i == plug for i in self._inputs.get(inputId, [])):
            raise NamedInputAlreadyExists(
                "Input by id: {} already exists!".format(inputId)
            )
        if self.hasPrimaryInputNode():
            newAttr = self._primaryInputNode.addAttribute(
                inputId,
    ...
    def addOutput(self, outputId, plug):
        ...
        if outputId in self._outputs:
            raise NamedInputAlreadyExists(
                "Output by id: {} already exists!".format(outputId)
            )
        if self.hasPrimaryOutputNode():
            newAttr = self._primaryOutputNode.addAttribute(
                outputId,
    ...
    def connectFromOutput(self, outputId, destinations):
        """Connects from the graphs output Attribute to the list destination attributes.

        :param outputId:
        :type outputId: str
        :param destinations:
        :type destinations: list[:class:`zapi.Plug`]
        :return:
        :rtype:  bool
        """
    ...
    def serializeIO(self):
        ...
        if self._primaryInputNode:
            for existingAttr in self._primaryInputNode.iterExtraAttributes():
                for dest in existingAttr.destinations():
                    inputNodes.setdefault(dest.node(), []).append(
                        {"plug": dest, "key": existingAttr.partialName(False)}
                    )
        else:
        ...
        if self._primaryOutputNode:
            for existingAttr in self._primaryOutputNode.iterExtraAttributes():
                ...
                if sourcePlug is not None:
                    outputNodes[sourcePlug.node()] = {
                        "plug": sourcePlug,
                        "key": existingAttr.partialName(False),
                    }
        else:
            outputNodes = {
                p.node(): {"plug": p, "key": k} for k, p in self._outputs.items()
            }
        ...
        for nodeId, node in self._nodes.items():
            ...
            for v in hasInputs:
                inputs.setdefault(v["key"], []).append(
                    ":".join((nodeId, v["plug"].partialName()))
                )
            if hasOutput:
                outputs[hasOutput["key"]] = ":".join(
                    (nodeId, hasOutput["plug"].partialName())
                )
    ...
    def externalConnections(self):
        ...
        externalOutputs = []
        for nodeId, node in self._nodes.items():
            for destination, source in node.iterConnections(
                source=False, destination=True
            ):
                sourceNode = source.node()
    ...
    def serialize(self):
        ...
        :rtype: dict
        """
        data = {"id": self._graphId, "name": self._name, "metaData": self._metaData}
        nodeData = []
        connections = []
        listedNodes = {v: k for k, v in self._nodes.items()}
        ...
        for nodeId, node in self._nodes.items():
            ...
            d = node.serializeFromScene(
                skipAttributes=("message",),
                includeNamespace=False,
                includeConnections=False,
                useShortNames=True,
            )
            nData = {"id": nodeId, "data": d}
            for destination, source in node.iterConnections(
                source=False, destination=True
            ):
                sourceNode = source.node()
...
def createNamedGraph(layer, namedGraphData, inputConnections, outputConnections,track=True, createIONodes=False):
    ...
    :type layer: :class:`cgrig.libs.hive.base.hivenodes.HiveLayer`
    ...
    :rtype: :class:`NamedDGGraph`
    """
    newGraph = NamedDGGraph.create(namedGraphData, inputConnections, outputConnections)
    if not track:
        return newGraph
    ...
    if createIONodes:
        ...
        newGraph.primaryInputNode().message.connect(
            newElement.child(DGGRAPH_INPUTNODE_IDX)
        )
        newGraph.primaryOutputNode().message.connect(
            newElement.child(DGGRAPH_OUTPUTNODE_IDX)
        )
...
def namedGraph(layer, graphName, graphRegistry):
    ...
    :type graphRegistry: :class:`cgrig.libs.hive.base.registry.GraphRegistry`
    :returns: The named graph if it exists, otherwise None.
    :rtype: :class:`cgrig.libs.hive.base.serialization.NamedDGGraph` or None
    """
...
def namedGraphs(layer, graphRegistry):
    ...
    :param graphRegistry: The registry instance to look for the graph.
    :type graphRegistry: :class:`cgrig.libs.hive.base.registry.GraphRegistry`
    :rtype: list[:class:`cgrig.libs.hive.base.serialization.NamedDGGraph`]
    """
    for graphElement in layer.attribute(constants.DG_GRAPH_ATTR):
        if len(graphElement) == 2:
            return
...
def findNamedGraphs(layer, graphRegistry, names):
    ...
    :type names: list[str]
    :rtype: list[:class:`cgrig.libs.hive.base.serialization.NamedDGGraph` or None]
    """
    ...
    for graphElement in layer.attribute(constants.DG_GRAPH_ATTR):
        ...
        if sceneGraphName not in names:
            continue
        sceneGraphId = graphElement.child(DGGRAPH_ID_IDX).value()
        results[names.index(sceneGraphName)] = _namedGraphFromElement(
            graphElement,
            sceneGraphName,
            sceneGraphId,
            graphRegistry.graph(sceneGraphId),
...
def deleteNamedGraph(layer, graphName, graphRegistry, modifier=None):
    ...
    for graphElement in layer.attribute(constants.DG_GRAPH_ATTR):
        ...
        if sceneGraphName != graphName:
            continue
        sceneGraphId = graphElement.child(DGGRAPH_ID_IDX).value()
        graphInstance = _namedGraphFromElement(
            graphElement,
            sceneGraphName,
            sceneGraphId,
            graphRegistry.graph(sceneGraphId),
...
def _namedGraphFromElement(graphElement, graphName, graphId, graphData):
    ...
    for element in graphElement.child(DGGRAPH_NODES_IDX):
        nodeData[element.child(DGGRAPH_NODEID_IDX).value()] = element.child(
            DGGRAPH_NODE_IDX
        ).sourceNode()
