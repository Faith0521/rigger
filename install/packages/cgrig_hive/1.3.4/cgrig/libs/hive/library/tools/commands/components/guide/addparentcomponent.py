...
from cgrig.libs.hive import api, constants
from cgrig.libs.maya import zapi


class AddParentComponentFromSelectionCommand(command.CgRigCommandMaya):
    ...
    _drivenComponents = [] # type: list[api.Component]
    _driverComponent = []
    ...
    def doIt(self, driver=None, driven=None):
        ...
        for comp in self._drivenComponents:
            success = comp.setParent(driver[0],
                                     driver[1])
            for guide in comp.guideLayer().iterGuides():
...
class AddParentComponent(command.CgRigCommandMaya):
    ...
    def doIt(self, parentComponent=None, parentGuide=None, childComponent=None):
        ...
        for guide in childComponent.guideLayer().iterGuides():
            for srt in guide.iterSrts():
                srt.setLockStateOnAttributes(zapi.localTransformAttrs, True)
...
class RemoveAllParentsComponentFromSelectionCommand(command.CgRigCommandMaya):
