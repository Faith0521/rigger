...
------------------------ BATCH RIG EXPORT SETTINGS ----------------------------
Example settings used in the batch exporter.
...
.. code-block:: python
    ...
    # Set settings ------------------------------------
    fbxBatch.outputPath = ""  # often overridden by the batch functions
    fbxBatch.skeletonDefinition = True
    fbxBatch.constraints = False
    fbxBatch.tangents = True
    fbxBatch.hardEdges = False
    fbxBatch.smoothMesh = False
    fbxBatch.smoothingGroups = True
    fbxBatch.version = "FBX201800"
    fbxBatch.shapes = True
    fbxBatch.skins = True
    fbxBatch.lights = False
    fbxBatch.cameras = False
    fbxBatch.animation = False
    fbxBatch.startEndFrame = []
    fbxBatch.ascii = False
    fbxBatch.triangulate = True
    fbxBatch.includeChildren = True
    fbxBatch.axis = "Y"
    fbxBatch.includeScale = True # If True then scale attributes will be included in the export
    fbxBatch.meshes = True



------------------------ BATCH RIG EXPORT SIMPLE ----------------------------

Batch by specifying exactly what you need to export, useful for a pipeline. This is the simplest way to export.

.. code-block:: python

    # Batch by specifying exactly what you need to export, useful for a pipeline.
    from cgrig.libs.hive.library.exporters import fbxexporter
    fbxBatch = fbxexporter.fbxBatchExport()
    # Main Settings ------------------------------------
    namespace = "ref1"
    rigName = "cgrig_mannequin"
    startEndFrames = [(0, 45), (46, 90)]
    outPaths = [r"C:/Users/Username/Desktop/manne_walk", r"C:/Users/Username/Desktop/manne_jump"]
    # Set settings (see above for all settings) ------------------------------------
    fbxBatch.skeletonDefinition = True
    fbxBatch.shapes = True
    fbxBatch.skins = True
    ...
    fbxBatch.version = "FBX201800"
    # Export ------------------------------------
    fbxBatch.exportHiveRigNameAnimClips(rigName, startEndFrames, outPaths, namespace=namespace)
...
.. code-block:: python
    ...
    nameSuffix = ""  # "shot05", eg will name cgrig_mannequin_shot05.fbx
...
Rig names are the Hive rig name ie "cgrig_mannequin".
If there are multiple rigs/clashing names in the scene, add the namespace ie "m1:cgrig_mannequin"
...
.. code-block:: python
    ...
    rigNames = ["m1:cgrig_mannequin", "mannequinReg_rig_STRD:cgrig_mannequin", "r1:robot", ":cgrig_mannequin"]
    nameSuffix = "" # "shot05" eg cgrig_mannequin.fbx becomes cgrig_mannequin_shot05.fbx
...
.. code-block:: python
    ...
    # Settings ------------------------------------
    # If no rig clashes then the rig name does not need a namespace
    rigName = "cgrig_mannequin"  # if multiple clashing rigs use the namespace "m1:cgrig_mannequin"
    folderPath = r"C:/Users/Username/Desktop/"
    fileOverrideList = None  # can override the bookmark names eg ["walk", "run"]
    prefixRigName = True  # Adds the rig name to the start of the filename eg. cgrig_mannequin_bookmark01.fbx
    fbxBatch.skeletonDefinition = True
    fbxBatch.shapes = True
    fbxBatch.skins = True
    fbxBatch.animation = True
...
.. code-block:: python
    ...
    prefixRigName = True  # True will prefix the rig name to the file name eg. cgrig_mannequin_walk.fbx
    fbxBatch.skeletonDefinition = True
    fbxBatch.shapes = True
    fbxBatch.skins = True
    fbxBatch.animation = True
...
from cgrig.libs.maya.zapi import utils as zapiutils
...
from cgrig.libs.maya.cmds.animation import animbookmarks
...
UI_EXPORT_SETTINGS_NODE = "cgrigHiveExportFbxUiSettings"
...
class ExportSettings(object):
    ...
    def __init__(self):
        ...
        self.axis = "Y"
        # if True then all joints wil have their local scale reset to (1, 1, 1)
        self.resetJointScale = False
...
class FbxExporterPlugin(api.ExporterPlugin):
    ...
    def _exportAnimationRig(self, rig, exportOptions):
        ...
        if not exportOptions.takes:
            exportOptions.takes[files.FBX_DEFAULT_TAKE_NAME] = exportOptions.startEndFrame
        ...
        if not exportOptions.debugScene and os.path.exists(currentScenePath):
    ...
    def _exportBindPoseRig(self, rig, exportOptions):
        ...
        rootGeoTransform, success = self._validate(rig, exportOptions)
        if not success:
            return
        if rootGeoTransform:
        ...
        api.skeletonutils.cleanSkeletonBeforeExport(skelRoots, resetScale=exportOptions.resetJointScale,
                                                    onlyHiveAttrs=True)
    ...
    def validate(self, rig, exportOptions):
        info = api.ValidationRigInfo(rig.name())
        components = [comp for comp in rig.iterComponents()]
        # validate meshes
        if exportOptions.meshes:
            if not any(comp.hasSkeleton() for comp in components):
                info.append("Missing Skeleton", 2, "Missing Skeleton please build the skeleton before exporting.")
            _, rootTransform = self._rootGeoHrc(rig)
            if rootTransform is None:
                info.append("Missing Geometry", 2, NO_GEOM_MSG)
            else:
                for _ in rootTransform.iterChildren(recursive=True, nodeTypes=(zapi.kNodeTypes.kMesh,)):
                    ...
                    info.append("Missing Geometry", 2, NO_GEOM_MSG)
                meshSrts = []
                for mesh in rootTransform.iterChildren(recursive=True, nodeTypes=(zapi.kNodeTypes.kMesh,)):
                    transform = mesh.parent()
                    if transform not in meshSrts:
                        meshSrts.append(transform)
                geoWithHistory = zapiutils.findNonDeformerHistory(meshSrts)
                if geoWithHistory:
                    info.append("The below meshes contain non-deformer history,", 2,

                                "Please delete non deformer history.\n{}".format(",".join([i.name() for i in geoWithHistory])))
        return info

    def _validate(self, rig, exportOptions):
    ...
    def _exportAnimNoSkin(self, skeletonRoots, exportOptions):
        ...
        with zapi.tempNamespaceContext("tempNamespace"):
            ...
            api.skeletonutils.cleanSkeletonBeforeExport(rootJoints, constraints=True, onlyHiveAttrs=True,
                                                    resetScale=exportOptions.resetJointScale)
            self.onProgressCallbackFunc(30, "Cleaned skeleton data")
            try:
                displayLayers = zapi.displayLayers(default=False)
                cmds.delete(displayLayers)
            except RuntimeError:
...
def _exportFbx(nodeNames, exportOptions):
    ...
    files.exportFbx(exportOptions.outputPath, [i.fullPathName() for i in nodeNames if i],
                    ...
                    upAxis=exportOptions.axis,
                    ascii=exportOptions.ascii,
                    takes=exportOptions.takes
                    )
...
def strToRigListUI(rigNamesStr):
    ...
    :param rigNamesStr: A string of rig names " cgrig_mannequin,  robot ,  zanzi  "
    :type rigNamesStr: str
    :return: A list of rig names ["cgrig_mannequin", "robot", "zanzi"]
    :rtype: list(str)
    """
    if not rigNamesStr:
        return list()
...
def saveUIExportSettingsScene(uiSettingsDict):
    """Save the UI settings to the scene on a network node named "cgrigHiveExportSettings"
...
class fbxBatchExport(object):
    ...
    def rigInstance(self, rigName, namespace=""):
        ...
        :param rigName: The Hive name of the rig to export ie "cgrig_mannequin"
        :type rigName: str
    ...
    def exportHiveRigNames(self, rigNames, folderPath, fileOverrideList=None, nameSuffix="", message=True):
        ...
        :param nameSuffix: Adds a suffix to the name eg cgrig_mannequin.fbx becomes cgrig_mannequin_shot05.fbx
        :type nameSuffix: str
    ...
    def exportAnimBookmarksStartEnd(self, rigName, folderPath, fileOverrideList=None, prefixRigName=True, message=True):
        ...
        :type fileOverrideList: list(str)
        :param prefixRigName: Prefix the rig name to the file name? eg. cgrig_mannequin_0_101.fbx
        :type prefixRigName: bool
