...
from cgrig.libs.maya.api import scene, nodes
from maya import cmds

from maya.api import OpenMaya as om2


class SelectGuidePivotCommand(command.CgRigCommandMaya):
...
class SelectAllGuideShapesCommand(command.CgRigCommandMaya):
    ...
    def undoIt(self):
        if self._oldSelection.length() > 0:
            names = [nodes.nameFromMObject(self._oldSelection.getDependNode(i))
                     for i in range(self._oldSelection.length())]
            cmds.select(names, deselect=True)


class SelectGuideShapesCommand(command.CgRigCommandMaya):
    """Select's The component guide shape transforms
    """
...
class SelectGuideRootsCommand(command.CgRigCommandMaya):
