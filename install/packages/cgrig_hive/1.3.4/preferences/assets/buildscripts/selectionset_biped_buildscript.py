...
class SelectionSetBipedBuildScript(api.BaseBuildScript):
    ...
    def postPolishBuild(self, properties):
        ...
        """
        r = self.rig
        self.armFingerL_R = dict()  # armFingerL_R is a dict of armFinger sets, where the key is the side
        self.legsL_R = list()  # leg sets are used to parent the legsAll_sSet
        self.extraSets = list()
        ...
        if addBody:
            ...
            bodySSet = allSSet  # if no body set use the all set instead
        ...
        # parent arms to the armsAll set and parent to the bodySSet ---------------
        if self.armFingerL_R:
            # create the armsAll_sSet set
            armsAllSetStr = selectionsets.addSSetCgRigOptions("armsAll_sSet", [],
                                            icon="st_trianglePink",
                                            visibility=False,
                                            parentSet=str(bodySSet),
                                            soloParent=True)

            for key in self.armFingerL_R:
                selectionsets.parentSelectionSets([str(self.armFingerL_R[key])], armsAllSetStr)
        # parent legs to the legsAll set and parent to the bodySSet ---------------
        if self.legsL_R:  # list of zapi set nodes
            # create the legsAll_sSet set
            legsAllSetStr = selectionsets.addSSetCgRigOptions("legsAll_sSet", [],
                                                            icon="st_trianglePink",
                                                            visibility=False,
                                                            parentSet=str(bodySSet),
                                                            soloParent=True)
            for zapiSet in self.legsL_R:
                selectionsets.parentSelectionSets([str(zapiSet)], legsAllSetStr)
    ...
    def _filterToeComponents(self, v):
        ...
        :param v: dict of components keyed by side
        :type v: dict[str, list[zapi.Node]]
        :return: tuple of three dicts:
        :rtype: tuple[dict[str, list[zapi.Node]], dict[str, list[zapi.Node]], dict[str, list[zapi.Node]]]
        """
    ...
    def _solveToesComponents(self, v, selectionSetsMap):
        ...
        :param v: dict of toe components keyed by side
        :type v: dict[str, list[zapi.Node]]
        :param selectionSetsMap: dict of selection sets keyed by component serialized token key
        :type selectionSetsMap: dict[str, zapi.Node]
        :return: None
        :rtype: None
        """
    ...
    def _solveFingerComponents(self, v, selectionSetsMap, excludeMetaC):
        ...
        :param v: dict of finger components keyed by side
        :type v: dict[str, list[zapi.Node]]
        :param selectionSetsMap: dict of selection sets keyed by component serialized token key
        :type selectionSetsMap: dict[str, zapi.Node]
        :param excludeMetaC: if True, then fingersAndThumb_sSet will be created and used
        :type excludeMetaC: bool
        """
    ...
    def _solveClavComponents(self, v, selectionSetMap, componentGroups):
        ...
        :param v: dict of clavicle components keyed by side
        :type v: dict[str, list[zapi.Node]]
        :param selectionSetMap: dict of selection sets keyed by component serialized token key
        :type selectionSetMap: dict[str, zapi.Node]
        :param componentGroups: dict of components grouped by type and side
        :type componentGroups: dict[str, dict[str, list[zapi.Node]]]
        """
    ...
    def _createSets(self, components):
        ...
        :param components: list of components to create selection sets for
        :type components: list[zapi.Node]
        """
        self.legsL_R = list()  # store the leg sets for later use.
        for component in components:
            ...
            if "headcomponent" == type:
                ...
                self.legsL_R.append(sel)  # store the leg set for later use.
