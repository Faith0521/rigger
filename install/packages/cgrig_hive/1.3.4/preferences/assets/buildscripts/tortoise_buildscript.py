...
class TortoiseBuildScript(api.BaseBuildScript):
    ...
    def postPolishBuild(self, properties):
        ...
        for jnt in eyeJoints:
            ...
            cmds.disconnectAttr(cmds.listConnections("{}.scale".format(str(jnt)),
                                                     source=True,
                                                     plugs=True)[0],
                                "{}.scale".format(str(jnt))
                                )
        ...
        allSSet = selectionsets.addSSetCgRigOptions("all_sSet",
                                                  ...
                                                  icon="st_starYellow",
                                                  visibility=True)
        bodySet = selectionsets.addSSetCgRigOptions("body_sSet",
                                                  [],
                                                  icon="st_starOrange",
                                                  visibility=True,
                                                  parentSet=allSSet)
        faceSet = selectionsets.addSSetCgRigOptions("face_sSet",
                                                  [],
                                                  icon="st_circleRed",
                                                  visibility=True,
                                                  parentSet=allSSet)
        headNeckSet = selectionsets.addSSetCgRigOptions("headNeck_sSet",
                                                      [],
                                                      icon="st_squarePink",
                                                      visibility=False,
                                                      parentSet=bodySet)
        eyesSet = selectionsets.addSSetCgRigOptions("eyes_sSet",
                                                  [],
                                                  icon="st_squarePink",
                                                  visibility=False,
                                                  parentSet=faceSet)
    ...
    def preDeleteRig(self, properties):
        ...
        """
        eyeL = self.rig.component("eye", "L")
        eyeR = self.rig.component("eye", "R")
        for eyeComp in (eyeL, eyeR):
            if eyeComp is None:
                continue
            rigLayer = eyeComp.rigLayer()
            if rigLayer:
                eye0_L = rigLayer.control("fk00")
                eye0_L.attribute("scaleY").set(1.0)
            deformLayer = eyeComp.deformLayer()
            if deformLayer:
                eyeJnt_L = deformLayer.joint("fk01")
                eyeJnt_L.attribute("scaleY").set(1.0)
                eyeJnt_L.attribute("segmentScaleCompensate").set(1)
