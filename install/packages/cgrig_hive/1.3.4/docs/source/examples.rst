...
There are 2 ways to load a template.

#. Have hive create a new rig a load the template on to it.

.. code-block:: python

    # create rig from template by first retrieving the template path from the registry.
    # you can provide a path outside of the registry as well eg. a file path on a network share.
    # note only absolute paths are supported so make sure you resolve variables.
    templateReg = api.Configuration().templateRegistry()
    # provide the name of the template, the one displayed in the UI
    templatePath = templateReg.templatePath("allosaurus")
    rig, createdComponents = api.commands.loadTemplate(templatePath, name="MyRigName")

#. Load the template directly into an existing rig instance.

.. note:
    It's important to mention that the existing rig instance configuration including
    build scripts, naming convention etc take precedence over the template configuration provide.
    so only the components are merged in. This is to avoid any conflicts.

.. code-block:: python

    # append the components from the template by first retrieving the template path from the registry.
    # you can provide a path outside of the registry as well eg. a file path on a network share.
    # note only absolute paths are supported so make sure you resolve variables.
    templateReg = api.Configuration().templateRegistry()
    # provide the name of the template, the one displayed in the UI
    templatePath = templateReg.templatePath("allosaurus")
    rig, createdComponents = api.commands.loadTemplate(templatePath, rig=char)

Mirroring Components
--------------------

Mirroring components should be done on the root component of a chain eg. the clavicle not the arm.
The underlying behaviour will mirror all children at the same time.

.. code-block:: python

    # the tokenised names of components to find by name
    rootComponentNames= set(["clavical:L"])
    rootComponentToMirror = []
    for component in rig.iterComponents():
        if component.serializedTokenKey() in rootComponentNames:
            # world axis to mirror on it's possible to mirror on multiple axis via translate
            # but quite rare but hey it's there.
            # duplicate: false would mirror the component in place
            rootComponentToMirror.append({
                'side': 'R',
                'translate': ('x',),
                'rotate': 'yz', # plane to mirror
                'duplicate': True,
                'component': component
            })
    api.commands.mirrorComponents(rig, rootComponentToMirror)
...
With the below Commandline script make sure you change the paths for your setup.::
    ...
    set MAYA_MODULE_PATH=cgrigtoolspro/install/core/extensions/maya;
    set CGRIG_LOG_LEVEL=DEBUG
