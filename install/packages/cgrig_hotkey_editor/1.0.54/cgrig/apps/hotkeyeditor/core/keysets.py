...
from cgrig.apps.hotkeyeditor.core import const
from cgrig.apps.hotkeyeditor.core import hotkeys
from cgrig.apps.hotkeyeditor.core import utils

from cgrig.core.util import zlogging, classtypes, filesystem
...
class KeySetManager(object):
    ...
    def prefVersion(self):
        """ Get the version of the cgrig hotkeys found in the preferences
    ...
    def keySetNames(self, excludeMaya=False):
        ...
        for i, k in enumerate(self.keySets):
            ...
            # Skip the the defaultKeyset one since its our default CgRig_Tools_Default one
            ...
            # Otherwise Assumes that it is prefixed with "CgRig_User_"
    ...
    def _displayMessage(self, keySet):
        ...
        if name == "Maya_Default_CgRigMod":
            messageName = "Maya Default"
        elif name == "CgRig_Tools_Default":
            messageName = "CgRig Tools Default"
        else:
            messageName = name.replace("CgRig_User_", "")
    ...
    def nextKeySet(self):
        ...
        Usage:
            from cgrig.libs.maya.cmds.hotkeys import keysets
    ...
    def getRuntimeCommandNames(self):
        ...
        cgrigRTCs = self.defaultKeySet.getRuntimeCommandNames()

        if current is not None:
            userRTCs = current.getRuntimeCommandNames()

        return userRTCs, cgrigRTCs, mayaRTCs
    ...
    def newKeySet(self, name):
        ...
        # Copy the CgRig_Tools_Default key set
    ...
    def isKeySetLocked(self, keySetName=None):
        """ Checks if the keyset is one of the locked keysets. Ie. the default cgrig tools key sets
    ...
    def setupRuntimeCmd(self, selectedHotkey, runtimeCmdName, keySet=""):
        ...
        if mayaRTC is not None:
            ...
            # Runtime command in CgRig key set
            ui = dict(defaultRTC.cmdAttrs)
            rtcType = const.RTCTYPE_CGRIG
    ...
    def installed(self):
        """ Returns true if cgrig hotkeys are installed.
...
class KeySet(object):
    ...
    def setSource(self, source):
        ...
        eg Maya_Default, Maya_Default_CgRigMod, CgRig_Tools_Default
    ...
    def install(self, override=True):
        ...
        try:
            ...
            logger.debug("Creating hotkey set: {} with default cgrig source because current source({})"
    ...
    def setUpRuntimeCommand(self, selectedHotkey, runtimeCmdName, rtcType):
        ...
        if rtcType == const.RTCTYPE_MAYA or rtcType == const.RTCTYPE_CGRIG:
            # We only want to connect the name command to the runtime command for
            # Maya runtime commands and CgRig runtime commands
