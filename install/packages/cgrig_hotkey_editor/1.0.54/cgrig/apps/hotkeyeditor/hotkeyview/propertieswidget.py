# -*- coding: utf-8 -*-
from cgrigvendor.Qt import QtWidgets, QtGui

from cgrig.apps.hotkeyeditor.core import const as c, hotkeys
...
class HotkeyPropertiesWidget(QtWidgets.QGroupBox):
    def __init__(self, parent=None):
        """The properties widget that sits to the right inside the CgRig Hotkey Editor window
    ...
    def initWidgetsLayouts(self):
        """Creates the Widgets and Layouts for the properties section of the CgRig Hotkey Editor"""
    ...
    def getRuntimeCmdList(self):
        userRTCs, cgrigRTCs, mayaRTCs = self.keySetManager.getRuntimeCommandNames()

        ret = [""]
        ret.append("=== USER COMMANDS ===")
        ret += utils.sortedIgnoreCase(userRTCs) + ["", ""]
        ret.append("=== CGRIG COMMANDS ===")
        ret += utils.sortedIgnoreCase(cgrigRTCs) + ["", ""]
    ...
    def setPropertiesByHotkey(self, hotkey):
        ...
        # If command script is empty chances are it's a runtime command from CgRig
        if commandScript == "" or hotkey.runtimeType == c.RTCTYPE_CGRIG or hotkey.runtimeType == c.RTCTYPE_MAYA:
            ...
            if mayaRTC is not None:
                ...
                hotkey.runtimeType = c.RTCTYPE_CGRIG
    ...
    def runtimeCmdComboActivated(self):
        ...
        if text == c.RTCOMBO_USERCMDS or text == c.RTCOMBO_MAYACMDS or text == c.RTCOMBO_CGRIGCMDS:
            return
    ...
    def setupRuntimeCmdUi(self, text):
        ...
        if rtcType == c.RTCTYPE_MAYA or rtcType == c.RTCTYPE_CGRIG:
            # CgRig is not editable unless admin mode
            if not utils.isAdminMode():
