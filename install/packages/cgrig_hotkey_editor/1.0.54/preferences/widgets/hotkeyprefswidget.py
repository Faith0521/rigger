...
from cgrig.apps.hotkeyeditor.core import admin
...
class HotkeyPrefsWidget(prefmodel.SettingWidget):
    categoryTitle = "cgrig hotkey editor"
    ...
    def allWidgets(self):
        ...
        toolTip = "Install the CgRig Tools Pro hotkey sets, this will install... \n\n" \
                  "1. `{0}`: the main hotkey set, this will be the active hotkey set. \n" \
                  "2. `{1}`: the Maya Default set that can be toggled with the `;` hotkey. \n\n" \
                  "Custom/user hotkey sets will not be affected and CgRig Tools Pro hotkeys can be removed at \n" \
                  "anytime.".format(c.KEYSETS[0], c.KEYSETS[1])
        self.installHotkeysBtn = elements.styledButton(text="Install CgRig Hotkeys",
                                                       icon="keyboard",
                                                       parent=self,
                                                       toolTip=toolTip,
        ...
        toolTip = "Update the CgRig Tools Pro default hotkey sets while upgrading CgRig Tools Pro.\n" \
        ...
        self.updateHotkeysBtn = elements.styledButton(text="Update CgRig Hotkeys",
                                                      icon="reload",
                                                      parent=self,
                                                      toolTip=toolTip,
                                                      style=uic.BTN_DEFAULT,
                                                      minWidth=uic.BTN_W_REG_LRG,
                                                      maxWidth=uic.BTN_W_REG_LRG)
        toolTip = "Delete the two CgRig Tools Pro hotkey sets... \n\n" \
                  "1. `{0}` \n" \
                  "2. `{1}` \n\n" \
                  "Any custom/user hotkey sets will not be affected.".format(c.KEYSETS[0], c.KEYSETS[1])
        ...
        toolTip = "Save the userPrefs .mhk sets `{}` and `{}`\n" \
                  ...
                  "WARNING CgRig natively saves to the prefs JSON\n" \
        ...
        self.saveJSONBtn = elements.styledButton(text="Save JSON From MHK",
                                                 ...
                                                 minWidth=uic.BTN_W_REG_LRG,
                                                 maxWidth=uic.BTN_W_REG_LRG)
        toolTip = "Opens the CgRig Tools Pro `Hotkey Editor` window."
        self.openHotkeyEditorBtn = elements.styledButton(text="Open CgRig HK Editor",
                                                         icon="keyboard",
                                                         parent=self,
                                                         toolTip=toolTip,
                                                         style=uic.BTN_DEFAULT,
    ...
    def hotkeysInstalledShowHide(self):
        """Depending if 'CgRig_Tools_Default' hotkey set exists show/hide the GUI elements"""
        if self.keysetManager.installed():  # CgRig_Tools_Default hotkey set exists
            self.installHotkeysBtn.hide()
            self.updateHotkeysBtn.show()
            self.openHotkeyEditorBtn.show()
            self.deleteBtn.show()
            self.deleteAllBtn.show()
        else:  # CgRig_Tools_Default hotkey set does not exist
    ...
    def adminShowHide(self, checked):
        ...
        Admin mode is the env variable (bool)... os.environ["CGRIG_ADMIN"]
        """
        if checked:
    ...
    def adminChecked(self):
        ...
        if checked:
            ...
            output.displayInfo("Admin Mode is set to: `Off`. To return use `os.environ['CGRIG_ADMIN'] = '1'`")

    def saveMHKBtnClicked(self):
        """Saves the two user preferences .mhk files from the current Maya hotkey sets:

            "CgRig_Tools_Default.mhk" and "Maya_Default_CgRigMod.mhk"
    ...
    def saveJSONBtnClicked(self):
        """Saves the two user preferences .mhk files to the .json files:
            ...
            "CgRig_Tools_Default.json" and "Maya_Default_CgRigMod.json"
    ...
    def reloadDefaultHotkeys(self):
        """Re-installs (overrides) the CgRig Maya Hotkey Sets, inside of Maya:

            "CgRig_Tools_Default" and "Maya_Default_CgRigMod"

        With popup window ok/cancel
        """
        reinstall = elements.MessageBox.showOK(title="Update CgRig Hotkeys?",
                                               parent=None,
                                               message="Are you sure want to update the CgRig Tools Pro hotkey sets? \n"
                                                       "This may be desired if you have upgraded CgRig Tools Pro.\n\n"
    ...
    def installHotkeys(self):
        """Saves the CgRig Maya Hotkey Sets, inside of Maya:

            "CgRig_Tools_Default" and "Maya_Default_CgRigMod"

        With popup window ok/cancel.  Will override if the hotkey sets already exist in Maya
        """
        install = elements.MessageBox.showOK(title="Install CgRig Hotkeys?",
                                             parent=None,
                                             message="Install the CgRig Tools Pro hotkey sets, this will install... \n\n"
                                                     "1. `{0}`: the main hotkey set, this will be the active "
                                                     "hotkey set. \n"
                                                     "2. `{1}`: the Maya Default set that can be toggled with the "
                                                     "`;` hotkey. \n\n"
                                                     "Custom/user hotkey sets will not be affected and CgRig Tools Pro "
    ...
    def loadDefaults(self):
        ...
        output.displayInfo("Success: CgRig Tools Pro hotkeys are installed, and the "
                           "current hotkey set has been set to `CgRig_Tools_Default`")

    def copyFromDefaults(self):
        pass

    def deleteBtnClicked(self):
        """Deletes the CgRig Maya hotkey sets and also the userPreferences json and mhk files:
        ...
        delete = elements.MessageBox.showOK(title="Delete Hotkeys?",
                                            parent=None,
                                            message="Are you sure want to delete the two CgRig Tools Pro hotkey sets? \n\n"
                                                    "1. `{}` \n"
                                                    "2. `{}`\n\n"
        ...
        self.hotkeysInstalledShowHide()
        output.displayInfo("All CgRig Tools Pro default key sets have been deleted")

    def deleteAllHotkeys(self):
        delete = elements.MessageBox.showOK(title="Delete ALL Hotkeys?",
                                            parent=None,
                                            message="This will delete ALL cgrig hotkeys, "
                                                    "including any custom ones. Do you want to continue?")
        if not delete:
            return
        admin.deleteMayaKeySets()
        admin.deleteCgRigKeySets(deletePrefHotkeys=True)
        self.hotkeysInstalledShowHide()
        output.displayInfo("All CgRig Tools Pro hotkey sets have been deleted")

    def openCgRigHkEditor(self):
        """Opens the CgRig Hotkey Editor"""
        from cgrig.apps.toolpalette import run
        run.load().executePluginById("cgrig.hotkeyeditorui")
    ...
    def connections(self):
        ...
        self.openHotkeyEditorBtn.clicked.connect(self.openCgRigHkEditor)
