        ...
        if serialize_delta:
            ...
            for solver_name, solver_data in initial_solver_data.items():
                ...
                for pose_name, pose_data in poses.items():
                    ...
                    driven = copy.deepcopy(pose_data['driven'])
                    ...
                    if pose_index == 0:
                        ...
                        default_pose_driven_matrices = driven
                    else:
                        ...
                        new_driven = {}
                        for name, driven_transform in driven.items():
                            default_driven = om.MMatrix(default_pose_driven_matrices[name])
                            driven_transform = om.MMatrix(driven_transform)
                            new_driven[name] = list(driven_transform * default_driven.inverse())
    ...
    def deserialize(cls, data, solver_names=None, **kwargs):
        ...
        if load_delta:
            ...
            for solver_name, solver_data in rbf_data.items():
                ...
                current_driven_transforms = {d: om.MMatrix(utils.get_local_matrix_without_joint_orient(d))
                                             for d in solver_data['driven_transforms']}
        ...
        for solver_name, solver_data in rbf_data.items():
            solver_node = api.RBFNode.create_from_data(solver_data)
            driverNode = solver_node.drivers()[0]
            poses = list(solver_node.poses().keys())
            selected_rbf_pose_grp = f"{driverNode}_rbf_pose_grp"
            if not cmds.objExists("weight_rbf_grp"):
                cmds.createNode('transform', n="weight_rbf_grp")

            if not cmds.objExists(selected_rbf_pose_grp):
                selected_rbf_pose_grp = cmds.createNode('transform', n=selected_rbf_pose_grp, p="weight_rbf_grp")
                for pose_name in poses:
                    if not cmds.objExists(f'{selected_rbf_pose_grp}.{pose_name}'):
                        # 创建base pose
                        attributes.createAttribute(
                            selected_rbf_pose_grp, pose_name, attributeType="double", channelBox=True, nonKeyable=False, defaultValue=0,
                            minValue=0, maxValue=1, multi=False
                        )
                        attributes.lock_and_hide_default(selected_rbf_pose_grp)

            solver_node.setDrivenNode(selected_rbf_pose_grp, poses)
