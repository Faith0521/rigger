...
shape_types = []  # type: List[Type[ShapeProtocol]]


def register_shape_type(cls):
    shape_types.append(cls)
    return cls


def find_shape_type(type_name):
    for shape_type in node_types:
        if shape_type.type_name == type_name:
            return shape_type
    raise LookupError('can not find node type: {}'.format(type_name))


class ShapeProtocol(object):
    """
    Node的增删改查协议
    """
    type_name = 'unimplemented'  # type: str
    ...
    @classmethod
    def load(cls, pose, data):
        # type: (NodeProtocol, str) -> ShapeProtocol
        """反序列化Shape信息"""
        raise NotImplementedError
