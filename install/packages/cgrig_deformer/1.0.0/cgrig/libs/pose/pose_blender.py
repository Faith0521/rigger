...
class UEPoseBlenderNode(object):
    ...
    def rbf_solver(self):
        ...
        if exists:
            ...
            if connections:
                ...
                return api.RBFNode(connections[0])
    ...
    def out_matrix(self, transform_name=None):
        ...
        if transform_name:
            ...
            mx_decompose_node = cmds.createNode('decomposeMatrix', name="{node}_dmt".format(node=self.node))
    ...
    def add_pose_from_current(self, pose_name, index=-1):
        ...
        self.set_pose(
            index=index,
            pose_name=pose_name,
            matrix=utils.get_local_matrix_without_joint_orient(
                self.driven_transform
            )
        )
    ...
    def set_pose(self, index=-1, pose_name="", overwrite=True, matrix=None):
        ...
        if index < 0:
            if pose_name:
                cmds.warning("Set Pose has not been implemented to support a pose name. FIXME")
                return
            else:
                # If the index is less than 0 find the next available index
                index = len(cmds.getAttr(self.poses_attr, multiIndices=True) or [0]) - 1
        ...
        if matrix is None:
            matrix = utils.get_local_matrix_without_joint_orient(transform_name=self.driven_transform)
    ...
    def go_to_pose(self, index=-1):
        ...
        cmds.setAttr("{driven_transform}.translate".format(driven_transform=self.driven_transform), *translation)
        cmds.setAttr("{driven_transform}.rotate".format(driven_transform=self.driven_transform), *rotation)
        cmds.setAttr("{driven_transform}.scale".format(driven_transform=self.driven_transform), *scale)
