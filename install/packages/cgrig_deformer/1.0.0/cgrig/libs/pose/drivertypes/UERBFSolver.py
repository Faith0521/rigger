...
@Time: 11:17
@Description:
@FilePath: UERBFSolver.py
"""
from cgrig.libs.protocol import register_node_type, NodeProtocol
...
class RBFNode(NodeProtocol):
    ...
    def mirror(self, mirror_poses=True, mirror_rotation_axis='x', mirror_translation_axis='x'):
        """
        ...
        target_rbf_solver_name = naming.convertRLName(str(self))
        print(
        ...
        mirrored_transform_data = {
            ...
            'controllers': solver_data.pop('controllers')
        }
        ...
        for key, transforms in mirrored_transform_data.items():
            ...
            mirrored_transform_data[key] = [naming.convertRLName(transform) for transform in transforms]
        ...
        # Update the solver data with the new controller name
        solver_data['controllers'] = mirrored_transform_data.pop('controllers')
    ...
    def info(self):
        return "RBF Node : {}".format(self._node)
    ...
    def setDrivenNode(self, drivenNode, drivenAttrs):
        ...
        for index, dAttr in enumerate(drivenAttrs):
            pose_index = self.pose_index(dAttr)
            nodePlug = "{}.outputs[{}]".format(str(self), pose_index)
            drivenPlug = "{}.{}".format(drivenNode, dAttr)
            attrs_dict.append(dAttr)
            if cmds.objExists(drivenPlug):
                conneections = cmds.listConnections(str(self), plugs=True, destination=True) or []
                if drivenPlug in conneections:
                    cmds.disconnectAttr("{}.outputs".format(str(self)), drivenPlug)
                attrs_dict.append(cmds.getAttr(drivenPlug))
                cmds.connectAttr(nodePlug, drivenPlug, f=True)
    ...
    def go_to_pose(self, pose_name):
        ...
        if self.num_controllers():
            ...
            for controller_index, controller in enumerate(self.controllers()):
                ...
                cmds.xform(controller, matrix=matrices[controller_index], worldSpace=True)
        ...
        # if not controllers, set drivers
            ...
            for driver_index, driver in enumerate(self.drivers()):
                # TODO: check if translate, rotate and scale are free to change
                cmds.xform(driver, matrix=matrices[driver_index], objectSpace=True)
    ...
    def add_pose_from_current(self, pose_name, update=False):
        ...
        if self.num_controllers():
            ...
            for controller in self.controllers():
                matrix = cmds.xform(controller, q=True, matrix=True, worldSpace=True)
                controller_matrices.append(matrix)
    ...
    def mirror_pose(
        ...
        target_solver_name = naming.convertRLName(str(self))
        ...
        mirrored_controllers = [naming.convertRLName(controller) for controller in controllers]

        for index, controller in enumerate(controllers):
            target_controller = mirrored_controllers[index]
            rotate = cmds.getAttr("{controller}.rotate".format(controller=controller))[0]
            cmds.setAttr("{target_controller}.rotate".format(target_controller=target_controller), *rotate)
        cmds.refresh()
        transforms = self.driven_nodes(pose_blender.UEPoseBlenderNode.node_type)
        # Generate a list of the mirrored transforms based on the transforms we found
        mirrored_transforms = [naming.convertRLName(transform) for transform in transforms]
    ...
    def delete_pose(self, pose_name):
        ...
        print("Removing pose: '{pose_name}'".format(pose_name=pose_name))

        # Get the existing poses from the solver
        poses = self.poses()
        pose_index = self.pose_index(pose_name)

        # Disconnect all the blendshapes
    ...
    def edit_blendshape(self, pose_name, edit=False):
        ...
        for data in blendshape_data:
            ...
            if edit:
                ...
                self.delete_blendshape(pose_name=pose_name, delete_mesh=True)
            else:
    ...
    def isolate_blendshape(self, pose_name, isolate=True):
        """
        Will disconnect or reconnect all the blendshapes that aren't directly controlled by this pose
    ...
    def mirror_blendshape(self, pose_name, mirror_mapping):
        ...
        if not blendshape_transforms:
            print("Unable to find blendshape associated with '{pose_name}' pose".format(pose_name=pose_name))
            return
        ...
        current_drivers = [cmds.xform(driver, query=True, translation=True, objectSpace=True) for driver in
                           self.drivers()]
        mirrored_drivers = [cmds.xform(driver, query=True, translation=True, objectSpace=True) for driver in
                            new_solver.drivers()]
