...
This page shows how to setup a connection to CgRig Tools on GitLab .
...
read the `Code Overview Page. <https://create3dcharacters.com/maya-cgrig-tools-code-overview/>`_
...
You must request git access to CgRig Tools Pro 2.
Please email us if you are interested at info@create3dcharacters.com, or contact us through the CgRig Discord Channel .
...
We use `GitLab.com <https://gitlab.com/>`_ to host the dev copy of CgRig Tools Pro.
...
#. Clone the dev installer via the below command.
    .. tabs::
        .. group-tab:: SSH
            ...
            git clone git@gitlab.com:cgrigtoolspro/cgrig_dev_download.git

        .. group-tab:: HTTPS

            git clone https://gitlab.com/cgrigtoolspro/cgrig_dev_download.git

#. Download the zip file of the repository.::

    https://gitlab.com/cgrigtoolspro/cgrig_dev_download/-/archive/main/cgrig_dev_download-main.zip



This downloaded folder contains our dev installer script
Now run the below shell script in the cgrig_dev_download folder for you're operating system.
...
#. Specify the install path. We recommend installing outside of the cgrig_dev_download folder to separate git folders.
...
#. Final Option Sets up Autocompletion for PySide2 and other modules in our cgrigvendor dependencies.
...
Optional Set Custom cgrig_preferences Location
--------------------------------------------
The CgRig Tools preferences are automatically created in a default location such as documents/cgrig_preferences.

You can change this default location with the file preference_roots.config.

The path should already exist and contain the folder cgrig_preferences Eg.::

    {"user_preferences": "D:/SomePath/cgrig_preferences"}


Loading CgRig Tools in Maya
=========================

This video shows how to use a Windows .BAT file to create a CgRig Dev environment and connect Maya to the CgRig Tools Pro module.
...
We’ll use a new Maya preferences directory so that CgRig can be run clean within it’s own environment,
this keeps the dev version separated from your regular Maya environment (usually found inside documents/maya).

The folder /cgrigtoolspro/mayaPrefs can be the new location for Maya’s preferences when you’re using CgRig Dev.

Maya .BAT To Start The CgRig Dev Environment
In the folder /cgrigtoolspro/bin you’ll edit a .BAT file (only for Windows) to start Maya and point it to CgRig.

In PyCharm edit the file maya2022.bat::

    set MAYA_APP_DIR=D:\somePath\repositories\cgrigtoolspro\mayaPrefs
    set MAYA_MODULE_PATH=D:\somePath\repositories\cgrigtoolspro\cgrig\install\core\extensions\maya
    set CGRIG_ADMIN=1
...
You can run this icon to load the new Maya Environment copy of Dev CgRig Tools Pro.
...
Note that the dev copy can break more than the official release versions of CgRig,
we are working on it live! Keep in touch with us on the CgRig Discord or via email
