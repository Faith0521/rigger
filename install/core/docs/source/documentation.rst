...
CgRig Tools allows for generating a single static html page for all of CgRig tools including custom packages.
...
You can install these libraries using pip, cgrig tools docs should be compatible with the latest version.::
...
To generate rst files for one or multiple packages::
     ...
     cgrigtoolsRoot/install/core/bin/cgrig_cmd generateDocs --packages myPackageA myPackageB --apiDocs
...
use the flag "--apiDocsOutputName test"::
     ...
     cgrigtoolsRoot/install/core/bin/cgrig_cmd generateDocs --packages myPackage --apiDocs --apiDocsOutputName test

To generate rst files for all packages for production use::

     cgrigtoolsRoot/install/core/bin/cgrig_cmd generateDocs --packages * --apiDocs
...
Once you've setup your packages and generated the rst files(if need be) you just run the below::
    ...
    cgrigtoolsRoot/install/core/bin/cgrig_cmd generateDocs --packages * --build --launch

Setting up a package for documentation
--------------------------------------

To setup documentation for a package you first need to tell cgrigtools where in your package
your docs are located.

First open your cgrig_package.json and add the documentation entry like the example below.
Note you should change the paths to match your package.

Example cgrig_package.json entry for specifying custom documentation.
...
+------------------+----------------------------------------------------------------------+
| sourceCodeFolder |  The path to the source code ie. {self}/cgrig                          |
+------------------+----------------------------------------------------------------------+
...
play, this script is run after our internal cgrig sphinx config is run this means you have the ability to
...
We've already mocked all Qt modules you shouldn't need to do these. If you include cgrig_maya when generating
documentation then all maya modules we also be mocked already for you.

Python Docstring examples
-------------------------

CgRig tools uses reStructured Text formatting for a docstring. The below provides some basic examples on formatting
options.
...
.. code-block:: python
    ...
    def testFuncDocstring(firstArg, **kwargs):
        ...
        **Bold** Text
        ...
        .. figure:: https://create3dcharacters.com/cgrig-dev-documentation/_images/hiveartistui.png
            :align: center
