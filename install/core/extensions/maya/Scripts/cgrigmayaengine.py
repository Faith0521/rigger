...
class MayaEngine(engine.Engine):
    ...
    def postEnvInit(self):
        ...
        # pre load all markingmenu and hive registry, metanode registry to avoid maya security error horse shit
        from cgrig.libs.maya.markingmenu import menu
        from cgrig.libs.maya.meta import base as metabase
        menu.Registry()
        metabase.MetaRegistry()
    ...
    def _createMenuAndShelf(self):
        ...
        try:
            ...
            if self.host.isHeadless:
                logger.debug("Not in maya.exe skipping cgrigtools menu boot")
            else:
                from cgrig.apps.toolpalette import run
                run.load(applicationName="maya")
            logger.debug("Finished Loading CgRig Tools PRO")
            om2.MGlobal.displayInfo("CgRig Tools PRO")

        except Exception as er:
            logger.error("Failed To load CgRig Tools PRO due to unknown Error",
                         exc_info=True)
            om2.MGlobal.displayError("Failed to start CgRig Tools PRO\n{}".format(er))

    def shutdownEngine(self):
        """Implemented by derived class"""
        # delete the artist palette which will delete the cgrigtools menu and shelf, windows etc.

        om2.MGlobal.displayInfo("Unloading CgRig Tools PRO, please wait!")
