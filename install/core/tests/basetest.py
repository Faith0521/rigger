...
class BaseTest(unittest.TestCase):
    ...
    def _runSetup(cls, batFile):
        tempDir = tempfile.mkdtemp("cgrigtoolstests")
        cls._pathsToRemove.add(tempDir)
        projectFolder = os.path.join(tempDir, "cgrigtoolspro")
        mayaFolder = os.path.join(tempDir, "maya")
        env = os.environ.copy()
        env["CGRIG_PYTHON_INTERPRETER"] = sys.executable
        process = subprocess.Popen(" ".join([batFile,
                                   ...
                                   close_fds=True)
        process.communicate()
        os.environ["CGRIGTOOLS_ROOT"] = projectFolder
    ...
    def setUpClass(cls):
        ...
        # temp/cgrigtolsndjnvjsb
        #           - Maya
        #           - cgrigtoolspro
        #               - config
        #               - install
        root = os.path.dirname(os.path.dirname(__file__))
        batFile = os.path.join(root, "bin", "cgrig_cmd.bat")
        ...
        pythonPath = os.path.join(tmpFile, "cgrigtoolspro", "install", "core", "python")
