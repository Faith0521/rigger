...
from cgrig.core.util import git
...
try:
    from cgrig.core.util.git import gitwrapper
...
class GitDescriptor(descriptor.Descriptor):
    ...
    def install(self, **arguments):
        ...
        if self.config.resolver.packageForDescriptor(self) is not None:
            return
        # grab a temp folder
        localFolder = tempfile.mkdtemp("cgrig_git")
        ...
        if package is None:
            shutil.rmtree(localFolder, onerror=_handleDeleteError)
            raise ValueError("Git Repo is not a cgrigtools repo cancelling!")
        ...
        if self.force:
            self._descriptorDict["type"] = "cgrigtools"
