...
from cgrig.core.commands import action
from cgrig.core.util import filesystem
from cgrig.core import manager, errors, constants
...
class Setup(action.Action):
    """
    This 'setup' Command installs cgrigtools core package into the specified
    ...
    def arguments(self, argParser):
        argParser.add_argument("--destination",
                               required=True,
                               type=str,
                               help="The destination for cgrigtools to be installed too")
        argParser.add_argument("--zip",
                               required=False,
                               help="Alternative cgrigtools installation to use",
                               type=str)
    ...
    def run(self):
        ...
        if os.path.exists(destination):
            ...
            if not self.options.force:
                raise ValueError("CgRigProject Already exists at: {}".format(destination))
        ...
        if self.options.zip:
            ...
            try:
                self._buildCgRigFolderStructure()
            except OSError:
    ...
    def _buildCgRigFolderStructure(self):
        ...
        installedCfg = manager.cgrigFromPath(self.options.destination)
