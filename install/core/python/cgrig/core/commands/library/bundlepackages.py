...
class BundlePackages(action.Action):
    ...
    def run(self):
        ...
        for packageName, rawDescriptor in loadedEnv.items():
            ...
            if descriptor.type != descriptor.CGRIGTOOLS:  # todo: temp
                logger.error("Only descriptor types: {} supported when bundling".format(descriptor.CGRIGTOOLS))
        ...
        installDir = os.path.join(tempDir, "cgrigtoolspro")
        ...
        try:
            ...
            newConfig = manager.cgrigFromPath(installDir)
            ...
            # now zip up the full cgrigtoolspro root folder
            #
            ...
            logger.error("Failed to setup and cache cgrigtools",
                         exc_info=True)
            return False
        finally:
