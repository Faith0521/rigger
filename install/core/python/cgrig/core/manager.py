...
from cgrig.core.commands import loader, action
from cgrig.core.packageresolver import resolver
from cgrig.core.descriptors import descriptor
from cgrig.core import constants
from cgrig.core.util import env, filesystem
from cgrig.core.plugin import pluginmanager, coreplugin
...
class CgRig(object):
    """CgRig class is the main entry point for operating on any part of cgrig tools.

    :param cgrigPath: The root location of cgrigtools root folder ie. the one above the installation folder.
    :type cgrigPath: str
    :raise: :class:`errors.FileNotFoundError`
    """

    def __init__(self, cgrigPath):
        """ Initializes the default paths for cgrigtools

        Raises :class:`errors.FileNotFoundError` in the case where the provided
        pat doesn't exist.
        """
        if not os.path.exists(cgrigPath):
            raise errors.FileNotFoundError(cgrigPath)
        cgrigPaths = _resolveCgRigPathsFromPath(cgrigPath)
        logger.debug("Initializing cgrigtools from cgrigPath: {}: {}".format(cgrigPath, cgrigPaths),
                     extra=cgrigPaths)
        self._rootPath = cgrigPaths["root"]  # type: str
        self._corePath = cgrigPaths["core"]  # type: str
        self._pyFolderPath = cgrigPaths["python"]  # type: str
        self._configPath = cgrigPaths["config"]  # type: str
        self._packagesPath = cgrigPaths["packages"]  # type: str
        self._resolver = resolver.Environment(self)
        self._buildVersion = self._cacheBuildVersion()
        ...
        # cgrig tools core plugins for extending startup and other behaviour
        ...
        self._descriptorLib = descriptor.DescriptorRegistry(self, [descriptor.Descriptor],
                                                            variableName="id")
        self._descriptorLib.registerByEnv("CGRIG_DESCRIPTOR_PATH")
    ...
    def coreVersion(self):
        ...
        :rtype: str
        """
        package = os.path.join(self._corePath, "cgrig_package.json")
        packageInfo = filesystem.loadJson(package)
        return packageInfo["version"]

    def buildVersion(self):
        """Returns the CgRig Tools build version string.
    ...
    def _cacheBuildVersion(self):
        """Internal method to retrieve the build version, runs once on initialization.
        """
    ...
    def buildPackagePath(self):
        """Returns the fullPath to the cgrig_package.json which is the build package.

        :return: str
        """
        return os.path.join(self._rootPath, "cgrig_package.json")
    ...
    def isAdmin(self):
        ...
        """
        try:
            return bool(int(os.getenv(constants.CGRIG_ADMIN, "0")))
        except TypeError:
            return False

    @isAdmin.setter
    def isAdmin(self, state):
        """Sets whether CgRigtools is in Admin mode for the current instance.
        ...
        :type state: bool
        """
        logger.info("CgRig Admin mode has been set to: {}".format(state))
        os.environ[constants.CGRIG_ADMIN] = str(int(state))
    ...
    @property
    def rootPath(self):
        """Returns the root path of cgrigtools.

        The root path directory is the folder above the installation folder

        :return: The root folder of cgrigtools.
        :rtype: str
        """
    ...
    def configPath(self):
        ...
        :return: The config folder for cgrigtools.
        :rtype: str
        """
    ...
    @property
    def corePath(self):
        """The core folder of cgrigtools which is relative to the root under the
        installation folder.

         The core folder houses internal logic of cgrigtools package management.
    ...
    def packagesPath(self):
        """Returns the package repository path under the cgrigtools/install folder.
    ...
    def commandLibrary(self):
        ...
        Only one is created per CgRig session
    ...
    def cacheFolderPath(self):
        """Returns the current CgRig tools cache folder.
    ...
    def sitePackagesPath(self):
        """Returns the site packages folder path where cgrigtools installs pip packages when needed.
    ...
    def descriptorFromDict(self, descriptorDict):
        """Helper method which calls for :func:`cgrig.core.descriptors.descriptor.descriptorFromDict`

        :param descriptorDict: See :func:`cgrig.core.descriptors.descriptor.descriptorFromDict` for more info
        :type descriptorDict: dict
        :return: The corresponding Descriptor instance
        :rtype: :class:`cgrig.core.descriptors.descriptor.Descriptor`
        """
    ...
    def descriptorForPackageName(self, name):
        ...
        :param name: The cgrig package name to find.
        :type name: str
        :return: The descriptor or None
        :rtype: :class:`cgrig.core.descriptors.descriptor.Descriptor` or None
        """
    ...
    def descriptorsForPackageNames(self, *names):
        ...
        :param names: The cgrig package names to find.
        :type names: tuple[str]
        :return: The descriptor or None
        :rtype: list[:class:`cgrig.core.descriptors.descriptor.Descriptor` or None]
        """
    ...
    def shutdown(self):
        """Shutdown Method for cgrigtools excluding Host application plugin related code.
        """
        engine.shutdownEngine()
        self.resolver.shutdown()
        # clear out the sys.modules of all cgrig modules currently in memory
        from cgrig.core.util import flush
        flush.reloadCgRig()
        from cgrig.preferences import core
        flush.flushUnder(os.path.dirname(core.__file__))
        setCurrentConfig(None)

    def reload(self):
        """Reloads all of cgrigtools python packages, libraries, environment variables.

        :return: A New CgRig Manager instance.
        :rtype: :class:`CgRig`
        """
        root = self.rootPath
        self.shutdown()
        cfg = cgrigFromPath(root)
        cfg.resolver.resolveFromPath(cfg.resolver.environmentPath())
        return cfg


# global storage of cgrig config,
...
_CGRIG_CONFIG_CACHE = None


def currentConfig():
    """Returns the :class:`CgRig` instance currently initialized globally.

    If this func returns None call :func:`cgrigFromPath`.

    :return: The currently initialized :class:`CgRig`
    :rtype: :class:`CgRig` or None
    """
    global _CGRIG_CONFIG_CACHE
    return _CGRIG_CONFIG_CACHE


def setCurrentConfig(config):
    """Sets the :class:`CgRig` global instance.

    :warning: This overrides the global instanced :class:`CgRig` instance.

    If you need to run multiple instances in parallel use :func:`cgrigConfig`
    """

    global _CGRIG_CONFIG_CACHE
    _CGRIG_CONFIG_CACHE = config


def cgrigFromPath(path):
    """Returns the cgrigtools instance for the given root path.

    :param path: The root path of cgrigtools to initialize, \
    See :class:`CgRig` for more information.
    :type path: str
    :return: The cgrig instance.
    :rtype: :class:`CgRig`
    """
...
def _resolveCgRigPathsFromPath(path):
    """ Internal function which resolves the cgrigtools main paths

    Resolves "root","config","core","packages"
    :todo  config, package folders to be part of the environment and applied here.
    """
    logger.debug("Resolving cgrigtools from supplied path: {}".format(path))
    ...
    if os.path.exists(installFolder):
        ...
        if os.path.exists(walkedInstallFolder):
            ...
            # case initialize cgrigtools without the folders at all
            outputPaths["config"] = ""
    ...
    env.addToEnv(constants.COMMANDLIBRARY_ENV,
                 [os.path.join(outputPaths["python"], "cgrig", "core", "commands", "library")])
    env.addToEnv(constants.CGRIG_DESCRIPTOR_PATH,
                 [os.path.join(outputPaths["python"], "cgrig", "core", "descriptors", "plugins")])
    return outputPaths


def _patchRootPath(rootPath):
    """Patch for cgrigtools preferences path with the use of "~" where python now prioritizes USERPROFILE
    over HOME which happens to affect maya 2022 and below which sets the USERPROFILE to ~/Documents but
    maya 2023 is set to ~/ so this patches it globally across cgrig only if "~" is the first character and
