...
from cgrig.core.packageresolver import package
from cgrig.core.util import filesystem, zlogging
...
class Environment(object):
    """
    :param cgrigConfig: The cgrig configuration instance
    :type cgrigConfig: :class:`cgrig.core.manager.CgRig`
    """

    def __init__(self, cgrigConfig):
        self.cgrigConfig = cgrigConfig
        self.cache = {}  # type: dict[str, package.Package]
        self.callbacks = {} # type: dict[str, list[callable]]

    def _environmentPath(self):
        """Internal function which handles discovery of the environment path for cgrig configuration.

        :return: The Path object representing the absolute path.
        :rtype: :class:`str`
        """
        # physical path to users custom specified config json file.
        definedPath = os.path.expandvars(os.path.expanduser(os.getenv(constants.CGRIG_PKG_VERSION_PATH, "")))
        ...
        configFile = os.getenv(constants.CGRIG_PACKAGE_VERSION_FILE, "")

        definedPath = os.path.join(self.cgrigConfig.configPath, "env", configFile)
        if os.path.isfile(definedPath):
            logger.debug("Loading package environment configuration from: {}".format(definedPath))
            return definedPath
        definedPath = os.path.join(self.cgrigConfig.configPath, "env", "package_version.config")
        logger.debug("Loading package environment configuration from: {}".format(definedPath))
        return definedPath
    ...
    def createEnvironmentFile(self, env=None):
        ...
        Location of the environment file::
            ...
            cgrigRootPath/config/env/package_version.config

        :return: True if the environment file was created
        :rtype: bool
        """
        env = env or {}
        configPath = os.path.join(self.cgrigConfig.configPath,
                                  "env",
    ...
    def resolve(self, requestsInfo, apply=True, runCommandScripts=True):
        ...
        for packageName, rawDescriptor in requestsInfo.items():
            ...
            pkgDescriptor = self.cgrigConfig.descriptorFromDict(rawDescriptor)
        ...
        _patchReloadCgRigNamespacePackage()
        if not apply:
            return resolved
        sitePackagesFolder = self.cgrigConfig.sitePackagesPath()
        ...
        # just before cgrigtools fully loads but contains all packages ie. copy preferences etc
        visited = set()
    ...
    def packageFromPath(self, path):
        ...
        The path can either be the directory containing the package or the cgrig_package.json file.

        :param path: The directory or the cgrig_package.json absolute path
        :type path: str
        :return:
        :rtype: :class:`Package`
        """
    ...
    def _searchForPackage(self, packageName, packageVersion):
        return glob.glob(os.path.join(self.cgrigConfig.packagesPath,
                                      packageName,
                                      str(packageVersion),
...
def _patchReloadCgRigNamespacePackage():
    """Patch function to reload cgrig namespace after all current packages have been loaded, this
    is to work around limitations with pythons import system since we dynamically build sys.path
    with this lib which also contains the cgrig namespace.
