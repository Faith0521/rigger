"""Helper utilities for manipulating a CgRig package changelog build for RST/Sphinx.

CgRig ChangeLog has a certain syntax which we'll outline here.
...
Example Changelog Output::
    ...
    Added - (docs) Pycharm setup documentation for cgrig tools.
    Added - (docs) sphinxcontrib.youtube thirdparty dependency when build docs to support youtube and vimeo links.

    Bug Fixes
    ~~~~~~~~~
    BugFix - (MayaPlugin) Fix FileNotFoundError being raised due to incorrect cgrig initialization order.

"""

from collections import OrderedDict
from cgrigvendor.version import LooseVersion
import re
...
MESSAGE_TOKEN = "-"
MESSAGE_FORMAT = "- ({}) {}"
ACCEPTED_CATEGORIES = ("added", "change", "updated", "enhancement", "bug", "removed", "remove", "misc")
...
class ChangelogCategory(object):
    ...
    def sortMessages(self):
        ...
        :rtype: list[:class:`ChangeMessage`]
        """
        return sorted(self.messages, key=lambda x: x.label.lower())
    ...
    def asRstLines(self):
        ...
        body = [self.label, CATEGORY_UNDERLINE_TOKEN[0] * len(self.label), ""]
        if self.sorted:
...
def _parseLines(lines):
    ...
    for lineno, line in enumerate(lines):
        ...
        # category
            ...
            categoryName = parseCategory(lines[lineno - 1].strip())
            if categoryName not in ACCEPTED_CATEGORIES:
                currentCategory = None
                continue
            currentCategory = ChangelogCategory(currentVersion, categoryName)
            currentVersion.categories.append(currentCategory)
        # commit message
        elif token == MESSAGE_TOKEN and currentCategory:
...
def parseCategory(cat):
    ...
    """
    category = cat.lower()
    if "bug" in category or category.startswith("fix"):
        return "bug"
    elif category.startswith("ch"):
        return "change"
    elif category.startswith("add"):
        return "added"
    # Igore was a typo in git commits
    elif category.startswith("ignore") or category.startswith("Igore"):
        return "ignore"
    return category
