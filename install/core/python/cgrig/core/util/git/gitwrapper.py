"""CgRig tools wrapper around gitpython with a strong focus cgrig package release workflows.
"""
...
from cgrig.core.util import changelog, zlogging
...
class GitRepo(object):
    """ Wrapper around the gitpython Repo class to provide CgRig related operations.
...
def tagToChangeLog(repo, previousTag, toTag, version=None, change=None):
    ...
    for commit in repo.commitMessagesSinceTag(previousTag, toTag):
        ...
        for i in range(0, len(splitMessage), 2):
            ...
            if category not in changelog.ACCEPTED_CATEGORIES or category == "ignore":
                continue
