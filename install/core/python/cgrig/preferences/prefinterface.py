...
from cgrig.core.util import typing

from cgrig.core.util import zlogging
from cgrig.preferences import constants, errors

logger = zlogging.getLogger(__name__)

if typing.TYPE_CHECKING:
    import cgrig.core.tooldata.tooldata.SettingObject
    import cgrig.preferences.core.PreferenceManager


class PreferenceInterface(object):
    """Preference interface class which is responsible for interfacing to .pref files within cgrig.

    PreferenceInterface shouldn't be instanced directly but through
    :class:`cgrig.preferences.core.PreferenceManager` instance.

    It's the intention for interface subclasses to handle creating and manipulating one
    or more .pref belonging to the installed cgrig package.
    ...
    The .pref internal data structure may change over the life of cgrigtools so it is
    ...
    See :class:`preference.interface.preference_interface.CgRigToolsPreference` for
    example usage.
    """
    ...
    def __init__(self, preference):
        """
        :param preference: The main cgrig preference manager
        :type preference: :class:`cgrig.preferences.core.PreferenceManager`
        """
        self.preference = preference  # type: cgrig.preferences.core.PreferenceManager
        self._revertSettings = None  # type: cgrig.core.tooldata.tooldata.SettingObject
